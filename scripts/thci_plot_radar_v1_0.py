# -*- coding: utf-8 -*-
"""Plot deterministic THCI v1.0 six-axis radar charts.

The script reads precomputed THCI axis scores only. It does not recalculate
scores and does not call any runtime LLM.
"""
...
AXIS_SCORE_ROOT = PROJECT_ROOT / "outputs" / "thci_axis_scores_v1_0"
OUT_ROOT = PROJECT_ROOT / "outputs" / "thci_radar_v1_0"
...
def load_axis_scores(case_id: str) -> tuple[pd.DataFrame, Path, Path]:
    """Load one case's precomputed THCI axis score CSV."""
    case_dir = AXIS_SCORE_ROOT / case_id
    score_csv = case_dir / f"{case_id}_thci_axis_scores_v1_0.csv"
    score_summary_json = case_dir / f"{case_id}_thci_axis_score_summary_v1_0.json"
    ...
    return df, score_csv, score_summary_json
...
def validate_axis_scores(score_df: pd.DataFrame) -> dict[str, Any]:
    ...
    numeric_values = [v for v in axis_scores.values() if v is not None]
    return {
...
def plot_radar(
    ...
    """Plot and save one THCI v1.0 six-axis radar chart."""
    labels = [
    ...
    ax.plot(closed_angles, closed_values, color="#2f6f8f", linewidth=2.2)
    ax.fill(closed_angles, closed_values, color="#2f6f8f", alpha=0.20)
    ax.set_ylim(0, 1)
...
def write_case_outputs(
    ...
    score_df, score_csv, score_summary_json = load_axis_scores(case_id)
    validation = validate_axis_scores(score_df)

    out_dir = OUT_ROOT / case_id
    out_dir.mkdir(parents=True, exist_ok=True)
    output_png = out_dir / f"{case_id}_thci_radar_v1_0.png"
    output_plot_data_csv = out_dir / f"{case_id}_thci_radar_plot_data_v1_0.csv"
    output_summary_json = out_dir / f"{case_id}_thci_radar_summary_v1_0.json"

    radar_status = "PASS" if validation["ok"] else "FAIL"
    plot_rows = []
    ...
    plot_df.to_csv(output_plot_data_csv, index=False, encoding="utf-8-sig")
...
def write_batch_summary(case_rows: list[dict[str, Any]]) -> None:
    """Write THCI radar batch summary CSV."""
    batch_dir = OUT_ROOT / "_batch_summary"
    batch_dir.mkdir(parents=True, exist_ok=True)
    pd.DataFrame(case_rows).to_csv(
        batch_dir / "thci_radar_v1_0_case_summary.csv",
        index=False,
        encoding="utf-8-sig",
    )
...
def main() -> int:
    ...
    for case_id in CASES:
        ...
        print(
            ...
            f"png={row['png_exists']} score_min={row['score_min']} score_max={row['score_max']}"
        )
    write_batch_summary(case_rows)
    print("batch summary:", OUT_ROOT / "_batch_summary" / "thci_radar_v1_0_case_summary.csv")

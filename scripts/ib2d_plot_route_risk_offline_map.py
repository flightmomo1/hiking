...
OUT_PNG = OUT_DIR / "qixing_route_risk_offline_map.png"
...
def find_risk_band_col(df: pd.DataFrame):
    candidates = ["risk_band", "segment_risk_band"]
    for c in candidates:
        if c in df.columns:
            return c
    raise KeyError(f"找不到 risk_band 欄位，現有欄位：{list(df.columns)}")
...
def build_route_segments(points_gdf: gpd.GeoDataFrame, risk_df: pd.DataFrame):
    ...
    for i in range(len(points_gdf) - 1):
        ...
        risk_vals = []
        if "risk_score_smooth" in r1.index and pd.notna(r1["risk_score_smooth"]):
            risk_vals.append(float(r1["risk_score_smooth"]))
        if "risk_score_smooth" in r2.index and pd.notna(r2["risk_score_smooth"]):
            risk_vals.append(float(r2["risk_score_smooth"]))

        if not risk_vals:
            if "risk_score" in r1.index and pd.notna(r1["risk_score"]):
                risk_vals.append(float(r1["risk_score"]))
            if "risk_score" in r2.index and pd.notna(r2["risk_score"]):
                risk_vals.append(float(r2["risk_score"]))
...
def main():
    ...
    ensure_exists(SEMANTIC_GEOJSON)

...
def add_phase3c_recovery_interpretation(events: pd.DataFrame, args) -> pd.DataFrame:
    """
    Phase 3-C:
    Add high-level recovery interpretation labels.
    ...
    new_col = "semantic_recovery_interpretation"
    ...
    def _is_hr_drop(cls):
        return cls in {"hr_drop_moderate", "hr_drop_strong"}

    def _is_hr_stable(cls):
        return cls == "hr_stable"

    def interpret(row):
        event_type = _norm(row.get("event_type"))
        semantic_candidate = _norm(row.get("semantic_event_type_candidate"))
        hr_delta_class = _norm(row.get("semantic_hr_delta_class"))

        # Hard guards first
        if event_type == "terminal_artifact" or semantic_candidate == "terminal_artifact":
            return "terminal_artifact"

        if event_type in {"off_route_rest", "off_route_detour", "route_uncertainty_stop"}:
            return "off_route_event"

        if semantic_candidate in {"off_route_rest", "off_route_detour", "route_uncertainty_stop"}:
            return "off_route_event"

        # Facility rest: keep facility semantics distinguishable
        if event_type == "facility_rest" or semantic_candidate == "facility_rest":
            if _is_hr_drop(hr_delta_class):
                return "facility_rest_with_hr_drop"
            return "facility_rest_without_hr_drop"

        # High-HR recovery candidate
        if semantic_candidate == "high_hr_recovery_stop_candidate":
            if _is_hr_drop(hr_delta_class):
                return "confirmed_hr_recovery"
            if _is_hr_stable(hr_delta_class):
                return "high_hr_pause_without_recovery"
            return "high_hr_pause_without_recovery"

        # Non-high-HR recovery candidate
        if semantic_candidate == "recovery_candidate":
            if _is_hr_drop(hr_delta_class):
                return "confirmed_hr_recovery"
            if _is_hr_stable(hr_delta_class):
        ...
        # Short pause
        if semantic_candidate == "short_pause_candidate":
            if _is_hr_drop(hr_delta_class):
                return "possible_recovery"
    ...
    out[new_col] = out.apply(interpret, axis=1)
...
def main():
    ...
    # Phase 3-C recovery interpretation:
    # Add high-level interpretation labels without changing existing event or semantic fields.
    events = add_phase3c_recovery_interpretation(events, args)

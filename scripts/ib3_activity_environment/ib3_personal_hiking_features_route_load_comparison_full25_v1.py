...
SCHEMA_VERSION = "ib3_personal_hiking_features_route_load_comparison_full25_v1"
PASS_CONCLUSION = "PASS_ROUTE_LOAD_BEHAVIOR_RESPONSE_FULL25_DESCRIPTIVE_ONLY"
FAIL_CONCLUSION = "FAIL_ROUTE_LOAD_BEHAVIOR_RESPONSE_FULL25"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build the 25-activity descriptive route-load and behavior "
            "response evidence layer."
        )
    )
...
def collect_activity_windows(
    ...
    counters = {
        ...
        "wrong_route_review_row_count": 0,
        "off_target_review_row_count": 0,
        "noncanonical_review_row_count": 0,
    ...
    with path.open("r", encoding="utf-8-sig", newline="") as handle:
        ...
        for row in reader:
            ...
            wrong_route = as_bool(row.get("wrong_route_flag")) or (
                row.get("route_class", "") == "WRONG_ROUTE"
            )
            off_target = as_bool(row.get("off_target_flag")) or (
                row.get("route_class", "") == "OFF_TARGET"
            )
            if wrong_route:
                counters["wrong_route_review_row_count"] += 1
            if off_target:
                counters["off_target_review_row_count"] += 1
            if row.get("route_class", "") not in ELIGIBLE_ROUTE_CLASSES:
                counters["noncanonical_review_row_count"] += 1
                continue
...
def summarize_activity(
    ...
    return {
        ...
        "interpretation_boundary": (
            "Full25 usable-activity summary only; high-load behavior is descriptive and "
...
def render_html(
        ...
        return [
            {
                ...
                "coverage_ratio": fmt(counts[key] / total if total else None),
            }
            for key in sorted(counts)
        ]

    def ratio_band(value: Any) -> str:
    ...
    def route_distance_band(value: Any) -> str:
        number = as_float(value)
        if number is None:
            return "MISSING"
        start = math.floor(number / 500.0) * 500
        return f"{int(start)}-{int(start + 500)}m"
    ...
    coverage_rows = [
        {
            "coverage_item": "activity summary rows",
            "available_count": len(summaries),
            "total_count": len(usable_ids),
            "coverage_ratio": fmt(
                len(summaries) / len(usable_ids) if usable_ids else None
            ),
        },
        {
            "coverage_item": "windows with route-load evidence",
            "available_count": len(windows)
            - int(audit["missing_route_load_evidence_count"]),
        ...
        {
            "coverage_item": "windows with behavior evidence",
            "available_count": len(windows)
            - int(audit["missing_behavior_response_evidence_count"]),
            "total_count": len(windows),
            "coverage_ratio": fmt(
                (
                    len(windows)
        ...
        {
            "coverage_item": "activities with numeric weather context",
            "available_count": sum(
                row["weather_context_available"] == "True"
    ...
    phase_distribution = distribution_rows(
        [str(row["route_phase"]) for row in windows], "route_phase"
    )
    distance_distribution = distribution_rows(
        [
            route_distance_band(row["route_distance_window_start_m"])
            for row in windows
        ],
        "route_distance_band",
    )
    load_distribution = distribution_rows(
        [str(row["route_load_context_band"]) for row in windows],
        "route_load_context_band",
    )
    stopped_distribution = distribution_rows(
        [ratio_band(row["stopped_ratio"]) for row in windows],
        "stopped_ratio_band",
    )
    low_speed_distribution = distribution_rows(
        [ratio_band(row["low_speed_ratio"]) for row in windows],
        "low_speed_ratio_band",
    )
    hr_distribution = distribution_rows(
        [
            "HR_AVAILABLE"
            if as_float(row["heart_rate_bpm_median"]) is not None
            else "HR_MISSING"
            for row in windows
        ],
        "heart_rate_context",
    )
    weather_distribution = distribution_rows(
        [
            "WEATHER_AVAILABLE"
            if row["weather_context_available"] == "True"
            else "WEATHER_MISSING"
            for row in summaries
        ],
        "weather_coverage",
    )
    load_behavior_distribution = distribution_rows(
        [
            f"{row['route_load_context_band']} | {ratio_band(row['low_speed_ratio'])}"
            for row in windows
        ],
        "route_load_x_low_speed_band",
    )
    case_examples = sorted(
        summaries, key=lambda row: str(row["activity_id_short"])
    )[:10]
...
<head>
<meta charset="utf-8">
<title>IB3 Route Load Behavior Response Full25</title>
<style>
...
<body>
<h1>IB3 Route Load Behavior Response Full25</h1>
<p class="note"><strong>Boundary:</strong> Descriptive usable-activity evidence only.
...
<div class="cards">
<div class="card"><strong>{len(usable_ids)}</strong><span>usable activities</span></div>
...
{table(coverage_rows, ["coverage_item", "available_count", "total_count", "coverage_ratio"])}
<h2>Route-distance window distribution</h2>
{table(distance_distribution, ["route_distance_band", "row_count", "coverage_ratio"])}
{table(phase_distribution, ["route_phase", "row_count", "coverage_ratio"])}
<h2>Route load x behavior response distribution</h2>
{table(load_distribution, ["route_load_context_band", "row_count", "coverage_ratio"])}
{table(load_behavior_distribution, ["route_load_x_low_speed_band", "row_count", "coverage_ratio"])}
{table(stopped_distribution, ["stopped_ratio_band", "row_count", "coverage_ratio"])}
{table(low_speed_distribution, ["low_speed_ratio_band", "row_count", "coverage_ratio"])}
{table(hr_distribution, ["heart_rate_context", "row_count", "coverage_ratio"])}
<h2>Weather coverage</h2>
{table(weather_distribution, ["weather_coverage", "row_count", "coverage_ratio"])}
<h2>Case examples</h2>
<p>Examples are selected by activity id only. They are not an activity or
person ranking.</p>
{table(case_examples, summary_view)}
<h2>Window examples</h2>
<p>Rows are shown in source aggregation order and are not ranked.</p>
...
def main() -> None:
    ...
    review_only_rows = [
        row
        for row in gate_rows
        if row.get("v0_usability_gate", "").strip()
        not in SUPPORTED_USABLE_GATES
    ]
    usable_ids = [
        row.get("activity_id_short", "").strip() for row in usable_rows
    ]
    ...
    for gate in usable_rows:
        activity_short = gate.get("activity_id_short", "").strip()
        context = context_by_short.get(activity_short)
        if not activity_short:
            activity_failures.append("BLANK_ACTIVITY_ID:MISSING_GATE_ID")
            continue
        if context is None:
            activity_failures.append(f"{activity_short}:MISSING_CONTEXT_ROW")
            continue
        if context.get("legacy_gain_features_blocked", "").strip() != "True":
            activity_failures.append(f"{activity_short}:LEGACY_GAIN_NOT_BLOCKED")
            continue
        if (
        ...
        if not path.exists():
            activity_failures.append(f"{activity_short}:MISSING_ENRICHED_CSV")
            continue
        ...
        if not windows:
            activity_failures.append(f"{activity_short}:NO_OUTPUT_WINDOWS")
            continue
    ...
    weather_missing_numeric_value_count = sum(
        int(row["weather_missing_numeric_count"]) for row in summaries
    )
    missing_weather_context_count = sum(
        row["weather_context_available"] != "True" for row in summaries
    )
    ...
    pass_checks = (
        len(usable_rows) == 25
        and len(review_only_rows) == 1
        and len(summaries) == len(usable_rows)
        ...
        and not activity_failures
        and not prohibited_generated
    )
    audit = {
        "schema_version": SCHEMA_VERSION,
        "input_gate_activity_count": len(gate_rows),
        "input_usable_activity_count": len(usable_rows),
        "usable_activity_ids": "|".join(usable_ids),
        "review_only_excluded_count": len(review_only_rows),
        "review_only_excluded_activity_ids": "|".join(
            row.get("activity_id_short", "").strip()
            for row in review_only_rows
        ),
        "output_activity_summary_row_count": len(summaries),
        "source_point_row_count": aggregate_counters["source_row_count"],
        "canonical_eligible_route_point_row_count": aggregate_counters[
            "eligible_route_row_count"
        ],
        "canonical_joined_route_point_row_count": aggregate_counters[
            "joined_route_row_count"
        ],
        "wrong_route_review_evidence_row_count": aggregate_counters[
            "wrong_route_review_row_count"
        ],
        "off_target_review_evidence_row_count": aggregate_counters[
            "off_target_review_row_count"
        ],
        "noncanonical_review_evidence_excluded_row_count": aggregate_counters[
            "noncanonical_review_row_count"
        ],
        "output_window_row_count": len(all_windows),
        ...
        "missing_route_load_evidence_count": missing_route_load_count,
        "missing_behavior_response_evidence_count": missing_behavior_count,
        "missing_weather_context_count": missing_weather_context_count,
        "missing_weather_numeric_value_count": weather_missing_numeric_value_count,
        "weather_zero_fill_count": 0,
        "legacy_gain_fields_used_count": 0,
        ...
        "activity_failure_count": len(activity_failures),
        "activity_failures": "|".join(activity_failures),
        "window_size_m": fmt(WINDOW_SIZE_M),
        "audit_conclusion": PASS_CONCLUSION if pass_checks else FAIL_CONCLUSION,
        "authorization_note": (
            "Descriptive full25 evidence only. No ability score, rank, class, "
    ...
    write_csv(
        args.out_root / "activity_route_load_behavior_response_full25_audit.csv",
        [audit],
        list(audit.keys()),
    )
    render_html(
        args.out_root / "activity_route_load_behavior_response_full25_report.html",
        usable_ids,
        summaries,
        all_windows,
        audit,
    ...
    print(f"usable_activity_ids={audit['usable_activity_ids']}")
    print(f"output_window_row_count={audit['output_window_row_count']}")
    print(
        "output_activity_summary_row_count="
        f"{audit['output_activity_summary_row_count']}"
    )

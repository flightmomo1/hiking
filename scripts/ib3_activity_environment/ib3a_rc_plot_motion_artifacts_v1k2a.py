...
# -*- coding: utf-8 -*-
"""
IB3A-RC v1k2a motion artifact QA HTML plot.
...
def make_qa_decision(rows, summary):
    total = max(1, int(summary.get("total_rows") or 0))
    artifacts = int(summary.get("speed_outlier_rows_gt5") or 0)
    drift = int(summary.get("gps_drift_rows") or 0)
    jumps = int(summary.get("distance_jump_rows_gt20") or 0)

    route_counts = Counter(r.get("route_class", "") for r in rows)
    state_counts = Counter(r.get("movement_state", "") for r in rows)
    artifact_counts = Counter(r.get("motion_artifact_type", "") for r in rows)

    off_target = route_counts.get("OFF_TARGET", 0)
    low_conf = state_counts.get("LOW_CONFIDENCE_REVIEW", 0)
    raw_fallback = Counter(r.get("horizontal_calibration_source", "") for r in rows).get("RAW_GPS_FALLBACK", 0)

    summit_artifacts = artifact_counts.get("SUMMIT_ANCHOR_TRANSITION_JUMP", 0)
    source_artifacts = artifact_counts.get("SOURCE_TRANSITION_JUMP", 0)
    raw_fallback_artifacts = artifact_counts.get("RAW_FALLBACK_TRANSITION_JUMP", 0)
    calibrated_speed_outliers = artifact_counts.get("CALIBRATED_SPEED_OUTLIER", 0)

    cal_dist = summary.get("calibrated_distance_m")
    raw_delta = summary.get("raw_distance_delta_m")
    ratio = summary.get("raw_calibrated_ratio")
    ...
    if artifacts > 0:
        ...
        reasons.append(f"{artifacts:,} speed/artifact rows require review.")

    if summit_artifacts > 0:
        decision = "PASS_WITH_SUMMIT_ANCHOR_TRANSITION_ARTIFACT"
        reasons.append(f"{summit_artifacts:,} artifacts are summit-anchor transition related.")

    if raw_fallback_artifacts > 0:
        if decision == "PASS":
            decision = "PASS_WITH_REVIEW"
        reasons.append(f"{raw_fallback_artifacts:,} artifacts involve RAW_GPS_FALLBACK transition.")

    if off_target > 0:
        if decision == "PASS":
            decision = "PASS_WITH_OFF_TARGET_REVIEW"
        reasons.append(f"{off_target:,} rows are OFF_TARGET and should not be treated as mainline training data.")

    if ratio is not None:
        try:
            rr = float(ratio)
            if rr < 0.85 or rr > 1.15:
                if decision == "PASS":
                    decision = "PASS_WITH_DISTANCE_DIVERGENCE_REVIEW"
                reasons.append(f"Raw/calibrated distance ratio is {rr:.3f}; distance divergence review is recommended.")
        except Exception:
            pass
    ...
    if jumps > 0:
        if decision == "PASS":
            decision = "PASS_WITH_REVIEW"
        reasons.append(f"{jumps:,} calibrated step-distance jumps exceed 20 m.")

    if low_conf > 0:
        reasons.append(f"{low_conf:,} rows are LOW_CONFIDENCE_REVIEW.")

    if not reasons:
        reasons.append("No major motion QA issue detected by current rules.")

    backend_hint = (
        "For normal speed/fitness analytics, use representative rows with "
        "time_interval_valid=True, motion_artifact_flag=False, and movement_state in "
        "[MOVING, SLOW_MOVING, STOPPED]. Keep OFF_TARGET, GPS_DRIFT_SUSPECTED, "
        "DUPLICATE_TIMESTAMP_NON_REPRESENTATIVE, and artifact rows as QA or behavior evidence."
    )
...
def qa_decision_block(rows, summary):
    q = make_qa_decision(rows, summary)
    reasons_html = "".join(f"<li>{esc(x)}</li>" for x in q["reasons"])

    artifact_rows = "".join(
        f"<tr><td>{esc(k)}</td><td class='num'>{v:,}</td></tr>"
        for k, v in q["artifact_counts"].most_common()
    )

    return f"""
    <div class="qa-box">
      <h2>QA decision: {esc(q["decision"])}</h2>
      <h3>Review reasons</h3>
      <ul>{reasons_html}</ul>
      <h3>Backend usage hint</h3>
      <p>{esc(q["backend_hint"])}</p>
      <h3>Motion artifact type counts</h3>
      <table>
        <tr><th>motion_artifact_type</th><th>Rows</th></tr>
        {artifact_rows}
      </table>
    </div>
    """
...
<head>
<meta charset="utf-8"/>
<title>v1k2a motion artifact QA - {esc(route)} {esc(activity_id)}</title>
<style>
...
th {{ background: #f2f2f2; }} .qa-box {{ background:#eef7ff; border-left:6px solid #1976d2; padding:12px 16px; margin:16px 0 24px 0; }}
...
<body>
<h1>IB3A-RC v1k2a motion artifact QA</h1>
...
{qa_decision_block(rows, summary)}
...
def main():
    ...
    for aid in ids:
        csv_path = root / route / aid / f"{route}_{aid}_calibrated_motion_artifacts_v1k2a.csv"
        ...
        html_path = out_dir / route / aid / f"{route}_{aid}_calibrated_motion_artifacts_v1k2a_qa.html"

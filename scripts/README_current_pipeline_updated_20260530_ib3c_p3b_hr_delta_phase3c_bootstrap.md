...
## 2026-05-30 Changelog: IB3C adaptive speed Phase 3-A / Phase 3-B verified
...
```text
outputs\_checkpoints\cp_20260530_ib3c_p3b_hr_delta_verified
```
...
### Phase 3-A: semantic annotation only
...
Phase 3-A output root：

```text
outputs\ib3c_activity_behavior_events_adaptive_speed_v1_phase3a_semantic_annotations_26batch\
```

Phase 3-A compare output：

```text
outputs\ib3c_adaptive_speed_event_detection_v1_phase3a_compare_phase2\
  qixing_lengshuikeng_phase3a_vs_phase2_compare.csv
```
...
```text
Phase 3-A 只新增 semantic annotation。
原始 event_type、nonterminal event count、terminal artifact count 均不得改變。
```
...
```text
status = ok，Count = 26
semantic_columns_complete = True，Count = 26
events_total_delta = 0，全部 26 筆
nonterminal_event_count_delta = 0，全部 26 筆
terminal_artifact_count_delta = 0，全部 26 筆
```

### Phase 3-B: HR delta event annotations
...
Phase 3-B output root：

```text
outputs\ib3c_activity_behavior_events_adaptive_speed_v1_phase3b_hr_delta_26batch\
```

Phase 3-B compare output：

```text
outputs\ib3c_adaptive_speed_event_detection_v1_phase3b_compare_phase3a\
  qixing_lengshuikeng_phase3b_vs_phase3a_compare.csv
```
...
```text
Phase 3-B 只新增 HR delta annotation。
不改 event_type。
不改 semantic_event_type_candidate。
不改 adaptive threshold。
不改 IB3A / IB3A2。
```
...
```text
status = ok，Count = 26
hr_columns_complete = True，Count = 26
events_total_delta = 0，全部 26 筆
...
`semantic_hr_delta_class` 可用於解釋停留期間 HR 變化：
...
```text
terminal_artifact 仍會產生 HR delta 欄位。
分析 route-core recovery 或個人能力模型時，通常應排除：
event_type != "terminal_artifact"
```

### Phase 3-C current status

Phase 3-C 目前只完成 pre-branch 與 bootstrap parity，尚未納入正式 pipeline：

```text
runs\branch_plans\
  branch_plan_20260530_ib3c_phase3c_recovery_interpretation.md

scripts\ib3_activity_environment\
  ib3c_detect_activity_behavior_events_adaptive_speed_v1_phase3c_recovery_interpretation.py
```

Phase 3-C bootstrap output：

```text
outputs\ib3c_activity_behavior_events_adaptive_speed_v1_phase3c_recovery_interpretation_bootstrap_smoketest\
```

目前已完成：

```text
37_1 bootstrap smoke test：pass
Phase 3-B vs Phase 3-C event_type parity：pass
Phase 3-B vs Phase 3-C semantic_event_type_candidate parity：pass
Phase 3-B vs Phase 3-C semantic_hr_delta_class parity：pass
```
...
下一步若繼續 Phase 3-C，請保持 annotation-only：

```text
新增 semantic_recovery_interpretation
不覆蓋 original event_type
不改 semantic_event_type_candidate
不改 semantic_hr_delta_class
不改 HR delta fields
不改 Phase 3-A / Phase 3-B verified outputs
```

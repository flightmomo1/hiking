...
# -*- coding: utf-8 -*-
"""
Build CH6.7 event-based HR recovery evidence from IB3C phase3c behavior events v1.
...
DEFAULT_OUTPUT_ROOT = "outputs/report_figures/ch6_7_hr_recovery_from_ib3c_events_v1"
...
def classify_route_core_status(row: pd.Series) -> str:
    ...
    if event_type in {"high_hr_recovery_stop", "short_pause"}:
...
def summarize_activity(events: pd.DataFrame) -> pd.DataFrame:
    ...
    route_core = events["route_core_event_status"].eq("ROUTE_CORE_RECOVERY_REVIEW_EVENT")
    route_core_events = events[route_core].copy()

    def agg_activity(g: pd.DataFrame) -> pd.Series:
        rc = g[g["route_core_event_status"].eq("ROUTE_CORE_RECOVERY_REVIEW_EVENT")]
        ...
        if len(rc) == 0:
            ...
            status = "ROUTE_CORE_RECOVERY_REVIEW_EVENT_PRESENT_UNCLASSIFIED"
...
def summarize_phase(events: pd.DataFrame) -> pd.DataFrame:
    rc = events[events["route_core_event_status"].eq("ROUTE_CORE_RECOVERY_REVIEW_EVENT")].copy()
    if rc.empty:
        return pd.DataFrame()
...
def summarize_group(events: pd.DataFrame, activity_summary: pd.DataFrame) -> pd.DataFrame:
    ...
    route_core = events[events["route_core_event_status"].eq("ROUTE_CORE_RECOVERY_REVIEW_EVENT")]
    rows = [{
        "scope": "all_events",
...
<head>
<meta charset="utf-8">
<title>CH6.7 HR Recovery From IB3C Events v1</title>
{css}
</head>
<body>
<h1>CH6.7 HR Recovery From IB3C Events v1</h1>
...
{table_html(events[events["route_core_event_status"].eq("ROUTE_CORE_RECOVERY_REVIEW_EVENT")], 120)}
...
def build_audit(
    ...
    conclusion = (
        "PASS_CH6_7_HR_RECOVERY_FROM_IB3C_EVENTS_V1_DESCRIPTIVE_ONLY"
        if len(events) > 0 and forbidden_absent
        else "FAIL_CH6_7_HR_RECOVERY_FROM_IB3C_EVENTS_V1_REVIEW_REQUIRED"
    )
...
def main() -> None:
    ...
    events_csv = output_root / "activity_hr_recovery_events_from_ib3c_v1.csv"
    activity_summary_csv = output_root / "activity_hr_recovery_activity_summary_from_ib3c_v1.csv"
    phase_summary_csv = output_root / "activity_hr_recovery_phase_summary_from_ib3c_v1.csv"
    group_summary_csv = output_root / "activity_hr_recovery_group_summary_from_ib3c_v1.csv"
    audit_csv = output_root / "activity_hr_recovery_from_ib3c_audit_v1.csv"
    html_report = output_root / "activity_hr_recovery_from_ib3c_report_v1.html"

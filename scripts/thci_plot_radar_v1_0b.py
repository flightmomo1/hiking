...
def write_case_outputs(
    ...
    output_png = out_dir / f"{case_id}_thci_radar_v1_0b.png"
    output_plot_data_csv = out_dir / f"{case_id}_thci_radar_plot_data_v1_0b.csv"
    output_summary_json = out_dir / f"{case_id}_thci_radar_summary_v1_0b.json"

    scoring_version_ok = axis_score_summary.get("scoring_version") == SCORING_VERSION
    calibrated_ok = axis_score_summary.get("calibrated_from_v1_0a") is True
    navigation_calibrated_ok = axis_score_summary.get("navigation_semantics_calibrated") is True
    previous_nav = _previous_navigation_score(axis_score_summary)
    v10b_nav = validation["axis_scores"].get("navigation_risk_score")
    metadata_ok = scoring_version_ok and calibrated_ok and navigation_calibrated_ok and previous_nav is not None
    radar_status = "PASS" if validation["ok"] and metadata_ok else "FAIL"
...
def main() -> int:
    ...
    for case_id in cases:
        ...
        print(
            ...
            f"score_max={row['score_max']} prev_nav={row['previous_v1_0a_navigation_risk_score']} "

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Build Chapter 6.7 completion feasibility review v1 evidence outputs.
...
DEFAULT_OUTPUT_ROOT = "outputs/report_figures/ch6_7_completion_feasibility_review_v1"
...
BOUNDARY = (
    "Descriptive completion feasibility review only. Completion time, group, "
    "route-load, behavior-response, weather, and event evidence are descriptive "
...
def build_early_checkpoint_review(
    ...
    for activity_id, group in overlap.groupby("activity_id_short"):
        comp_group = completion.set_index("activity_id_short").loc[activity_id, "completion_group"]
        missing_fields = [
            col
...
def build_conclusion(completion: pd.DataFrame, early: pd.DataFrame, weather_groups: pd.DataFrame) -> pd.DataFrame:
...
def build_audit(
    ...
    conclusion = "PASS_CH6_7_COMPLETION_FEASIBILITY_REVIEW_V1_DESCRIPTIVE_ONLY"
    if forbidden:
...
<head>
<meta charset="utf-8">
<title>CH6.7 Completion Feasibility Review v1</title>
<style>
...
<body>
<h1>CH6.7 Completion Feasibility Review v1</h1>
...
def write_run_report(path: Path, audit: pd.DataFrame, conclusion: pd.DataFrame, stats: dict[str, object]) -> None:
    lines = [
        "# CH6.7 Completion Feasibility Review v1 Run Report",
        "",
        "## Summary",
        "",
    ]
...
def main() -> None:
    ...
    conclusion = build_conclusion(completion, early, weather_groups)

    completion_csv = output_root / "completion_time_distribution_v1.csv"
    groups_csv = output_root / "completion_feasibility_group_summary_v1.csv"
    activity_weather_csv = output_root / "activity_completion_weather_context_v1.csv"
    weather_groups_csv = output_root / "completion_weather_group_summary_v1.csv"
    early_csv = output_root / "early_checkpoint_segment_review_v1.csv"
    conclusion_csv = output_root / "completion_feasibility_conclusion_v1.csv"
    audit_csv = output_root / "completion_feasibility_review_audit_v1.csv"
    report_md = output_root / "completion_feasibility_review_run_report_v1.md"
    html_report = output_root / "completion_feasibility_review_report_v1.html"
    completion_png = output_root / "completion_time_distribution_v1.png"
    group_png = output_root / "fast_middle_slow_group_comparison_v1.png"
    early_png = output_root / "early_checkpoint_1350_1700m_review_v1.png"
    ...
    save_group_png(groups, group_png)
    save_early_png(early, early_png)
    ...
    audit = build_audit(paths, completion, groups, early, activity_weather, output_files, source)

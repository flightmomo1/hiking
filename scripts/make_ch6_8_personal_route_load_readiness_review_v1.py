...
# -*- coding: utf-8 -*-
"""
Build CH6.8 personal route-load readiness review v1.
...
DEFAULT_OUTPUT_ROOT = (
    "outputs/report_figures/ch6_8_personal_route_load_readiness_review_v1"
)
...
def make_review_flags(row: pd.Series) -> list[str]:
    ...
    if (
        ...
        or (pd.notna(early_high_hr_ratio) and early_high_hr_ratio >= 0.5)
    ):
...
<head>
<meta charset="utf-8">
<title>CH6.8 Personal Route-Load Readiness Review v1</title>
{css}
</head>
<body>
<h1>CH6.8 Personal Route-Load Readiness Review v1</h1>
...
def build_audit(
    ...
    conclusion = (
        "PASS_CH6_8_PERSONAL_ROUTE_LOAD_READINESS_REVIEW_V1_DESCRIPTIVE_ONLY"
        if review is not None
        and not review.empty
        and forbidden_absent
        and not missing_inputs
        else "FAIL_CH6_8_PERSONAL_ROUTE_LOAD_READINESS_REVIEW_V1_REVIEW_REQUIRED"
    )
...
def main() -> None:
    ...
    review_csv = output_root / "personal_route_load_readiness_review_v1.csv"
    group_summary_csv = output_root / "personal_route_load_readiness_group_summary_v1.csv"
    input_contract_csv = output_root / "personal_route_load_readiness_input_contract_v1.csv"
    audit_csv = output_root / "personal_route_load_readiness_audit_v1.csv"
    html_report = output_root / "personal_route_load_readiness_report_v1.html"

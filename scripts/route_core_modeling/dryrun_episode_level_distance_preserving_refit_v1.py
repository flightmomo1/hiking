...
def main():
    ...
    ap.add_argument("--max-step-m", type=float, default=8.0)
        ...
        if prev is not None:
            if all(v is not None for v in [prev["_lat"], prev["_lon"], r["_lat"], r["_lon"]]):
                ...
                if raw_step < args.min_step_m:
                    used_step = 0.0
                    note = "below_min_step_treated_as_stationary"
                elif raw_step > args.max_step_m:
                    used_step = args.max_step_m
                    note = "step_clipped_to_max_step"
                else:
                    used_step = raw_step
                    note = "step_used"
    ...
    for r in work:
        if total_cum > 0:
            progress = r["_raw_cum_m"] / total_cum
        else:
            progress = (r["_elapsed"] - start) / max(1e-9, end - start)

        target = start_route_dist + progress * route_span

        projected = fnum(r.get("projected_route_dist_m"))
        refit_h = fnum(r.get("route_dist_refit_m_v1h"))
        refit_g = fnum(r.get("route_dist_refit_m_v1g"))
        ...
        out_rows.append({
            ...
            "used_step_m": r["_used_step_m"],
            "raw_cum_m": r["_raw_cum_m"],
            "raw_progress_ratio": progress,
            ...
            "episode_total_used_m": total_cum,
            "episode_distance_preserving_target_route_dist_m": target,
            "current_projected_route_dist_m": projected if projected is not None else "",
            "current_route_dist_refit_m_v1h": refit_h if refit_h is not None else "",
            "current_route_dist_refit_m_v1g": refit_g if refit_g is not None else "",
            "delta_target_vs_projected_m": (target - projected) if projected is not None else "",
            "delta_target_vs_v1h_refit_m": (target - refit_h) if refit_h is not None else "",
            "dryrun_recommendation": "episode_level_distance_preserving_refit_candidate",
        })
    ...
    print("episode total used movement:", total_cum)

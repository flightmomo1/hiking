...
## 2026-05-31 Changelog: V1H reviewed recovery v1 completed and 36_1 holdout policy

### Current V1H official status

`activity_points_osm_nlsc_corrected` V1H reviewed recovery v1 已完成。V1H 接在 V1G reviewed recovery v2 之後，用於處理仍殘留的 orange / low-confidence / no-route-dist candidate rows。

正式定位：

```text
v1h_reviewed_mainline_corridor_recovery_v1
```

V1H 採人工 reviewed-list controlled recovery，不做全自動 terminal recovery。

核心原則：

```text
1. 僅救人工視覺 QA 確認可穩定接回 official mainline 的區段。
2. 僅套用到 V1G 後仍 unusable 的 candidate rows。
3. 不改 elapsed_sec、timestamp、lat/lon、HR、distance 或 activity order。
4. 不把 terminal artifact、休息 GPS drift、需要折返的 off-route excursion 混入 usable core。
```

### Official V1H scripts / support tools

正式 recovery 腳本：

```text
scripts\route_core_modeling\recover_activity_points_osm_nlsc_corrected_v1h_reviewed_recovery.py
```

QA / support scripts：

```text
scripts\route_core_modeling\plot_activity_points_osm_nlsc_corrected_refit_target_qa_v1h.py
scripts\route_core_modeling\plot_v1h_cluster_focused_refit_target_qa_v2_anchor_lines.py
```

Experimental scripts used only for 36_1 investigation：

```text
scripts\route_core_modeling\dryrun_episode_level_distance_preserving_refit_v1.py
scripts\route_core_modeling\dryrun_episode_level_monotonic_refit_v1.py
scripts\route_core_modeling\plot_36_episode_refit_threeway_compare_v1.py
```

注意：36_1 的 dry-run / three-way compare 腳本不是 V1H official recovery flow，只是 V1I / V1H+ terminal-constrained recovery 的研究素材。
...
```text
outputs\activity_points_osm_nlsc_corrected_v1h_remaining_orange_block_diagnosis\
qixing_lengshuikeng_v1h_episode_visual_review_list_reviewed.csv
```
...
```text
outputs\activity_points_osm_nlsc_corrected_v1h_remaining_orange_block_diagnosis\
qixing_lengshuikeng_v1h_local_exclusion_reviewed.csv
```

Holdout special cases：

```text
outputs\activity_points_osm_nlsc_corrected_v1h_remaining_orange_block_diagnosis\
qixing_lengshuikeng_v1h_holdout_special_cases_reviewed.csv
```

### Official V1H outputs

```text
outputs\activity_points_osm_nlsc_corrected_v1h_reviewed_recovery\
```

每筆 activity output：

```text
outputs\activity_points_osm_nlsc_corrected_v1h_reviewed_recovery\qixing_lengshuikeng\<activity_id>\
qixing_lengshuikeng_<activity_id>_activity_points_osm_nlsc_corrected_v1h.csv
```

Summary outputs：

```text
outputs\activity_points_osm_nlsc_corrected_v1h_reviewed_recovery\qixing_lengshuikeng\
qixing_lengshuikeng_v1h_reviewed_recovery_source_summary.csv

outputs\activity_points_osm_nlsc_corrected_v1h_reviewed_recovery\qixing_lengshuikeng\
qixing_lengshuikeng_v1h_reviewed_recovery_applied_rows_all.csv
```

QA local context：

```text
outputs\activity_points_osm_nlsc_corrected_v1h_reviewed_recovery\qa_local_context\
```

### Final V1H verified result

```text
review rows = 5
applied rows total = 2913
activity output CSV count = 26
```

Applied rows by activity：
...
Applied rows by original V1G status：

```text
15_1, matched_low_confidence_offset    163
15_1, no_activity_route_dist             2
23_1, matched_low_confidence_offset   1234
23_1, no_activity_route_dist             1
29_1, matched_low_confidence_offset   1136
29_1, no_activity_route_dist             1
38_1, matched_low_confidence_offset    376
```

V1H status summary：

```text
matched_core_clean                                             185114
matched_core_recovered_from_isolated_branch_ambiguous             853
matched_core_recovered_from_route_corridor_ambiguity              175
matched_core_recovered_from_route_phase_ambiguity                  45
matched_core_recovered_from_v1h_reviewed_mainline_corridor       2913
matched_core_refit_from_anchors                                 39607
matched_core_refit_to_summit                                    12851
matched_low_confidence_offset                                    7183
no_activity_route_dist                                          97238
```

### V1H reviewed recovery cases

```text
15_1 cluster_group 2;3:
  elapsed 4335–4471
  required-way junction ambiguity with temporal and anchor support

23_1 episode 5 / clusters 5;6:
  elapsed 8074–8527
  episode-level mainline corridor recovery

29_1 cluster 3:
  elapsed 2060–2270
  safe corridor with anchor support
...
38_1 cluster 1:
  elapsed 1224–1513
  fragmented safe corridor with anchor support
```

### 38_1 local exclusion policy

`38_1 cluster2` 包含涼亭休息 GPS drift，不適合整段自動救回。已新增單點 local exclusion：

```text
activity_id = 38_1
cluster_ids = 2
elapsed_start_sec = 7438
elapsed_end_sec = 7438
exclusion_type = local_rest_gps_drift_single_point_exclusion
```

### 36_1 holdout policy

`36_1` 已登記為 V1H holdout special case：

```text
holdout_type = terminal_side_route_distance_collapse
```

原因：raw trajectory 顯示活動持續到終端附近，但 processed route distance 後段崩掉。

Raw trajectory facts：

```text
raw_record_rows = 8015
raw_activity_duration_sec = 8000
raw_last_distance_m = 4416.26
raw_last_lat = 25.1655071508139
raw_last_lon = 121.564120603725
```

Processed facts：

```text
processed_rows_after_5643 = 2371
processed_no_activity_route_dist_after_5643 = 2017
```

Holdout decision：

```text
do_not_use_as_v1h_negative_or_positive_case
recovery_allowed_v1h = false
exclude_from_auto_filter_training = true
```

Interpretation：

```text
36_1 不是一般 large-offset negative sample。
36_1 不應被 V1H 初判規則直接過濾。
36_1 應保留給後續 V1I / V1H+ terminal-constrained recovery 研究。
```

### Current V1H scope boundary

```text
V1H reviewed recovery v1 可收束。
不要在 V1H 直接擴張全自動 terminal-constrained recovery。
若要處理 36_1 類型，另開 V1I / V1H+ branch。
```

---


## 2026-05-31 Changelog: V1G reviewed recovery v2 verified and script registry handoff next

### Current V1G official status

`activity_points_osm_nlsc_corrected` V1G 已完成 `qixing_lengshuikeng` reviewed-list v2 驗證。此段為目前 V1G 正式口徑，取代較早的 37_1-only V1G 單筆驗證說明。

正式定位：

```text
v1g_route_phase_aware_recovery_reviewed_list_v2
```

V1G 採 reviewed-list controlled recovery，不做全自動 spatial assignment。

核心原則：

```text
Refit 是空間幾何歸屬，不是活動時間歸屬。
```
...
route_context_model_status_v1g
route_context_model_usable_v1g
route_phase_recovery_* metadata
```

### Official V1G scripts

Diagnosis：

```text
scripts\route_core_modeling\diagnose_activity_points_osm_nlsc_corrected_v1g_route_phase_ambiguity.py
```

Recovery：

```text
scripts\route_core_modeling\recover_activity_points_osm_nlsc_corrected_v1g_route_phase_aware.py
```

Recovery script 目前已支援：

```text
apply_only_unusable_v1f
```

當 reviewed list 中 `apply_only_unusable_v1f=true`，只補：

```text
route_context_model_usable_v1f == False
```

用於避免同一 elapsed_sec 內 clean / unusable 混合時誤改 clean rows。

### Official V1G reviewed list

```text
outputs\activity_points_osm_nlsc_corrected_v1g_route_phase_diagnosis_batch\
...
### Official V1G output roots

```text
outputs\activity_points_osm_nlsc_corrected_v1g_route_phase_aware_recovery_reviewed\
```

每筆主檔：

```text
outputs\activity_points_osm_nlsc_corrected_v1g_route_phase_aware_recovery_reviewed\qixing_lengshuikeng\<activity_id>\
qixing_lengshuikeng_<activity_id>_activity_points_osm_nlsc_corrected_v1g.csv
```

Total applied rows summary：

```text
outputs\activity_points_osm_nlsc_corrected_v1g_route_phase_aware_recovery_reviewed\qixing_lengshuikeng\
qixing_lengshuikeng_v1g_reviewed_recovery_applied_rows_all.csv
```
...
```text
outputs\activity_points_osm_nlsc_corrected_v1g_route_phase_aware_recovery_reviewed\
qixing_lengshuikeng_v1g_status_summary.csv
```
...
Unusable-only QA result：

```text
15_1, matched_low_confidence_offset     2
15_1, no_activity_route_dist            1
23_1, matched_low_confidence_offset     6
23_1, no_activity_route_dist            1
29_1, matched_low_confidence_offset     3
29_1, no_activity_route_dist            1
```
...
### V1G skip / future cases

以下不放入 V1G：

```text
13_1：low-confidence downhill corridor → future V1H
42_1：late high-offset transition → map QA / future late-transition
8_1 ：low-confidence downhill corridor → future V1H
```
...
```text
Do not expand V1G into full automatic spatial assignment.
```

若要處理更長段 low-confidence corridor、late transition、alternate connector 自動判斷，另開：

```text
v1h_auto_spatial_route_segment_assignment
```

V1H 應負責：

```text
1. 自動判斷 alternate connector。
2. 自動區分可自然接回主線 vs 需要折返。
3. 自動判斷 spatial_route_phase_target。
4. 避免 endpoint drift / terminal artifact 進入 usable core。
```

## Script registry and pipeline handoff v1 / 下一階段腳本盤點

目前 `115_osm` 腳本數量與版本命名已超出人工記憶負荷。下一階段建議先做腳本資產盤點，而不是立刻新增模型功能。

建議分支：

```text
script_registry_and_pipeline_handoff_v1
```

資料來源：

```text
README_current_pipeline_updated_*.md
latest_handoff_prompt_updated_*.md
changelog_updated_*.md
folder_inventory_depth4.csv
```

目的：建立可交接的 script registry，明確區分：
...
```text
outputs\script_registry_and_pipeline_handoff_v1\scripts_inventory_review_v1.csv
outputs\script_registry_and_pipeline_handoff_v1\scripts_inventory_review_v1.md
```
...
official_status
input_root_or_files
output_root_or_files
current_verified_case
verified_command_exists
related_docs
supersedes
superseded_by
do_not_use_reason
...
```text
IA / OSM raw fetch
IB0 route matching and route definition
IB1 route profile, OSM semantics, NLSC terrain
IB2 route risk visualization
IB3A activity mapmatching
IB3A2 route-core filter
IB3B / IB3B2 mapmatching QA visualization
IB3C behavior events
IB3D observed behavior timeline
route_core_modeling / activity_points_osm_nlsc_corrected
model_usability
baseline_model / behavior_load_groups
batch / checkpoint / utility scripts
```

工程原則：

```text
此分支目的為專案降噪與交接，不重新設計架構。
先標示每支腳本是否可用、是否正式、是否 legacy，再決定是否整理或刪除。
```

---


## 2026-05-31 Changelog: activity_points_osm_nlsc_corrected V1G route phase aware recovery verified

### Current V1G status

本次針對 `qixing_lengshuikeng / activity 37_1` 完成 `activity_points_osm_nlsc_corrected` 的 V1G 分支驗證與收束。

正式分支定位：

```text
v1g_route_phase_aware_recovery_offset3
```

V1G 採 **lightweight manual spatial override**，不做全自動完整版救點。本版只處理人工確認的 route phase ambiguity 區塊：
...
V1G 保留原始 elapsed_sec、timestamp_s、lat/lon、heart_rate、distance_m 與 activity order。
V1G 僅修正 route_dist_refit_m_v1g、route_context_model_status_v1g、route_context_model_usable_v1g 與 recovery metadata。
```

### Why V1G was needed

V1F `safe corridor recovery` 已成功救回一般 route-corridor ambiguity 小缺口，但仍保守留下 978–981 附近的點。這些點在 QA 圖上位於主幹路線附近，offset 很小，但因冷水坑七星山 forced-required-way route axis 具有上山／下山必經路段空間接近與 route distance phase ambiguity，導致它們被標成 non-usable 或 low-confidence。

V1G 診斷確認：

```text
elapsed_sec = 978, 979, 980, 981
phase_jump_detected = True
candidate_phase_hint = ambiguous_between_uphill_downhill
recommended_action = diagnose_only_route_phase_ambiguity_candidate
```

每個候選點在 10 / 20 / 30 秒時間窗皆成立，因此共有：

```text
4 points × 3 windows = 12 diagnosis rows
```
...
```text
outputs\activity_points_osm_nlsc_corrected_v1f_safe_corridor_recovery_offset3\qixing_lengshuikeng\37_1\qixing_lengshuikeng_37_1_activity_points_osm_nlsc_corrected_v1f.csv
```

輸出：

```text
outputs\activity_points_osm_nlsc_corrected_v1g_route_phase_diagnosis\qixing_lengshuikeng_37_1_route_phase_ambiguity_candidates.csv
```
...
```text
recommended_action
  diagnose_only_route_phase_ambiguity_candidate    12
  skip_excluded_time_range                         12
  skip_insufficient_context                         9
```

實際 phase ambiguity candidate：
...
```text
outputs\activity_points_osm_nlsc_corrected_v1g_route_phase_aware_recovery_offset3\qixing_lengshuikeng\37_1\qixing_lengshuikeng_37_1_activity_points_osm_nlsc_corrected_v1g.csv
```

applied rows：

```text
outputs\activity_points_osm_nlsc_corrected_v1g_route_phase_aware_recovery_offset3\qixing_lengshuikeng\37_1\qixing_lengshuikeng_37_1_activity_points_osm_nlsc_corrected_v1g_recovery_applied_rows.csv
```
...
```text
elapsed_sec  original_status_v1f           route_dist_refit_m_v1g  spatial_route_phase_target_v1g
978.0        no_activity_route_dist         3458.0                  downhill_required_segment
979.0        no_activity_route_dist         3457.0                  downhill_required_segment
980.0        matched_low_confidence_offset  3456.0                  downhill_required_segment
981.0        no_activity_route_dist         3455.0                  downhill_required_segment
```

Recovery status：

```text
route_context_model_status_v1g = matched_core_recovered_from_route_phase_ambiguity
route_context_model_usable_v1g = True
route_dist_refit_method_v1g    = v1g_route_phase_aware_spatial_segment_refit_manual_override
...
```text
spatial_corridor_supports_downhill_required_segment_temporal_order_preserved
```

### Verified local continuity

V1G 對 974–985 的 route distance 接續：

```text
974 → 563.0   matched_core_clean
975 → 564.0   matched_core_clean
976 → 564.0   matched_core_clean
977 → 565.0   matched_core_clean
978 → 3458.0  matched_core_recovered_from_route_phase_ambiguity
979 → 3457.0  matched_core_recovered_from_route_phase_ambiguity
980 → 3456.0  matched_core_recovered_from_route_phase_ambiguity
981 → 3455.0  matched_core_recovered_from_route_phase_ambiguity
982 → 3454.0  matched_core_clean
983 → 3453.0  matched_core_clean
984 → 3453.0  matched_core_clean
985 → 3452.0  matched_core_clean
```

Interpretation：

```text
978–981 發生在原活動時間序列中，時間軸不改。
但人工確認其空間位置屬於圖資 / NLSC 的 downhill_required_segment。
因此 V1G 將 route_dist_refit_m_v1g 指派到該 spatial route segment，並保留 temporal phase。
```

### Final V1G status summary

```text
route_context_model_status_v1g
  matched_core_clean                                     5968
  matched_core_recovered_from_isolated_branch_ambiguous    29
  matched_core_recovered_from_route_corridor_ambiguity      2
  matched_core_recovered_from_route_phase_ambiguity         4
  matched_core_refit_from_anchors                         275
  matched_core_refit_to_summit                            164
  matched_low_confidence_offset                            95
  no_activity_route_dist                                 2996

route_phase_recovery_applied_v1g = True count: 4
```
...
```text
1. 避免誤救走錯路且需要折返的 off_route_excursion。
2. 避免把 endpoint drift / terminal artifact 混入 usable core。
3. 避免將 temporal phase 誤當成 spatial route segment assignment。
4. V1G 只驗證 route phase ambiguity + manual spatial route segment override 的正確性。
```
...
```text
自動判斷 alternate connector
自動區分可接回主線 vs 需要折返
自動決定 spatial_route_phase_target
```

## 2026-05-30 Changelog: IB3C adaptive speed Phase 3-C recovery interpretation verified

### Current official IB3C adaptive-speed status

目前 `qixing_lengshuikeng` 26 筆活動的 IB3C adaptive-speed 分支已完成下列穩定版本：

```text
Phase 2:
  adaptive speed threshold verified

Phase 3-A:
  semantic annotation only verified

Phase 3-B:
  event-level HR delta fields verified on 26batch

Phase 3-C:
  semantic_recovery_interpretation verified on 26batch
```

正式後續工作基準：

```text
outputs\_checkpoints\cp_20260530_ib3c_p3c_recovery_interpretation_verified
```
...
### Phase 3-A / 3-B / 3-C verified outputs

Phase 3-A script：

```text
scripts\ib3_activity_environment\
  ib3c_detect_activity_behavior_events_adaptive_speed_v1_phase3_semantics.py
```
...
```text
scripts\ib3_activity_environment\
  ib3c_detect_activity_behavior_events_adaptive_speed_v1_phase3b_hr_delta.py
```
...
```text
scripts\ib3_activity_environment\
  ib3c_detect_activity_behavior_events_adaptive_speed_v1_phase3c_recovery_interpretation.py
```

Phase 3-C output root：

```text
outputs\ib3c_activity_behavior_events_adaptive_speed_v1_phase3c_recovery_interpretation_26batch\
```

Phase 3-C compare output：

```text
outputs\ib3c_adaptive_speed_event_detection_v1_phase3c_compare_phase3b\
  qixing_lengshuikeng_phase3c_vs_phase3b_compare.csv
```

Phase 3-C keycase summary：

```text
outputs\ib3c_adaptive_speed_event_detection_v1_phase3c_keycase_summary\
  qixing_lengshuikeng_phase3c_keycase_interpretation_summary.csv
```
...
Phase 3-C vs Phase 3-B compare：

```text
status = ok，Count = 26
interpretation_col_exists = True，Count = 26
events_total_delta = 0，全部 26 筆
nonterminal_delta = 0，全部 26 筆
terminal_artifact_delta = 0，全部 26 筆
event_counts_same = True，全部 26 筆
semantic_counts_same = True，全部 26 筆
hr_delta_counts_same = True，全部 26 筆
any failed rows = none
```

### Phase 3-C checkpoint
...
```text
ib3c_phase3c_recovery_interpretation.py
p3c_26batch
p3c_compare_p3b
p3c_keycase_summary
p3b_ref
branch_plan_phase3c_recovery_interpretation.md
...
### HR delta and recovery interpretation usage rule

`semantic_recovery_interpretation` 是高階解釋欄位，不取代原始 `event_type`。

注意：

```text
terminal_artifact 仍會產生 HR delta 與 interpretation 欄位。
分析 route-core recovery 或個人能力模型時，通常應排除：
...
```text
43_1:
  confirmed_hr_recovery = 16
  possible_recovery = 9

16_1:
  confirmed_hr_recovery = 15
  possible_recovery = 8

37_1:
  confirmed_hr_recovery = 11
  high_hr_pause_without_recovery = 9

48_1:
  confirmed_hr_recovery = 6
  possible_recovery = 15
```

### Next

下一步建議不要再修改 IB3C event detection 本體，先建立 route-core behavior features：

```text
confirmed_hr_recovery_count
confirmed_hr_recovery_duration_sec
possible_recovery_count
possible_recovery_duration_sec
high_hr_pause_without_recovery_count
pause_without_hr_drop_count
facility_rest_with_hr_drop_count
facility_rest_without_hr_drop_count
off_route_event_count
```

建議新分支：

```text
route_core_behavior_features_v1
```

建議輸入基準：

```text
outputs\_checkpoints\cp_20260530_ib3c_p3c_recovery_interpretation_verified\p3c_26batch\
```




## 2026-05-29 Changelog: IB3C adaptive speed Phase 2 verified

### Added

新增 adaptive speed 分支腳本：

```text
scripts\ib3_activity_environment\
  ib3c_detect_activity_behavior_events_adaptive_speed_v1.py
```

新增 Phase 2 26 筆批次輸出：

```text
outputs\ib3c_activity_behavior_events_adaptive_speed_v1_phase2_26batch\
```

新增 Phase 2 vs checkpoint v1d 對照輸出：

```text
outputs\ib3c_adaptive_speed_event_detection_v1_phase2_compare_v1d\
  qixing_lengshuikeng_phase2_vs_checkpoint_v1d_compare.csv
```

### Changed

IB3C adaptive branch 將 low-speed event detection 從固定速度門檻調整為：

```text
effective_low_speed_threshold_mps
```

當啟用 adaptive mode 時，實際門檻為：

```text
adaptive_low_speed_threshold_mps = min(0.7, max(0.25, P25_gps_speed))
```

`0.7 m/s` 保留為全域參考線：

```text
global_low_speed_reference_mps
```

不再解釋為 universal rest / stop threshold。

### Verified

完成 `qixing_lengshuikeng` 26 筆活動 Phase 2 批次驗證。

Phase 2 與 checkpoint v1d behavior load groups 完全一致：

```text
low_event_low_adaptive, low_event_low_adaptive        18
high_event_low_adaptive, high_event_low_adaptive       4
low_event_high_adaptive, low_event_high_adaptive       2
high_event_high_adaptive, high_event_high_adaptive     2
```

Largest event count changes 全部為 0。
...
```text
low_event_high_adaptive
phase2_event_count_nonterminal = 0
```

代表它們屬於整體慢速行進型，而非大量休息事件型。

`37_1`、`43_1`、`48_1` 則維持 behavior-rich / high-load 類型，表示 adaptive threshold 沒有洗掉真正停留或恢復事件。

### Next

建議先建立 Phase 2 verified checkpoint，再進 Phase 3 新增：

```text
slow_movement
recovery_candidate
high_hr_recovery_stop 分層條件
```


### Current modeling status and next steps

目前 route-core completion baseline 已完成以下版本：

```text
route_core_model_dataset_v1:
  已廢棄。
  原因：誤用 route_dist_m max-min 作為活動距離，受到 round-trip route phase ambiguity 影響。

route_core_model_dataset_v1b:
  目前可用之 route-core modeling dataset。
  改用 official_route_length_m 與 GPS distance_m。
  route_axis_dist_span_m 僅保留作為 QA，不作為活動距離。

baseline model v1b:
  使用 event_count / event types 進行簡單線性模型與 LOOCV。
  發現 event_count 對 event-rich slow 有訊號，但會漏掉 slow-without-events 型樣本。

baseline model v1c:
  加入 adaptive speed features。
  發現 low_speed_ratio_adaptive 能補足 23_1 / 38_1 這類事件少但慢速比例高的樣本。

baseline model v1d:
  使用 event_count_nonterminal 與 low_speed_ratio_adaptive 建立 behavior load groups。
  LOOCV MAE 約 8.97 分鐘，是目前最穩定且語意最清楚的 baseline。
```

目前已知問題：

```text
IB3A / IB3A2 在 round-trip route 上仍存在 route_dist_m start/end phase ambiguity。
部分活動可能一開始被配到 descent leg，因此 route_dist_m 不可直接用於：
  1. 活動距離估算
  2. route-axis speed
  3. ascent / descent split
  4. 5-min horizontal / vertical route-axis performance features
```

短期可用欄位：

```text
route_core_duration_min
official_route_length_m
gps_route_core_distance_m
gps_route_core_avg_speed_mps
official_route_core_avg_speed_mps
event_count_nonterminal
low_speed_ratio_adaptive
adaptive_low_block_duration_ratio_ge20s
recovery_candidate_block_count
recovery_candidate_duration_sec
behavior_load_group_v1d
model_weight_v3
```
...
```text
route_dist_m max-min
candidate_phase-based ascent/descent split
route-axis speed
implied_route_speed_mps
raw GPX elevation-derived vertical speed
```
...
```text
1. 文件收束：
   記錄 v1d behavior load groups、adaptive speed threshold、0.7 m/s 閾值語意修正。

2. IB3C 後續版本：
   將 adaptive GPS speed threshold 正式整合進事件偵測。
   新增 slow_movement / recovery_candidate / high_hr_recovery_stop 的分層語意。

3. IB3A route phase 修正：
   解決 round-trip start/end phase ambiguity。
   修正後才能進行 ascent/descent split 與 route-axis performance features。

4. rolling_5min_performance_features_v1：
   在 OSM route axis + NLSC corrected elevation 修正後，
   計算 5 分鐘水平推進、垂直爬升／下降與 HR response。
```

目前正式口徑：

```text
冷水坑 26 筆活動可用於 route-core completion baseline。
目前最穩定的解釋模型不是單純線性迴歸，而是 behavior load grouping。
event_count_nonterminal 可捕捉 event-rich slow 型活動；
low_speed_ratio_adaptive 可捕捉 event-light but slow-moving 型活動。
兩者合併後可形成四種具解釋性的行為負荷群組。
```


### IB3 / model usability

### Model usability summary and route-core trainsets

本流程新增 `model_usability_summary_v3_route_core_clipped` 作為冷水坑 26 筆活動進入 route-core performance model 前的正式資料可用性分級。

#### Why V3 is needed

原始 V2 分級使用 whole-activity usability，直接以整段活動資料計算：

```text
usable_on_route_ratio = rows_usable_on_route / rows_total
excluded_ratio = rows_excluded / rows_total
```

但實際檢查後發現，多數 excluded rows 來自終點後長尾資料：

```text
excluded_reason = route_endpoint_artifact
```

這些資料代表終點後停留、離開登山口、post-route GPS 紀錄或活動紀錄長尾，應保留於 observed behavior profile，但不應納入 route-core performance model denominator。

因此 V3 改用 route-core clipped usability：

```text
route_core_rows_total = rows_total - endpoint_artifact_excluded_rows
route_core_excluded_rows = rows_excluded - endpoint_artifact_excluded_rows
route_core_usable_rows = route_core_rows_total - route_core_excluded_rows
route_core_usable_ratio = route_core_usable_rows / route_core_rows_total
route_core_excluded_ratio = route_core_excluded_rows / route_core_rows_total
```

#### V3 output folders

```text
outputs\model_usability_summary_v3_route_core_clipped\
  qixing_lengshuikeng_model_usability_summary_v3_route_core_clipped.csv

outputs\model_usability_trainsets_v3_route_core_clipped\
  qixing_lengshuikeng_A_route_core_trainset.csv
  qixing_lengshuikeng_B_annotated_route_core_cases.csv
  qixing_lengshuikeng_A_plus_B_route_core_weighted_trainset.csv
  qixing_lengshuikeng_route_core_trainset_split_summary.csv
```
...
```text
A：
  clean route-core performance candidate after endpoint-artifact clipping

B：
  route-core usable but requires behavior / off-route annotation

C：
  not recommended after endpoint clipping
```

Current result for `qixing_lengshuikeng` 26 activities:

```text
A_route_core_trainset：23
B_annotated_route_core_cases：3
C：0
A_plus_B_route_core_weighted_trainset：26
```

Model weights:

```text
A：model_weight_v3 = 1.0
B：model_weight_v3 = 0.7
C：model_weight_v3 = 0.0
```

B annotated route-core cases:
...
For route-core performance modeling, use V3 trainsets rather than V2 whole-activity grades.
...
```text
After clipping endpoint / terminal artifact rows,
all 26 qixing_lengshuikeng activities remain usable for the route-core modeling pool.
23 are clean A-grade route-core samples.
3 are B-grade annotated samples and should be included with lower weight or explicit behavior annotation.
```
...
`IB3D observed behavior timeline` 是 IB3 活動層的 observed behavior 視覺化工具，目的在於補足 IB3B2 route-distance QA board 不適合直接呈現停留、低速、off-route 與 terminal artifact 行為事件的限制。

新增腳本：

```text
scripts\ib3_activity_environment\ib3d_plot_observed_behavior_timeline_v1.py
```
...
```text
IB3B2：
  route-distance QA board
  適合檢查 route axis、mapmatching、offset、on-route / off-route 品質

IB3D：
  observed behavior timeline
  適合檢查 elapsed-time-based behavior events
  包含低速、停留、心率變化、off-route、terminal_artifact 等
```
...
```text
outputs\ib3a2_on_route_activity_filter_v4b_after_forced_route
...
```text
outputs\ib3d_observed_behavior_timeline_v1\
<route_folder>\<activity_id>\
  <route_folder>_<activity_id>_observed_behavior_timeline.png
  <route_folder>_<activity_id>_observed_behavior_timeline.html
  <route_folder>_<activity_id>_observed_behavior_timeline_summary.txt
  <route_folder>_<activity_id>_observed_behavior_timeline_activity_plot_data.csv
  <route_folder>_<activity_id>_observed_behavior_timeline_events_plot_data.csv
```

#### Plot panels

```text
Panel 1：GPS movement speed smooth
Panel 2：heart_rate_bpm
Panel 3：offset_to_mainline_m
Panel 4：usable_on_route
Panel 5：IB3C behavior event lanes
```

#### Speed interpretation

IB3D speed panel 預設只繪製：

```text
GPS movement speed smooth
```

不預設繪製：

```text
route-axis speed raw
route-axis speed smooth
```

原因是 IB3D 的目標是 observed behavior interpretation，而不是 route-core progress QA。GPS movement speed 較適合用於判斷活動者是否實際移動；route-axis speed 則保留於 output plot data 與 summary available count 作為品質檢查資訊。

Speed reference lines:

```text
0.3 m/s
0.7 m/s：IB3C low-speed reference
1.0 m/s
1.5 m/s
2.0 m/s
```

Low-speed candidate spans:

```text
GPS movement speed smooth < 0.7 m/s
minimum duration = 20 sec
```

此區間以淡灰色背景表示，只代表低速候選，不等同休息事件。休息、恢復或停留仍以 IB3C event detection 結果為主。

#### Event interpretation

IB3D 同時呈現 IB3C event spans 與 event lanes。常見事件包含：
...
其中 `terminal_artifact` 是 observed activity behavior profile 的一部分，但不應納入 route-core recovery score，也不應覆蓋上方 speed / HR / offset panels。

#### Optional HR zones

IB3D 支援 optional HR zones，但預設不啟用。
...
HRmax selection priority:

```text
1. Use --hr-max-bpm if provided.
2. Otherwise estimate HRmax using --age and --sex.
3. If insufficient information is provided, do not draw HR zones.
```

HRmax estimation formulas are used only for visualization:

```text
female：206 - 0.88 × age
male / unknown：208 - 0.7 × age
```
...
.\.venv\Scripts\python.exe `
  ...
  --age 35 `
  --sex male `
  --out-dir outputs\ib3d_observed_behavior_timeline_v1_hrzones_test
```
...
optional HR zones output：pass
PNG / HTML / summary / plot data：completed
```
...
### IB3C V1c activity behavior event detection 已完成初版實作與驗證

Update date: 2026-05-29 Asia/Taipei
...
IB3C V1c 的定位是活動行為解釋層，不取代 IB3A / IB3A2：

```text
IB3A:
  activity sequence mapmatching，將活動點壓回 route axis，保留 projected / reliable route distance、phase、match quality 等資訊。

IB3A2:
  route-core filter，判斷哪些點可用於主線速度 / 心率 / 坡度反應模型。

IB3C:
  behavior event detection，將低速、停留、離線、繞行、導航確認、設施休息、心率恢復與終點後紀錄轉成 event-level CSV。
```

#### IB3C V1c 已支援功能

```text
1. low-speed / stationary behavior block detection
   目前實測 speed-threshold-mps = 0.7，較貼近登山低速行為。

2. off-route behavior block detection
   usable_on_route=False 且 excluded_reason 包含 off_route / low_confidence / branch_ambiguous / endpoint_artifact 時，
   即使 reliable_route_dist_m 為空，也會保留並形成 activity behavior candidate block。

3. OSM / NLSC / route-risk context join
   依 reliable_route_dist_m / route_dist_m 對齊 IB1E route context，補入 surface、route_semantic、facility/rest/support、hydrology、slope、terrain risk 等欄位。

4. Y1–Y6 多尺度停留時間分層
   Y1 = 15 sec    → micro pause
   Y2 = 60 sec    → short rest / navigation check
   Y3 = 180 sec   → rest
   Y4 = 600 sec   → long rest / meal-scale stop
   Y5 = 1800 sec  → extended stop / waiting / shelter-scale stop
   Y6 = 7200 sec  → camp / overnight / long-stay candidate

5. HR recovery effect
   依 HR start/end/delta 產生 hr_recovery_effect 與 estimated_recovery_score。

6. terminal_artifact special handling
   終點後活動紀錄保留於 observed activity behavior profile，
   但 estimated_recovery_score 設為空值，不納入 route-core recovery 評估。
```
...
```text
outputs\ib3c_activity_behavior_events_v1c_recovery_speed07_after_forced_route\<route_folder>\<activity_id>\
```

每筆 activity 產出：

```text
<route_folder>_<activity_id>_ib3c_points_labeled.csv
<route_folder>_<activity_id>_ib3c_behavior_events.csv
<route_folder>_<activity_id>_ib3c_behavior_events_summary.txt
```
...
.\.venv\Scripts\python.exe `
  ...
  --case-id qixing_lengshuikeng_main_peak_20260523 `
  --activity-id 37_1 `
  --mapmatched-root outputs\ib3a_sequence_mapmatched_activity_v4b_after_forced_route `
  --ib3a2-root outputs\ib3a2_on_route_activity_filter_v4b_after_forced_route `
  --out-dir outputs\ib3c_activity_behavior_events_v1c_recovery_speed07_after_forced_route `
  --speed-threshold-mps 0.7
```

`33_1` 使用相同命令，僅將 `--activity-id 37_1` 改為 `--activity-id 33_1`。

#### Verified result: 37_1
...
key event_id=13:
  ...
  event_subtype: off_route_exploration_navigation_rejoin_candidate
  elapsed: 4233–5113 sec
  duration: 880 sec
  rest_duration_tier: Y4_long_rest_or_meal
  max_offset_m: 176.02 m
  max_hr_bpm: 173
  interpretation: 偏離主線後的探索、導航確認、繞行或再接回主線候選事件。

key event_id=27:
  event_type: terminal_artifact
  duration: 2136 sec
  rest_duration_tier: Y5_extended_stop
  recovery_level: terminal_post_route_stop
  hr_recovery_effect: hr_recovered_after_finish
  estimated_recovery_score: (blank)
```

#### Verified result: 33_1
...
key event_id=1:
  ...
  duration: 66 sec
  rest_duration_tier: Y2_short_rest_or_navigation
  HR: 134 → 129 bpm
  estimated_recovery_score: 0.5

key event_id=2:
  event_type: facility_rest
  rest_context: bench
  duration: 304 sec
  rest_duration_tier: Y3_rest
  HR: 161 → 133 bpm
  estimated_recovery_score: 0.8
...
```text
1. forced-required-way route axis 保留作為 standard route axis。
2. IB3 活動層不把 standard route axis 視為每筆活動真實路徑。
3. IB3A2 排除的 off-route / terminal artifact 不是直接刪除，而是在 IB3C observed activity behavior profile 中保留與解釋。
4. route-core model 與 observed activity behavior profile 需要分層呈現。
5. terminal artifact 的 HR 下降可記錄，但不計入 route-core recovery score。
```

### IB3C V1c official 26 activity run completed

Update date: 2026-05-29 Asia/Taipei

`qixing_lengshuikeng` 共 26 筆標準化活動已完成 IB3A → IB3A2 → IB3C V1c 正式重跑與驗證。原先分散於 `verified_baseline`、`smoketest_batch10`、`smoketest_batch14` 的穩定結果，已正式整併至 official output roots。

Official output roots:

```text
outputs\ib3a_sequence_mapmatched_activity_v4b_after_forced_route\
outputs\ib3a2_on_route_activity_filter_v4b_after_forced_route\
outputs\ib3c_activity_behavior_events_v1c_recovery_speed07_after_forced_route\
```
...
```text
outputs\ib3c_activity_behavior_events_v1c_recovery_speed07_after_forced_route\_batch_summary\
qixing_lengshuikeng_ib3c_v1c_official_26_activity_summary.csv
```
...
ok_count                         : 26
missing_count                    : 0
terminal_artifact_not_one_count  : 0
recovery_score_count_error       : 0
off_route_event_activity_count   : 3
route_uncertainty_activity_count : 1
```

Top event_count QA samples:
...
Important behavior samples:

```text
37_1:
  中段 off_route_rest / off_route_exploration_navigation_rejoin_candidate。

6_1:
  短 off_route_detour，duration 89 sec，near-route low-confidence。

36_1:
  短 route_uncertainty_stop，duration 44 sec，near-route low-confidence。
```

Engineering conclusion:

```text
IB3A official qixing_lengshuikeng 26 筆：pass
IB3A2 official qixing_lengshuikeng 26 筆：pass
IB3C V1c official qixing_lengshuikeng 26 筆：pass
terminal artifact merge fix：pass
official 26 activity summary CSV：completed
```

Interpretation:

```text
IB3C V1c has passed official 26-activity validation for qixing_lengshuikeng.
Each activity has exactly one terminal_artifact event.
terminal_artifact is retained in observed activity behavior profile but excluded from route-core recovery score.
off-route / route-uncertainty events are preserved for behavior interpretation rather than discarded as invalid data.
```
...
| ib3c | 活動行為事件偵測：低速、停留、off-route exploration/navigation/rejoin candidate、facility rest、HR recovery、terminal artifact | `scripts\ib3_activity_environment\ib3c_detect_activity_behavior_events.py` |
...
7. IB3C activity behavior event detection V1c 已完成初版實作、37_1 / 33_1 驗證，以及 qixing_lengshuikeng official 26 筆正式驗證：
   已可將低速、停留、off-route exploration/navigation/rejoin candidate、facility rest、HR recovery 與 terminal artifact 輸出為 event-level CSV；official summary CSV 已輸出至正式 IB3C root 的 `_batch_summary`。
   後續可再擴充 weather observed/scenario context 與 IB3B2 observed behavior 圖面標註。
   IB3C V1c is implemented and officially validated for 26 qixing_lengshuikeng activities. Future work should add weather context and observed-behavior visualization overlays.

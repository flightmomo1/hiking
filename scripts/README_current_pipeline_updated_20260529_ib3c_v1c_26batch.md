...
#### Verified result: qixing_lengshuikeng 26 activity batch

Update date: 2026-05-29 Asia/Taipei

`qixing_lengshuikeng` 共 26 筆標準化活動已完成 IB3A → IB3A2 → IB3C V1c 批次驗證。

總表輸出：

```text
outputs\ib3c_activity_behavior_events_v1c_recovery_speed07_all_qixing_lengshuikeng_summary\
qixing_lengshuikeng_ib3c_v1c_26_activity_summary.csv
```
...
smoketest_batch10:
  13_1, 14_1, 15_1, 16_1, 20_1,
  23_1, 28_1, 29_1, 3_1, 30_1

smoketest_batch14:
  35_1, 36_1, 38_1, 40_1, 41_1, 42_1, 43_1,
  44_1, 45_1, 46_1, 48_1, 6_1, 8_1, 9_1
```

本次 terminal artifact merge 修正：

```text
同一 activity 內碎裂的 endpoint artifact events 會合併為單一 terminal_artifact。
terminal_artifact 保留於 observed activity behavior profile。
terminal_artifact 不納入 route-core recovery score。

30_1 修正確認：
  修正前 terminal_artifact_count = 12
  修正後 terminal_artifact_count = 1
  duration_sec = 1893.0
  recovery_level = terminal_post_route_stop
  estimated_recovery_score = blank
```
...
```text
37_1:
  中段 off_route_rest / exploration-navigation-rejoin candidate。
...
高事件數樣本，可作為後續 `IB3B2 observed activity behavior profile` 圖面 QA：
...
```text
IB3A qixing_lengshuikeng 26 筆：pass
IB3A2 qixing_lengshuikeng 26 筆：pass
IB3C V1c qixing_lengshuikeng 26 筆：pass
terminal artifact merge fix：pass
26 activity summary CSV：completed
```

後續工作：

```text
1. 將 smoketest batch10 / batch14 的穩定結果整併或重跑到正式 output root。
2. 更新 IB3B2，讓圖面同時顯示 route-core model profile 與 observed activity behavior profile。
3. 擴充 weather context：baseline / scenario / observed。
4. 對高 event_count 樣本進行圖面 QA。
```
...
7. IB3C activity behavior event detection V1c 已完成初版實作、37_1 / 33_1 驗證與 qixing_lengshuikeng 26 筆批次驗證：
   已可將低速、停留、off-route exploration/navigation/rejoin candidate、facility rest、HR recovery 與 terminal artifact 輸出為 event-level CSV；26 筆總驗證結果為 ok_count=26、missing_count=0、terminal_artifact_not_one_count=0、recovery_score_count_error=0。後續可再擴充 weather observed/scenario context 與 IB3B2 observed behavior 圖面標註。
   IB3C V1c is implemented and verified for 37_1 / 33_1 and the 26 qixing_lengshuikeng activities. Future work should add weather context and observed-behavior visualization overlays.

...
#
# 本版用途：
# - 支援不同 GPX 路線之 route-specific output
# - 避免不同路線的 ib0b 主幹路線結果互相覆蓋
# - 輸出可追溯 route_id / route_name / gpx_name / input_stage / ab_mode
# =========================================================
...
IB0_DIR = Path("ib0_gpx_osm_match_output")
IB0C_DIR = Path("ib0c_anchor_output") / CASE_ID

if AB_MODE == "A":
    INPUT_STAGE = "ib0a_pruned"
    IN_FP = IB0_DIR / f"{CASE_ID}_gpx_osm_matched_pruned.geojson"
elif AB_MODE == "B":
    INPUT_STAGE = "ib0_matched"
    IN_FP = IB0_DIR / f"{CASE_ID}_gpx_osm_matched.geojson"
else:
    raise ValueError("AB_MODE 只能是 'A' 或 'B'")

ANCHOR_FP = IB0C_DIR / f"{CASE_ID}_route_anchors.geojson"

OUT_DIR = Path("ib0b_output") / CASE_ID
...
# =========================================================
gpx_interval_rows_m = make_gpx_interval_points(gpx_line_m, GPX_ANCHOR_INTERVAL_M)

anchor_ranked = []

# ib0c start / via / end 仍需用 project 對應到 GPX progress
anchor_ranked.append(("start", start_pt_m, gpx_line_m.project(start_pt_m)))
anchor_ranked.append(("via", via_pt_m, gpx_line_m.project(via_pt_m)))
anchor_ranked.append(("end", end_pt_m, gpx_line_m.project(end_pt_m)))

# GPX interval anchors 本身已知 progress，不再用 project 反推
for role, pt, progress_m in gpx_interval_rows_m:

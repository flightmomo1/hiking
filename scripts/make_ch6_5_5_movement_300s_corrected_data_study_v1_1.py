#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Build CH6.5.5 movement 300s corrected-data study v1.1.
...
DEFAULT_OUTPUT_ROOT = "outputs/report_figures/ch6_5_5_movement_300s_corrected_data_study_v1_1"

BOUNDARY = (
    "CH6.5.5 movement 300s corrected-data study v1.1 is descriptive study evidence only. "
...
PASS = "PASS_CH6_5_5_MOVEMENT_300S_CORRECTED_DATA_STUDY_V1_1_DESCRIPTIVE_ONLY"
REVIEW = "REVIEW_REQUIRED_CH6_5_5_MOVEMENT_300S_CORRECTED_DATA_STUDY_V1_1"
...
def window_row(g: pd.DataFrame, start_pos: int, end_pos: int, profile: dict[str, Any], source_path: str) -> dict[str, Any]:
    ...
    positive_delta_gain = float(seg["positive_delta_elevation_m"].sum(skipna=True))

    horizontal_speed_candidate_mps = (
        dist_delta / duration
        if pd.notna(dist_delta) and pd.notna(duration) and duration > 0
        else np.nan
    )
    ...
    horizontal_valid = bool(
        ...
        and pd.notna(horizontal_sample_ratio)
        and horizontal_sample_ratio >= 0.7
        and pd.notna(horizontal_speed_candidate_mps)
        and horizontal_speed_candidate_mps >= 0.5
    )

    vertical_distance_valid = bool(
        route_continuity_valid
        and pd.notna(dist_delta)
        and dist_delta >= 50
    )

    vertical_gain_valid = bool(
        pd.notna(gain_delta)
        and gain_delta >= 10
        and gain_delta <= 80
    )

    positive_delta_artifact_suspect = bool(
        positive_delta_gain > 120
        or (
            pd.notna(gain_delta)
            and gain_delta >= 0
            and positive_delta_gain > max(60.0, gain_delta * 3.0)
        )
    )

    vertical_valid = bool(
        vertical_distance_valid
        and vertical_gain_valid
        and not positive_delta_artifact_suspect
    )

    if horizontal_valid:
        horizontal_status = "VALID_HORIZONTAL_WINDOW"
    elif not route_continuity_valid:
        horizontal_status = "INSUFFICIENT_HORIZONTAL_EVIDENCE_ROUTE_CONTINUITY"
    elif pd.isna(horizontal_sample_ratio) or horizontal_sample_ratio < 0.7:
        horizontal_status = "INSUFFICIENT_HORIZONTAL_EVIDENCE_LOW_HORIZONTAL_RATIO"
    elif pd.isna(horizontal_speed_candidate_mps) or horizontal_speed_candidate_mps < 0.5:
        horizontal_status = "INSUFFICIENT_HORIZONTAL_EVIDENCE_LOW_ACTIVE_SPEED"
    else:
        horizontal_status = "INSUFFICIENT_HORIZONTAL_EVIDENCE_OTHER"

    if vertical_valid:
        vertical_status = "VALID_VERTICAL_WINDOW"
    elif not route_continuity_valid:
        vertical_status = "INSUFFICIENT_VERTICAL_EVIDENCE_ROUTE_CONTINUITY"
    elif not vertical_distance_valid:
        vertical_status = "INSUFFICIENT_VERTICAL_EVIDENCE_DISTANCE_TOO_SHORT"
    elif pd.isna(gain_delta):
        vertical_status = "INSUFFICIENT_VERTICAL_EVIDENCE_MISSING_GAIN"
    elif gain_delta < 10:
        vertical_status = "INSUFFICIENT_VERTICAL_EVIDENCE_GAIN_TOO_LOW"
    elif gain_delta > 80:
        vertical_status = "INSUFFICIENT_VERTICAL_EVIDENCE_GAIN_OUTLIER_GUARD"
    elif positive_delta_artifact_suspect:
        vertical_status = "INSUFFICIENT_VERTICAL_EVIDENCE_POSITIVE_DELTA_ARTIFACT_SUSPECT"
    else:
        vertical_status = "INSUFFICIENT_VERTICAL_EVIDENCE_OTHER"
    row = {
        ...
        "route_continuity_valid": route_continuity_valid,
        "horizontal_speed_candidate_mps": horizontal_speed_candidate_mps,
        "vertical_distance_valid": vertical_distance_valid,
        "vertical_gain_valid": vertical_gain_valid,
        "positive_delta_artifact_suspect": positive_delta_artifact_suspect,
        ...
        "vertical_gain_300s_sum_positive_delta_m": positive_delta_gain,
        ...
        "horizontal_300s_window_status": horizontal_status,
        "vertical_300s_window_status": vertical_status,
        "horizontal_300s_route_speed_mps": horizontal_speed_candidate_mps if horizontal_valid else np.nan,
...
def summarize_windows(windows: pd.DataFrame, radar_activities: set[str]) -> pd.DataFrame:
    rows = []
    for activity_id, g in windows.groupby("activity_id_short", sort=True):
        ...
        first = g.iloc[0]
        in_radar_baseline = activity_id in radar_activities
        study_population_status = (
            "RADAR_BASELINE_ACTIVITY"
            if in_radar_baseline
            else "EXTRA_SOURCE_ACTIVITY_NOT_IN_RADAR_BASELINE"
        )
        row = {
            "activity_id_short": activity_id,
            "participant_id": first.get("participant_id"),
            "in_radar_baseline": in_radar_baseline,
            "study_population_status": study_population_status,
...
def audit_frame(
    ...
    def count_status(col: str, value: str) -> int:
        if windows.empty or col not in windows.columns:
            return 0
        return int(windows[col].eq(value).sum())

    def count_bool(col: str) -> int:
        if windows.empty or col not in windows.columns:
            return 0
        return int(windows[col].fillna(False).astype(bool).sum())
    ...
    row = {
        ...
        "route_ok_window_count": count_bool("route_continuity_valid"),
        "horizontal_valid_window_count": count_status("horizontal_300s_window_status", "VALID_HORIZONTAL_WINDOW"),
        "horizontal_low_horizontal_ratio_count": count_status("horizontal_300s_window_status", "INSUFFICIENT_HORIZONTAL_EVIDENCE_LOW_HORIZONTAL_RATIO"),
        "horizontal_low_active_speed_count": count_status("horizontal_300s_window_status", "INSUFFICIENT_HORIZONTAL_EVIDENCE_LOW_ACTIVE_SPEED"),
        "horizontal_route_continuity_rejected_count": count_status("horizontal_300s_window_status", "INSUFFICIENT_HORIZONTAL_EVIDENCE_ROUTE_CONTINUITY"),
        "vertical_valid_window_count": count_status("vertical_300s_window_status", "VALID_VERTICAL_WINDOW"),
        "vertical_distance_valid_count": count_bool("vertical_distance_valid"),
        "vertical_gain_valid_count": count_bool("vertical_gain_valid"),
        "vertical_positive_delta_artifact_suspect_count": count_bool("positive_delta_artifact_suspect"),
        "vertical_rejected_by_route_continuity_count": count_status("vertical_300s_window_status", "INSUFFICIENT_VERTICAL_EVIDENCE_ROUTE_CONTINUITY"),
        "vertical_rejected_by_distance_too_short_count": count_status("vertical_300s_window_status", "INSUFFICIENT_VERTICAL_EVIDENCE_DISTANCE_TOO_SHORT"),
        "vertical_rejected_by_gain_too_low_count": count_status("vertical_300s_window_status", "INSUFFICIENT_VERTICAL_EVIDENCE_GAIN_TOO_LOW"),
        "vertical_rejected_by_gain_outlier_guard_count": count_status("vertical_300s_window_status", "INSUFFICIENT_VERTICAL_EVIDENCE_GAIN_OUTLIER_GUARD"),
        "vertical_rejected_by_positive_delta_artifact_suspect_count": count_status("vertical_300s_window_status", "INSUFFICIENT_VERTICAL_EVIDENCE_POSITIVE_DELTA_ARTIFACT_SUSPECT"),
        "hr_valid_window_count": count_status("hr_window_status", "VALID_HR_WINDOW"),
...
<head>
<meta charset="utf-8">
<title>CH6.5.5 Movement 300s Corrected-Data Study v1.1</title>
<style>
...
<body>
<h1>CH6.5.5 Movement 300s Corrected-Data Study v1.1</h1>
...
{table(summary, ["activity_id_short", "study_population_status", "rolling_window_count", "horizontal_300s_valid_window_count", "horizontal_300s_route_speed_p90_mps", "vertical_300s_valid_window_count", "vertical_300s_gain_p90_m", "vertical_300s_vam_p90_mph", "hr_valid_window_count", "profile_join_status"], 50)}
...
def main() -> int:
    ...
    summary = summarize_windows(windows, radar_activities) if not windows.empty else pd.DataFrame()
    ...
    outputs = {
        "inventory": out_root / "movement_300s_source_inventory_v1_1.csv",
        "windows": out_root / "movement_300s_window_candidates_v1_1.csv",
        "summary": out_root / "movement_300s_activity_summary_v1_1.csv",
        "bouts": out_root / "movement_hr_ascent_bouts_v1_1.csv",
        "recovery": out_root / "movement_hr_pause_recovery_events_v1_1.csv",
        "hr_summary": out_root / "movement_hr_response_activity_summary_v1_1.csv",
        "audit": out_root / "movement_300s_audit_v1_1.csv",
        "html": out_root / "movement_300s_report_v1_1.html",
    }

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Build Chapter 6.7 planning context fusion v1.1 evidence tables.
...
v1.1 calibrates the planning caution rules so generic weather metadata,
event annotations, and slope fallback QA flags do not escalate caution levels
by themselves.
"""
...
DEFAULT_OUTPUT_ROOT = "outputs/report_figures/ch6_7_planning_context_fusion_v1_1"
DEFAULT_V1_ROUTE_WINDOWS = (
...
STRONG_IB2_ROUTE_CONTEXT_FLAGS = {
    "IB2_ROUTE_RISK_HIGH_CONTEXT",
    "IB2_SUPPORT_GAP_CONTEXT",
    "IB2_NAVIGATION_CONTEXT",
    "IB2_EXPOSURE_CONTEXT",
    "IB2_TERRAIN_CONTEXT_HIGH",
}

TURNAROUND_STRONG_CONTEXT_FLAGS = {
    "IB2_SUPPORT_GAP_CONTEXT",
    "IB2_NAVIGATION_CONTEXT",
    "IB2_EXPOSURE_CONTEXT",
...
CONTEXTUAL_IB2_EVIDENCE_NOT_STANDALONE_ESCALATION = {
    "IB2_WEATHER_SENSITIVE_ROUTE_EXPOSURE_CONTEXT",
}

ADVERSE_WEATHER_CONTEXT_FLAGS = {
...
WEATHER_METADATA_QA_FLAGS = {
    "WEATHER_ACTIVITY_LEVEL_ONLY",
    "WEATHER_CONTEXT_MISSING",
    "WEATHER_CONTEXT_AVAILABLE_NO_FLAGS",
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--v1-route-windows", default=DEFAULT_V1_ROUTE_WINDOWS)
...
def has_any_flag(value: object, candidates: set[str]) -> bool:
    return bool(set(split_flags(value)) & candidates)
...
def caution_level(row: pd.Series) -> tuple[str, str]:
    ...
    route_flags = split_flags(row.get("route_load_evidence_flags", ""))
    ib2_flags = split_flags(row.get("ib2_route_context_flags", ""))
    weather_flags_value = split_flags(row.get("weather_planning_context_flags", ""))

    route_load_high = route_band == "HIGH_ROUTE_LOAD_CONTEXT"
    route_load_very_high = route_band == "VERY_HIGH_ROUTE_LOAD_CONTEXT"
    strong_ib2_flags = sorted(set(ib2_flags) & STRONG_IB2_ROUTE_CONTEXT_FLAGS)
    turnaround_strong_flags = sorted(set(ib2_flags) & TURNAROUND_STRONG_CONTEXT_FLAGS)
    adverse_weather_flags = sorted(set(weather_flags_value) & ADVERSE_WEATHER_CONTEXT_FLAGS)

    reasons: list[str] = []

    if route_load_very_high and is_candidate and (turnaround_strong_flags or adverse_weather_flags):
        reasons.extend(["VERY_HIGH_ROUTE_LOAD_CONTEXT", "ROUTE_LOAD_BEHAVIOR_CANDIDATE"])
        reasons.extend(turnaround_strong_flags)
        reasons.extend(adverse_weather_flags)
        return TURNAROUND, pipe_flags(reasons)

    if route_load_very_high or is_candidate or (route_load_high and (strong_ib2_flags or adverse_weather_flags)):
        if route_load_very_high:
            reasons.append("VERY_HIGH_ROUTE_LOAD_CONTEXT")
        if is_candidate:
            reasons.append("ROUTE_LOAD_BEHAVIOR_CANDIDATE")
        if route_load_high:
            reasons.append("HIGH_ROUTE_LOAD_CONTEXT")
        reasons.extend(strong_ib2_flags)
        reasons.extend(adverse_weather_flags)
        return CONSERVATIVE, pipe_flags(reasons)

    if route_load_high or strong_ib2_flags:
        if route_load_high:
            reasons.append("HIGH_ROUTE_LOAD_CONTEXT")
        reasons.extend(strong_ib2_flags)
        return REVIEW, pipe_flags(reasons)

    # v1.1 calibration: metadata/QA/annotation-only evidence stays routine.
    # WEATHER_ACTIVITY_LEVEL_ONLY, WEATHER_CONTEXT_MISSING,
    # SLOPE_LOAD_FALLBACK_VERTICAL_RANGE, EVENT_*, and
    # TERMINAL_ARTIFACT_REVIEW_ONLY are retained as evidence but do not
    # escalate planning caution levels by themselves.
    _ = route_flags
    return ROUTINE, "NONE"
...
def distribution_text(series: pd.Series) -> str:
    return " | ".join(f"{k}:{v}" for k, v in series.value_counts().sort_index().items())


def v1_distribution_text(v1_route_windows_path: Path) -> str:
    if not v1_route_windows_path.exists():
        return "V1_ROUTE_WINDOWS_NOT_FOUND"
    try:
        v1 = read_csv(v1_route_windows_path, "v1 route windows")
    except Exception as exc:  # pragma: no cover - defensive run-report helper
        return f"V1_ROUTE_WINDOWS_READ_FAILED:{exc}"
    if "planning_caution_level" not in v1.columns:
        return "V1_PLANNING_CAUTION_LEVEL_MISSING"
    return distribution_text(v1["planning_caution_level"])


def lower_or_moderate_mask(route_windows: pd.DataFrame) -> pd.Series:
    return route_windows["route_load_context_band"].isin(
        ["LOWER_ROUTE_LOAD_CONTEXT", "MODERATE_ROUTE_LOAD_CONTEXT"]
    )


def strong_ib2_mask(route_windows: pd.DataFrame) -> pd.Series:
    return route_windows["ib2_route_context_flags"].apply(
        lambda value: has_any_flag(value, STRONG_IB2_ROUTE_CONTEXT_FLAGS)
    )


def adverse_weather_mask(route_windows: pd.DataFrame) -> pd.Series:
    return route_windows["weather_planning_context_flags"].apply(
        lambda value: has_any_flag(value, ADVERSE_WEATHER_CONTEXT_FLAGS)
    )


def event_annotation_mask(route_windows: pd.DataFrame) -> pd.Series:
    return route_windows["event_annotation_flags"].fillna("NONE").ne("NONE")


def metadata_weather_only_mask(route_windows: pd.DataFrame) -> pd.Series:
    def is_metadata_only(value: object) -> bool:
        flags = set(split_flags(value))
        return bool(flags) and flags.issubset(WEATHER_METADATA_QA_FLAGS)

    return route_windows["weather_planning_context_flags"].apply(is_metadata_only)


def slope_fallback_only_mask(route_windows: pd.DataFrame) -> pd.Series:
    def is_slope_fallback_only(value: object) -> bool:
        flags = set(split_flags(value))
        return bool(flags) and flags.issubset({"SLOPE_LOAD_FALLBACK_VERTICAL_RANGE"})

    return route_windows["route_load_evidence_flags"].apply(is_slope_fallback_only)
...
def build_audit(
    ...
    candidates: pd.DataFrame,
    ib2_agg: pd.DataFrame,
    event_ann: pd.DataFrame,
    v1_route_windows_path: Path,
) -> pd.DataFrame:
    ...
    nonroutine = route_windows["planning_caution_level"].ne(ROUTINE)
    lower_or_moderate = lower_or_moderate_mask(route_windows)
    strong_ib2 = strong_ib2_mask(route_windows)
    adverse_weather = adverse_weather_mask(route_windows)
    candidate = route_windows["is_route_load_behavior_candidate"].fillna(False).astype(bool)
    high_or_very_high = route_windows["route_load_context_band"].isin(
        ["HIGH_ROUTE_LOAD_CONTEXT", "VERY_HIGH_ROUTE_LOAD_CONTEXT"]
    )
    weather_only_escalation = int((lower_or_moderate & adverse_weather & ~strong_ib2 & ~candidate & nonroutine).sum())
    event_only_escalation = int(
        (
            event_annotation_mask(route_windows)
            & ~high_or_very_high
            & ~strong_ib2
            & ~candidate
            & ~adverse_weather
    ...
    weather_metadata_only_escalation = int(
        (
            metadata_weather_only_mask(route_windows)
            & ~high_or_very_high
            & ~strong_ib2
            & ~candidate
            & nonroutine
    ...
    slope_fallback_only_escalation = int(
        (
            slope_fallback_only_mask(route_windows)
            & ~high_or_very_high
            & ~strong_ib2
            & ~candidate
            & ~adverse_weather
    ...
    conclusion = "PASS_CH6_7_PLANNING_CONTEXT_FUSION_V1_1_RULE_CALIBRATED_DESCRIPTIVE_ONLY"
    if forbidden:
        conclusion = "REVIEW_REQUIRED_FORBIDDEN_COLUMNS_PRESENT"
    elif any(
        [
            weather_only_escalation,
            event_only_escalation,
            weather_metadata_only_escalation,
            slope_fallback_only_escalation,
        ]
    ):
        conclusion = "REVIEW_REQUIRED_RULE_CALIBRATION_ESCALATION_CHECK_FAILED"
        ...
        [
            {
                ...
                "routine_planning_context_count": int(route_windows["planning_caution_level"].eq(ROUTINE).sum()),
                "lower_or_moderate_escalated_by_weather_only_count": weather_only_escalation,
                "event_only_escalation_count": event_only_escalation,
                "weather_metadata_only_escalation_count": weather_metadata_only_escalation,
                "slope_fallback_only_escalation_count": slope_fallback_only_escalation,
                "planning_caution_level_distribution": distribution_text(route_windows["planning_caution_level"]),
                "v1_planning_caution_level_distribution": v1_distribution_text(v1_route_windows_path),
...
def main() -> None:
    ...
    paths = {
        ...
        "v1_route_windows": resolve(root, args.v1_route_windows),
    }
    ...
    audit = build_audit(route_windows, summary, segments, candidates, ib2_agg, events, paths["v1_route_windows"])

    route_windows_csv = output_root / "planning_context_route_windows_v1_1.csv"
    summary_csv = output_root / "planning_context_activity_summary_v1_1.csv"
    segments_csv = output_root / "planning_caution_segments_v1_1.csv"
    audit_csv = output_root / "planning_context_fusion_audit_v1_1.csv"
    report_md = output_root / "planning_context_fusion_run_report_v1_1.md"
    ...
    result = {
        ...
        "v1_planning_caution_level_distribution": str(audit.iloc[0]["v1_planning_caution_level_distribution"]),
        "routine_planning_context_count": int(audit.iloc[0]["routine_planning_context_count"]),
        "lower_or_moderate_escalated_by_weather_only_count": int(
            audit.iloc[0]["lower_or_moderate_escalated_by_weather_only_count"]
        ),
        "event_only_escalation_count": int(audit.iloc[0]["event_only_escalation_count"]),
        "weather_metadata_only_escalation_count": int(audit.iloc[0]["weather_metadata_only_escalation_count"]),
        "slope_fallback_only_escalation_count": int(audit.iloc[0]["slope_fallback_only_escalation_count"]),

...
def read_activity_full(route_folder: str, activity_id: str, mapmatched_root: Path) -> pd.DataFrame:
    labeled_fp = Path(
        f"outputs/ib3a2_on_route_activity_filter/{route_folder}/"
    ...
    fp = labeled_fp if use_legacy_ib3a2_flow(mapmatched_root) and labeled_fp.exists() else require_file(mapmatched_fp, "mapmatched activity")
    return normalize_activity_columns(pd.read_csv(fp), fp)


def read_activity_for_speed(route_folder: str, activity_id: str, full_df: pd.DataFrame, mapmatched_root: Path) -> tuple[pd.DataFrame, str]:
    if not use_legacy_ib3a2_flow(mapmatched_root):
        speed_df = full_df[full_df["usable_on_route"]].copy()
        return speed_df.reset_index(drop=True), f"{Path(mapmatched_root).name}_filtered_usable_on_route"

    on_route_fp = Path(
        f"outputs/ib3a2_on_route_activity_filter/{route_folder}/"
        f"{route_folder}_{activity_id}_mapmatched_activity_on_route.csv"
    )
    if on_route_fp.exists():
        return normalize_activity_columns(pd.read_csv(on_route_fp), on_route_fp), "ib3a2_on_route_csv"

    speed_df = full_df[full_df["usable_on_route"]].copy()
    return speed_df.reset_index(drop=True), "labeled_csv_filtered_usable_on_route"
...
def write_png(out_png: Path, df: pd.DataFrame, route: pd.DataFrame, anchors: pd.DataFrame, segments: list[dict]) -> None:
    ...
    ax_speed = fig.add_subplot(gs[1], sharex=ax_ele)
    ax_hr = fig.add_subplot(gs[2], sharex=ax_ele)
    ax_qa = fig.add_subplot(gs[3], sharex=ax_ele)
    ax_map = fig.add_subplot(gs[4])

    for ax in [ax_ele, ax_speed, ax_hr, ax_qa]:
        add_segment_bands(ax, segments)
        ax.grid(True, alpha=0.22)

    ax_ele.plot(route["dist_m"], route["ele_gpx_m"], color="#1f2937", lw=0.9, alpha=0.7, label="GPX raw elevation")
    if {"contour_elev_min_m", "contour_elev_max_m"}.issubset(route.columns):
        ax_ele.fill_between(
            route["dist_m"],
            route["contour_elev_min_m"],
            route["contour_elev_max_m"],
            color="#38bdf8",
            alpha=0.22,
    ...
    if "ele_smooth" in route.columns:
        ax_ele.plot(route["dist_m"], route["ele_smooth"], color="#16a34a", lw=1.1, label="corrected / route smoothed elevation")
    ax_ele.scatter(df["route_dist_m"], df["raw_ele_m"], color="#64748b", s=2, alpha=0.12, label="activity raw elevation points")
...
def write_summary(out_summary: Path, df: pd.DataFrame, segments: list[dict], mapmatched_root: Path) -> None:
    ...
    lines = [
        ...
        "one_d_speed_filter: usable_on_route and not speed_mapmatch_jump/stationary_gps_drift/speed_gap_break/speed_invalid_dt/speed_negative_delta",
...
def main() -> None:
    ...
    activity_full = read_activity_full(args.route_folder, args.activity_id, args.mapmatched_root)
    speed_activity, speed_input_source = read_activity_for_speed(args.route_folder, args.activity_id, activity_full, args.mapmatched_root)
    speed_activity = sort_activity_time_order(merge_route_context(speed_activity, route))
    ...
    write_summary(out_summary, df, segments, args.mapmatched_root)

...
INSPECTION_DIR = (
    PROJECT_ROOT
    / "outputs"
    / "hehuan_north_peak_gpx_onboarding_to_ib2d_v1"
    / "00_gpx_seed_inspection"
)
CANDIDATE_ROOT = PROJECT_ROOT / "outputs" / "route_onboarding_candidate_runs_v1" / CASE_ID
...
def write_reports(manifest: list[dict[str, Any]], inspection: dict[str, Any], py_status: str) -> None:
    write_csv(CANDIDATE_ROOT / "hehuan_north_peak_gpx_to_ib2d_candidate_manifest_v1.csv", manifest)
    summary_rows = []
    for row in manifest:
        summary_rows.append(
            {
    ...
    write_csv(CANDIDATE_ROOT / "hehuan_north_peak_gpx_to_ib2d_candidate_run_summary_v1.csv", summary_rows)
    ...
    md = "\n".join(
        [
            "# Hehuan North Peak Garmin GPX to IB2D candidate run report v1",
            "",
            ...
            f"2. Waypoints converted to anchors/control points: `{'PASS' if (CANDIDATE_ROOT / '00_inputs' / f'{CASE_ID}_control_points_proposal_v1.csv').exists() else 'FAIL'}`.",
            f"3. IA1 success: `{ok('01_ia1_osm_fetch')}`.",
            f"4. IB0 success: `{ok('02_ib0c_anchors')}` / `{ok('03_ib0a_control_point_projection')}` / `{ok('04_ib0a2_component_qa')}` / `{ok('05_ib0b_mainline')}`.",
            f"5. IB0D success: `{ok('07_ib0d_trimmed_mainline')}`.",
            f"6. IB1A / IB1C / IB1E success: `{ok('08_ib1_pipeline')}`.",
            f"7. IB2D success: `{ok('10_ib2d_candidate_map')}`.",
            "8. WARN / REVIEW_REQUIRED: source GPX standard XML is malformed; sanitized copy used. 1.5K/high waypoint reflector interpretation requires review. Out-and-back anchors are duplicated by phase.",
            "9. Out-and-back ambiguity: possible; wrapper uses phase-aware duplicate ascent/descent control points to preserve route order.",
            f"10. THCI baseline candidate readiness: `{'YES_AFTER_IB2D_QA' if ok('10_ib2d_candidate_map') == 'PASS' else 'NO_STOPPED_BEFORE_IB2D'}`.",
            f"11. Manual review still needed: `{'YES'}`.",
            f"12. py_compile result: `{py_status}`.",
            "13. git status --short:",
            "",
            "```text",
            git_status(),
    ...
    (CANDIDATE_ROOT / "hehuan_north_peak_gpx_to_ib2d_candidate_run_report_v1.md").write_text(md, encoding="utf-8")
...
def main() -> None:
    ...
    control_points = propose_control_points(points, waypoints, distances)
    control_points_fp = inputs_dir / f"{CASE_ID}_control_points_proposal_v1.csv"
    ...
    if not run_cmd("01_ia1_osm_fetch", ia1_cmd, manifest):
        write_reports(manifest, inspection, compile_status())
        return
    ...
    # Existing IB1 runner writes to standard roots; for no-overwrite candidate
    # audit we stop before using it unless the candidate case is accepted into
    # contract roots. This stage is intentionally planned, not executed.
    manifest.append(
        {
            "stage_id": "08_ib1_pipeline",
            "status": "NOT_RUN_REQUIRES_NO_OVERWRITE_WRAPPER_OR_CONTRACT_ROOT_ADMISSION",
            "returncode": "",
            "started_at": "",
            "finished_at": datetime.now(timezone.utc).isoformat(),
            "command": "planned: ib1_run_v1_3b_contract_qa_pipeline.py --case-id " + CASE_ID,
            "log_fp": "",
            "failure_reason": "IB1 runner writes to standard contract roots; stop for manual admission/no-overwrite wrapper.",
        }
    )
    write_reports(manifest, inspection, compile_status())

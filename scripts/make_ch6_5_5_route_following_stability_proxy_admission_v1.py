...
# -*- coding: utf-8 -*-
"""
CH6.5.5 route_following_stability proxy admission review v1.
...
OUT_ROOT = ROOT / "outputs/report_figures/ch6_5_5_route_following_stability_proxy_admission_v1"
...
{html_table(activity_rows, activity_cols)}
    ...
    (OUT_ROOT / "route_following_stability_proxy_admission_report_v1.html").write_text(text, encoding="utf-8")
...
def main() -> None:
    ...
    for act, rows in sorted(group_by(events, "activity_id").items()):
        ...
        if not baseline_ok:
            ...
            admission = "INSUFFICIENT_EVENT_EVIDENCE_NO_NON_TERMINAL_ROWS"
            value_out = ""
            note = "No non-terminal event evidence."
        else:
    ...
    extra_admitted = sum(1 for r in extra if r["admission_status"] == "LIMITED_PROXY_CANDIDATE_FOR_ROUTE_FOLLOWING_STABILITY")

    reasons: List[str] = []
    if baseline_activity_count < 25:
        reasons.append("Baseline activity coverage is below expected 25.")
    if baseline_with_events < baseline_activity_count:
        reasons.append("Not all baseline activities have non-terminal IB3C event evidence.")
    if baseline_with_route_issue == 0:
        reasons.append("No route-following issue events detected.")
    if extra_admitted > 0:
    ...
    if forbidden_fields:
        ...
        audit = "PASS_CH6_5_5_ROUTE_FOLLOWING_STABILITY_PROXY_ADMISSION_V1_GOVERNED_LIMITED_PROXY_CANDIDATE"
        audit_reasons = "NONE"
    else:
        audit = "REVIEW_REQUIRED_ROUTE_FOLLOWING_STABILITY_PROXY_ADMISSION_V1"
    ...
    write_csv(
        OUT_ROOT / "route_following_stability_proxy_event_inventory_v1.csv",
        inventory,
        [
    ...
    write_csv(
        OUT_ROOT / "route_following_stability_proxy_activity_summary_v1.csv",
        activity_rows,
        [
            "activity_id", "study_population_status", "baseline_population_gate", "event_rows",
            "non_terminal_event_rows", "route_issue_event_count", "off_route_event_count",
    ...
    write_csv(
        OUT_ROOT / "route_following_stability_proxy_admission_decision_v1.csv",
        decisions,
        [
            "axis_id", "axis_label_zh", "current_axis_output_mode", "recommended_axis_output_mode",
            "admission_decision", "baseline_activity_count", "baseline_with_events_count",
    ...
    write_csv(
        OUT_ROOT / "route_following_stability_proxy_admission_audit_v1.csv",
        [audit_row],
        [
            "source_event_root", "event_rows", "activity_rows", "baseline_activity_count",
            "baseline_with_events_count", "baseline_with_route_issue_count", "extra_source_count",

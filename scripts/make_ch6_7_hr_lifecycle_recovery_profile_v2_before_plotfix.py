...
def save_activity_plot(activity_id: str, df: pd.DataFrame, events: pd.DataFrame, out_path: Path) -> None:
    if df.empty or df["heart_rate_bpm"].isna().all():
        return
    x = numeric(df["route_distance_m"])
    if x.isna().all():
        x = numeric(df["elapsed_sec"]) / 60.0
        xlabel = "elapsed time (min)"
    else:
        x = x / 1000.0
        xlabel = "route distance (km)"
    fig, ax = plt.subplots(figsize=(14, 5), constrained_layout=True)
    ax.plot(x, numeric(df["heart_rate_bpm"]), color="#DC2626", linewidth=1.0, label="heart rate bpm")
    ax.set_title(f"Activity {activity_id} HR lifecycle profile")
    ax.set_xlabel(xlabel)
    ax.set_ylabel("HR bpm")
    ax.grid(alpha=0.25)
    if "route_load_context_band" in df.columns:
        # light vertical hints for very high route-load points
        high = df["route_load_context_band"].eq("VERY_HIGH_ROUTE_LOAD_CONTEXT")
        if high.any():
            for val in x[high].dropna().iloc[::max(1, int(high.sum()/80))]:
                ax.axvline(val, color="#7C2D12", alpha=0.05, linewidth=1)
    if events is not None and not events.empty:
        ev = events[events["activity_id_short"].eq(activity_id)]
        for _, row in ev.iterrows():
            ev_x = row.get("pause_start_route_distance_m")
            if pd.notna(ev_x) and not numeric(df["route_distance_m"]).isna().all():
                ev_x = float(ev_x) / 1000.0
            else:
                ev_x = float(row.get("pause_start_sec", np.nan)) / 60.0
            if pd.notna(ev_x):
                ax.axvline(ev_x, color="#111827", alpha=0.25, linewidth=0.7)
    fig.text(0.01, 0.01, "Descriptive HR lifecycle evidence only; not ability scoring.", fontsize=9, color="#475569")
...
def main() -> None:
    ...
    for aid in reps:
        if aid in plot_cache:
            png = output_root / f"activity_{aid}_hr_lifecycle_profile_v2.png"
            save_activity_plot(aid, plot_cache[aid], events_df, png)
            if png.exists():
                pngs.append(png)

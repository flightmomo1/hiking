...
def main():
    ...
    if {"osm_type", "osm_id"}.issubset(gdf_osm_m.columns):
        # Preferred path: preserve IA1 canonical identity.
        gdf_osm_m["osm_element_type"] = (
            gdf_osm_m["osm_type"]
            .astype("string")
            .str.lower()
        )
        ...
        # 不再使用 dataframe row index 冒充真正的 OSM ID。

...
def parse_args():
    ...
    parser.add_argument(
        "--input-stage",
        default=None,
        choices=["ib0_matched", "ib0a_pruned", "ib0_candidates"],
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "IB0A control point projection top-k CSV。"
            "若提供，IB0B 會解析 ascent/descent via 對應的 required OSM way ids，"
            "並檢查主幹線是否包含這些 required ways。"
        ),
    )
    parser.add_argument(
        "--required-control-roles",
        default="ascent_via,descent_via",
        help="要視為 required way 的 control_role，以逗號分隔。預設 ascent_via,descent_via。",
    )
    parser.add_argument(
        "--check-required-ways",
        action="store_true",
        help="啟用 required-way QA。若未指定但有 --control-points-projection-fp，也會自動啟用。",
    )
    parser.add_argument(
        "--enforce-required-ways",
        action="store_true",
        help="若 required way 缺漏，直接 raise error。第一版建議先不要啟用，只做 QA。",
    )
    parser.add_argument(
        "--control-points-fp",
        default=None,
        help=(
            "Route control points CSV。control_point_constrained 模式會依 order 使用這些點，"
            "例如 via_up / via_down，作為標準主幹線順序約束。"
        ),
    )

    parser.add_argument(
        "--route-definition-mode",
        default="legacy_gpx_interval",
        choices=["legacy_gpx_interval", "control_point_constrained"],
        help=(
            "legacy_gpx_interval=沿用活動/GPX interval anchors；"
            "control_point_constrained=不使用密集 GPX anchors，改用 start/control/summit/end anchors。"
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "In control_point_constrained mode, use only --control-points-fp rows "
            "as ordered anchors. Do not mix IB0C start/via/end anchors into the "
            "ordered anchor sequence. Recommended for full route-definition cases."
        ),
    )
...
CONTROL_POINTS_PROJECTION_FP = (
    resolve_path(args.control_points_projection_fp)
    if args.control_points_projection_fp is not None
    else None
)
CONTROL_POINTS_FP = (
    resolve_path(args.control_points_fp)
    if args.control_points_fp is not None
    else None
)
ROUTE_DEFINITION_MODE = args.route_definition_mode
CONTROL_POINTS_ONLY_ANCHORS = bool(args.control_points_only_anchors)

REQUIRED_CONTROL_ROLES = {
    x.strip()
    for x in str(args.required_control_roles).split(",")
    if x.strip()
}
CHECK_REQUIRED_WAYS = bool(args.check_required_ways or CONTROL_POINTS_PROJECTION_FP is not None)
ENFORCE_REQUIRED_WAYS = bool(args.enforce_required_ways)
...
OUT_REQUIRED_WAY_QA_CSV = OUT_DIR / f"{CASE_ID}_mainline_required_way_qa_{INPUT_STAGE}.csv"
OUT_REQUIRED_WAY_QA_TXT = OUT_DIR / f"{CASE_ID}_mainline_required_way_qa_{INPUT_STAGE}.txt"
...
if CHECK_REQUIRED_WAYS and CONTROL_POINTS_PROJECTION_FP is not None and not CONTROL_POINTS_PROJECTION_FP.exists():
    raise FileNotFoundError(
        f"找不到 control point projection CSV：{CONTROL_POINTS_PROJECTION_FP.resolve()}"
    )

if ROUTE_DEFINITION_MODE == "control_point_constrained":
    if CONTROL_POINTS_FP is None:
        raise ValueError("control_point_constrained 模式需要提供 --control-points-fp")
    if not CONTROL_POINTS_FP.exists():
        raise FileNotFoundError(f"找不到 control points CSV：{CONTROL_POINTS_FP.resolve()}")
...
def read_control_points_csv(control_points_fp: Path) -> pd.DataFrame:
    """Read route control points CSV used by control_point_constrained mode."""
    df = pd.read_csv(control_points_fp)
    required = {"control_id", "control_role", "lat", "lon", "order"}
    missing = required - set(df.columns)
    if missing:
        raise KeyError(f"control points CSV 缺少欄位：{sorted(missing)}")
...
def add_anchor_if_valid(anchor_rows, role: str, pt, order_value: float, source: str):
    if pt is None or getattr(pt, "is_empty", False):
        return
    anchor_rows.append((role, pt, float(order_value), source))
...
def extract_id_from_matched_text(text, key="osm_way_id"):
    if pd.isna(text):
        return ""
    s = str(text)
    for part in s.split(";"):
        ...
        if part.startswith(prefix):
            return normalize_id_value(part[len(prefix):])
    return ""


def load_required_way_ids_from_projection(
    projection_fp,
    required_roles,
    id_key="osm_way_id",
):
    """
    Read IB0A control point projection top-k CSV and extract required OSM way ids.
    ...
    - candidate_rank
    - projection_ok
    - matched_id_text, e.g. "osm_id=15;osm_way_id=15"
    """
    if projection_fp is None:
        return [], pd.DataFrame()

    df = pd.read_csv(projection_fp)

    if df.empty:
        return [], pd.DataFrame()

    if "candidate_rank" in df.columns:
        df = df[pd.to_numeric(df["candidate_rank"], errors="coerce") == 1].copy()

    if "control_role" in df.columns:
        df = df[df["control_role"].astype(str).isin(required_roles)].copy()

    if "projection_ok" in df.columns:
        ok = df["projection_ok"].astype(str).str.lower().isin(["true", "1", "yes"])
        df = df[ok].copy()

    if "matched_id_text" not in df.columns:
        df["required_way_id"] = ""
    else:
        df["required_way_id"] = df["matched_id_text"].apply(
            lambda x: extract_id_from_matched_text(x, id_key)
        )

    df = df[df["required_way_id"].astype(str) != ""].copy()
    required_way_ids = sorted(set(df["required_way_id"].astype(str).tolist()))

    return required_way_ids, df


def collect_way_ids(df):
    id_cols = ["osm_way_id", "way_id", "osm_id", "id"]
    found = set()

    for col in id_cols:
        if col in df.columns:
            vals = df[col].dropna().map(normalize_id_value)
            found.update(v for v in vals if v != "")

    return found

def row_way_id(row):
    """
    Return normalized OSM way id from a GeoDataFrame row.
    Prefer osm_way_id, then way_id, osm_id, id.
    """
    for col in ["osm_way_id", "way_id", "osm_id", "id"]:
        if col in row.index:
            v = normalize_id_value(row.get(col))
            if v != "":
                return v
    return ""


def line_nodes_from_geometry(geom):
    """
    Convert LineString / MultiLineString geometry to graph node coordinate tuples.
    This must match the same coordinate system used when building G.
    """
    if geom is None or geom.is_empty:
        return []

    if geom.geom_type == "LineString":
        return [tuple(x) for x in geom.coords]

    if geom.geom_type == "MultiLineString":
        nodes = []
        for line in geom.geoms:
            coords = [tuple(x) for x in line.coords]
            if not coords:
                continue
            if not nodes:
                nodes.extend(coords)
...
def find_required_way_nodes(gdf_m, required_way_id):
    """
    Find the geometry of a required OSM way from input candidate segments
    and return:
    - selected source row index
    - graph nodes in metric CRS
    """
    required_way_id = normalize_id_value(required_way_id)

    hits = []
    for idx, row in gdf_m.iterrows():
        wid = row_way_id(row)
        if wid == required_way_id:
            nodes = line_nodes_from_geometry(row.geometry)
            if len(nodes) >= 2:
                hits.append((idx, nodes, row.geometry.length))

    if not hits:
        raise ValueError(f"找不到 required_way_id={required_way_id} 對應的 geometry")

    hits = sorted(hits, key=lambda x: x[2], reverse=True)
    best_idx, best_nodes, _ = hits[0]
    return best_idx, best_nodes
...
print("CONTROL_POINTS_PROJECTION_FP:", CONTROL_POINTS_PROJECTION_FP.resolve() if CONTROL_POINTS_PROJECTION_FP else "")
print("CHECK_REQUIRED_WAYS:", CHECK_REQUIRED_WAYS)
print("ENFORCE_REQUIRED_WAYS:", ENFORCE_REQUIRED_WAYS)
print("CONTROL_POINTS_ONLY_ANCHORS:", CONTROL_POINTS_ONLY_ANCHORS)
print("segments input:", len(gdf))

metric_crs = gdf.estimate_utm_crs()
gdf_m = gdf.to_crs(metric_crs)

required_way_ids, required_way_rows = load_required_way_ids_from_projection(
    CONTROL_POINTS_PROJECTION_FP,
    REQUIRED_CONTROL_ROLES,
    id_key="osm_way_id",
)

required_way_by_control_role = {}
required_way_by_control_id = {}

if required_way_rows is not None and not required_way_rows.empty:
    for _, r in required_way_rows.iterrows():
        wid = normalize_id_value(r.get("required_way_id", ""))
        cid = str(r.get("control_id", "")).strip()
        role = str(r.get("control_role", "")).strip()

        if wid:
            if cid:
                required_way_by_control_id[cid] = wid
            if role:
                required_way_by_control_role[role] = wid

if CHECK_REQUIRED_WAYS:
    print("required control roles:", ",".join(sorted(REQUIRED_CONTROL_ROLES)))
    print("required way ids:", ",".join(required_way_ids) if required_way_ids else "(none)")
...
def get_required_way_id_for_anchor_role(anchor_role):
    """
    anchor_role examples:
    - via_up:ascent_via
    ...
    if ":" in s:
        control_id, control_role = s.split(":", 1)
    else:
        control_id, control_role = s, s

    control_id = control_id.strip()
    control_role = control_role.strip()

    if control_id in required_way_by_control_id:
        return required_way_by_control_id[control_id]

    if control_role in required_way_by_control_role:
        return required_way_by_control_role[control_role]
...
if ROUTE_DEFINITION_MODE == "legacy_gpx_interval":
    # Legacy mode:
    # Use dense activity / GPX interval anchors. This reproduces the seed activity
    # trajectory more strongly, including possible personal behaviors such as
    # repeated summit revisits. Keep this for backward compatibility only.
    ...
    # 依 GPX/activity progress 排序
    anchor_ranked = sorted(anchor_ranked, key=lambda x: x[2])

elif ROUTE_DEFINITION_MODE == "control_point_constrained":
    # Control-point constrained mode:
    # The seed GPX is used for candidate discovery / cost evidence only.
    # It is NOT used as dense behavioral ordering anchors.
    # Standard route order is defined by route intent / control points.
    control_df = read_control_points_csv(CONTROL_POINTS_FP)
    control_gdf = gpd.GeoDataFrame(
        control_df.copy(),
        geometry=[Point(lon, lat) for lon, lat in zip(control_df["lon"], control_df["lat"])],
        crs="EPSG:4326",
    ).to_crs(metric_crs)

    control_anchor_rows = []

    # Default anchor order convention:
    # start=1, summit/via=3, end=5.
    # User control points can use intermediate orders, e.g. ascent_via=2, descent_via=4.
    #
    # In full route-definition cases, control_points_csv already contains
    # start / turnaround / end. Do not mix IB0C anchors, otherwise same-entry
    # or out-and-back routes may get polluted by duplicated summit/end anchors.
    if not CONTROL_POINTS_ONLY_ANCHORS:
        add_anchor_if_valid(
            control_anchor_rows,
            "start",
            start_pt_m,
    ...
    for _, r in control_gdf.iterrows():
        control_id = str(r.get("control_id", "control"))
        control_role = str(r.get("control_role", "control"))
        role = f"{control_id}:{control_role}"
        add_anchor_if_valid(
            control_anchor_rows,
            role,
            r.geometry,
    ...
    if not CONTROL_POINTS_ONLY_ANCHORS:
        # Preserve ib0c via anchor as summit / phase boundary unless the control file already has a summit/turnaround role.
        role_text = control_df["control_role"].astype(str)
        has_summit_control = (
            role_text.str.contains("summit", case=False, na=False).any()
            or role_text.str.contains("turnaround", case=False, na=False).any()
        )
        if not has_summit_control:
            add_anchor_if_valid(
                control_anchor_rows,
                "via:summit_anchor",
                via_pt_m,
    ...
    # Sort by declared route-intent order, not seed GPX progress.
    control_anchor_rows = sorted(control_anchor_rows, key=lambda x: x[2])
    anchor_ranked = [(role, pt, order_value) for role, pt, order_value, source in control_anchor_rows]

    print("control-point constrained anchor order:")
    for role, _, order_value, source in control_anchor_rows:
        print(f"  order={order_value:g} role={role} source={source}")

else:
    raise ValueError(f"Unsupported ROUTE_DEFINITION_MODE: {ROUTE_DEFINITION_MODE}")
...
for role, pt, prog in anchor_ranked:
    if not filtered:
        ...
        # In control-point mode, order spacing is semantic, not meters.
        # Still remove only exact / near-identical projected points to avoid zero-length routing.
        if ROUTE_DEFINITION_MODE == "control_point_constrained":
            if pt.distance(filtered[-1][1]) >= 1.0:
                filtered.append((role, pt, prog))
            else:
                print(f"WARNING: skip near-duplicate control anchor {role} order/progress={prog}")
        else:
...
# =========================================================
# 10. 依序 shortest path / forced required-way stitching
# =========================================================

def shortest_path_or_raise(G, node_a, node_b, role_a="", role_b=""):
    if node_a == node_b:
        return [node_a]

    try:
        return nx.shortest_path(G, node_a, node_b, weight="weight")
    except nx.NetworkXNoPath:
        raise nx.NetworkXNoPath(f"no path between {role_a} and {role_b}")
...
def path_cost(G, path):
    ...
    for u, v in zip(path[:-1], path[1:]):
        if G.has_edge(u, v):
            total += float(G.get_edge_data(u, v).get("weight", 0.0))
        else:
            total += Point(u).distance(Point(v))
    return total


def append_path(full_path, sub_path):
    if not sub_path:
        return full_path
...
def build_forced_way_segment(G, node_a, node_b, required_nodes, role_a="", role_b=""):
    """
    Build path:
      node_a -> required_way_entry -> required_way_nodes -> required_way_exit -> node_b

    Try both required way directions and choose lower total cost.
    """
    if len(required_nodes) < 2:
        return shortest_path_or_raise(G, node_a, node_b, role_a, role_b)
    ...
    for req_nodes in [required_nodes, list(reversed(required_nodes))]:
        ...
        try:
            p1 = shortest_path_or_raise(G, node_a, entry, role_a, f"{role_b}:required_entry")
            p3 = shortest_path_or_raise(G, exit_, node_b, f"{role_b}:required_exit", role_b)
        except nx.NetworkXNoPath:
            continue

        # 中段直接使用 required way geometry nodes，確保 required way 一定進入 full_path
        merged = []
        append_path(merged, p1)
        append_path(merged, req_nodes)
        append_path(merged, p3)

        candidates.append((path_cost(G, p1) + path_cost(G, req_nodes) + path_cost(G, p3), merged))

    if not candidates:
        raise nx.NetworkXNoPath(
            f"無法 forced stitch required way between {role_a} and {role_b}"
        )
...
full_path = []
forced_required_way_ids_used = []
forced_required_way_source_idx_used = []

for i in range(len(anchor_nodes) - 1):
    role_a, node_a, prog_a = anchor_nodes[i]
    role_b, node_b, prog_b = anchor_nodes[i + 1]

    # 判斷下一個 anchor 是否是 required control point
    required_way_id_b = ""
    if ROUTE_DEFINITION_MODE == "control_point_constrained":
        required_way_id_b = get_required_way_id_for_anchor_role(role_b)

    if required_way_id_b:
        required_idx, required_nodes = find_required_way_nodes(gdf_m, required_way_id_b)
        forced_required_way_source_idx_used.append(required_idx)

        print(
            f"forced required-way stitching: "
            f"{role_a} -> {role_b}, required_way_id={required_way_id_b}, "
            f"required_nodes={len(required_nodes)}"
        )

        sub_path = build_forced_way_segment(
            G,
            node_a,
            node_b,
            required_nodes,
        ...
        forced_required_way_ids_used.append(required_way_id_b)

    else:
        try:
            sub_path = shortest_path_or_raise(G, node_a, node_b, role_a, role_b)
        except nx.NetworkXNoPath:
            print(f"WARNING: no path between {role_a} and {role_b}")
            continue

    append_path(full_path, sub_path)

forced_required_way_ids_used = sorted(set(forced_required_way_ids_used))
forced_required_way_source_idx_used = sorted(set(forced_required_way_source_idx_used))

print(f"path length nodes: {len(full_path)}")
print(
    "forced required way ids used:",
    ",".join(forced_required_way_ids_used) if forced_required_way_ids_used else "(none)"
)
...
ordered_path_gdf = gpd.GeoDataFrame(
    [{
        ...
        "route_definition_mode": ROUTE_DEFINITION_MODE,
        "control_points_fp": str(CONTROL_POINTS_FP) if CONTROL_POINTS_FP else "",
...
if "forced_required_way_source_idx_used" in globals():
    valid_idx = sorted(set(valid_idx).union(set(forced_required_way_source_idx_used)))
...
mainline["route_definition_mode"] = ROUTE_DEFINITION_MODE
mainline["control_points_fp"] = str(CONTROL_POINTS_FP) if CONTROL_POINTS_FP else ""
...
# =========================================================
# 11a-2. Required-way QA：檢查 control points 指定之 OSM ways 是否進入 mainline
# =========================================================
selected_way_ids = collect_way_ids(mainline)
input_way_ids = collect_way_ids(gdf)

required_way_ids_set = set(required_way_ids)
selected_required_way_ids = sorted(required_way_ids_set.intersection(selected_way_ids))
missing_required_way_ids = sorted(required_way_ids_set.difference(selected_way_ids))
input_missing_required_way_ids = sorted(required_way_ids_set.difference(input_way_ids))

required_way_status_rows = []

if CHECK_REQUIRED_WAYS:
    if required_way_rows is not None and not required_way_rows.empty:
        for _, r in required_way_rows.iterrows():
            wid = normalize_id_value(r.get("required_way_id", ""))
            required_way_status_rows.append(
                {
                    "case_id": CASE_ID,
                    "input_stage": INPUT_STAGE,
                    "ab_mode": AB_MODE,
                    ...
                    "name": r.get("name", ""),
                    "required_way_id": wid,
                    "projection_offset_to_osm_m": r.get("offset_to_osm_m", ""),
                    "projection_ok": r.get("projection_ok", ""),
                    "required_way_in_input": wid in input_way_ids,
                    "required_way_in_mainline": wid in selected_way_ids,
                    "matched_id_text": r.get("matched_id_text", ""),
                    "highway": r.get("highway", ""),
                    "name_osm": r.get("name_osm", ""),
                }
            )

    required_way_qa_df = pd.DataFrame(required_way_status_rows)
    required_way_qa_df.to_csv(
        OUT_REQUIRED_WAY_QA_CSV,
        index=False,
        encoding="utf-8-sig",
    )

    qa_lines = [
        "IB0B required-way QA",
        f"case_id: {CASE_ID}",
        f"input_stage: {INPUT_STAGE}",
        f"ab_mode: {AB_MODE}",
        f"control_points_projection_fp: {CONTROL_POINTS_PROJECTION_FP}",
        f"required_control_roles: {','.join(sorted(REQUIRED_CONTROL_ROLES))}",
        f"required_way_ids: {','.join(required_way_ids)}",
        f"selected_required_way_ids: {','.join(selected_required_way_ids)}",
        f"missing_required_way_ids: {','.join(missing_required_way_ids)}",
        f"input_missing_required_way_ids: {','.join(input_missing_required_way_ids)}",
        f"required_way_all_present_in_input: {len(input_missing_required_way_ids) == 0}",
        f"required_way_all_present_in_mainline: {len(missing_required_way_ids) == 0}",
        "",
        "per-control-point:",
    ]
    for row in required_way_status_rows:
        qa_lines.append(
            f"- {row['control_id']} ({row['control_role']}): "
            f"required_way_id={row['required_way_id']}; "
            f"in_input={row['required_way_in_input']}; "
            f"in_mainline={row['required_way_in_mainline']}; "
            f"offset={row['projection_offset_to_osm_m']}; "
            f"osm_name={row['name_osm']}"
        )

    OUT_REQUIRED_WAY_QA_TXT.write_text("\n".join(qa_lines), encoding="utf-8-sig")

    print("\n--- required-way QA ---")
    print(f"required way ids: {','.join(required_way_ids) if required_way_ids else '(none)'}")
    print(f"selected required way ids: {','.join(selected_required_way_ids) if selected_required_way_ids else '(none)'}")
    print(f"missing required way ids: {','.join(missing_required_way_ids) if missing_required_way_ids else '(none)'}")
    print(f"input missing required way ids: {','.join(input_missing_required_way_ids) if input_missing_required_way_ids else '(none)'}")
    print(f"required-way QA CSV: {OUT_REQUIRED_WAY_QA_CSV}")
    print(f"required-way QA TXT: {OUT_REQUIRED_WAY_QA_TXT}")

    if ENFORCE_REQUIRED_WAYS:
        if input_missing_required_way_ids:
            raise SystemExit(
                "[IB0B required-way enforce failed] "
                "Required ways are missing from input candidate graph: "
                + ",".join(input_missing_required_way_ids)
                + ". Please check --in-fp and control point projection source."
            )

        if missing_required_way_ids:
            raise SystemExit(
                "[IB0B required-way enforce failed] "
                "Required ways are present in input but missing from selected mainline: "
                + ",".join(missing_required_way_ids)
                + ". This indicates that anchor-based path assembly bypassed required OSM ways. "
                "Next step: implement forced required-way stitching."
            )
...
if CHECK_REQUIRED_WAYS and required_way_ids:
    if "osm_way_id" in mainline.columns:
        mainline["required_way_match"] = mainline["osm_way_id"].map(normalize_id_value).isin(required_way_ids_set)
    elif "way_id" in mainline.columns:
        mainline["required_way_match"] = mainline["way_id"].map(normalize_id_value).isin(required_way_ids_set)
    elif "osm_id" in mainline.columns:
        mainline["required_way_match"] = mainline["osm_id"].map(normalize_id_value).isin(required_way_ids_set)
...
for i, pt in enumerate(anchor_points_wgs):
    folium.CircleMarker(
        ...
        fill=True,
        fill_opacity=0.5,
        tooltip=f"ordered_anchor_{i}: {anchor_ranked[i][0]}",
    ).add_to(m)
...
summary = {
    ...
    "anchor_source_mode": ANCHOR_SOURCE_MODE,
    "route_definition_mode": ROUTE_DEFINITION_MODE,
    "control_points_only_anchors": CONTROL_POINTS_ONLY_ANCHORS,
    "control_points_fp": str(CONTROL_POINTS_FP) if CONTROL_POINTS_FP else "",
    "out_dir": str(OUT_DIR),
    ...
    "check_required_ways": CHECK_REQUIRED_WAYS,
    "control_points_projection_fp": str(CONTROL_POINTS_PROJECTION_FP) if CONTROL_POINTS_PROJECTION_FP else "",
    "required_control_roles": ",".join(sorted(REQUIRED_CONTROL_ROLES)),
    "required_way_ids": ",".join(required_way_ids),
    "selected_required_way_ids": ",".join(selected_required_way_ids),
    "missing_required_way_ids": ",".join(missing_required_way_ids),
    "input_missing_required_way_ids": ",".join(input_missing_required_way_ids),
    "required_way_all_present_in_input": len(input_missing_required_way_ids) == 0,
    "required_way_all_present_in_mainline": len(missing_required_way_ids) == 0,
    "forced_required_way_ids_used": ",".join(forced_required_way_ids_used) if "forced_required_way_ids_used" in globals() else "",
    "forced_required_way_stitching_enabled": ROUTE_DEFINITION_MODE == "control_point_constrained",
    "forced_required_way_source_idx_used": ",".join(map(str, forced_required_way_source_idx_used)) if "forced_required_way_source_idx_used" in globals() else "",

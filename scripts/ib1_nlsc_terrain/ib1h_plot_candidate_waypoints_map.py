...
def is_named_waypoint(row) -> bool:
    name = str(row.get("name", "")).strip()
    return name.lower() not in UNNAMED_SCENIC_NAMES


def get_offset_m(row) -> float:
    for key in [
...
def classify_waypoint_layer(row) -> str:
    """
    將 waypoint 分成：
    - display: 給登山者看的景點 / 目的地 / 設施
    - observe: 系統內部觀測用，例如 risk / pacing / recovery / 地形休息候選
    - debug: 低優先 scenic candidate
    """
    waypoint_type = str(row.get("waypoint_type", "")).lower()
    parts = set(split_waypoint_type(waypoint_type))
    offset_m = get_offset_m(row)
    named = is_named_waypoint(row)

    # 目的地：一定給使用者看
    if "destination_stop" in parts:
        return "display: destination"

    # 展望點：offset 合理時給使用者看；過遠則 debug
    if "viewpoint_stop" in parts:
        if offset_m <= 80:
            return "display: viewpoint"
        return "debug: low priority viewpoint"

    # 設施：未來加入 bench / shelter / toilets / drinking water 後可直接分類
    if parts.intersection(DISPLAY_FACILITY_TYPES):
        return "display: facility"

    # 有名稱的 scenic / guide map：給使用者看
    if named and parts.intersection({"scenic_stop", "guide_map_stop", "tourism_stop"}):
        return "display: named scenic"

    # risk / pacing / recovery / rest_candidate：預設當背景觀測
    if parts.intersection(RISK_OBSERVE_TYPES):
        return "observe: risk / behavior"

    # 未命名 scenic / generic tourism：預設低優先
    if parts.intersection({"scenic_stop", "guide_map_stop", "tourism_stop"}):
        if offset_m <= 80:
            return "observe: scenic behavior"
        return "debug: low priority scenic"
...
# ---------------------------------------------------------
# 5b. waypoint layers by display / observe / debug layer
# ---------------------------------------------------------
wp_gdf["primary_waypoint_type_for_map"] = wp_gdf["waypoint_type"].apply(primary_waypoint_type)
wp_gdf["waypoint_display_layer"] = wp_gdf.apply(classify_waypoint_layer, axis=1)
...
layer_order = [
    ...
    "observe: scenic behavior",
    "debug: low priority scenic",
    "debug: low priority viewpoint",
    "debug: other",
]

actual_layers = list(
    wp_gdf["waypoint_display_layer"]
    .fillna("debug: other")
    .astype(str)
    .unique()
)
...
ordered_layers = [
    ...
    if layer in actual_layers
] + [
    layer for layer in sorted(actual_layers)
    if layer not in layer_order
]

for layer_name in ordered_layers:
    show_layer = layer_name.startswith("display:")
    count = int((wp_gdf["waypoint_display_layer"] == layer_name).sum())

    wp_layers[layer_name] = folium.FeatureGroup(
        name=f"{layer_name}（{count}）",
        show=show_layer,
    )
    wp_layers[layer_name].add_to(m)


for _, row in wp_gdf.iterrows():
    wp_type = primary_waypoint_type(row.get("waypoint_type", "unknown"))
    display_layer = row.get("waypoint_display_layer", "debug: other")
    ...
    projected_dist = get_first_existing(
        row,
        ["projected_dist_m", "target_dist_m", "dist_m"],
        "",
    )
    ...
    layer = wp_layers.get(display_layer, list(wp_layers.values())[0])
...
print("\n--- waypoint display layer ---")
print(wp_gdf["waypoint_display_layer"].value_counts(dropna=False))

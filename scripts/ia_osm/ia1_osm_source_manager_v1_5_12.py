...
VERSION = "v1.5.12-adaptive-query-geometry-connectivity-qa-map"
STATUS = (
...
def write_route_subset_connectivity_qa_map(
    ...
    # Reconstruct the production raw buffer from the same v1.5.12 adaptive contract for display only.
    ...
    _folium_outline(
        m,
        raw_query_buffer_wgs,
        name=f"Production raw query buffer ({args.buffer_m:g} m)",
        color="#FB8C00",
        weight=2,
        dash_array="8 5",
    )
    ...
    # QA sample points use the exact same spacing/distance threshold as run_basic_connectivity_qa.
    if network_geom is not None and not network_geom.is_empty:
        ...
        for idx, (point_m, point_wgs) in enumerate(zip(samples_m, samples_wgs)):
            ...
            folium.CircleMarker(
                ...
                fill_opacity=0.9,
                tooltip=(
                    f"sample={idx} distance_to_OSM={distance_m:.1f} m "
    ...
    folium.Marker(
        [start.y, start.x],
        tooltip=(
            f"START | component={connectivity_qa.get('start_component')} | "
            f"distance={connectivity_qa.get('start_distance_m')} m"
        ),
    ...
    folium.Marker(
        [end.y, end.x],
        tooltip=(
            f"END | component={connectivity_qa.get('end_component')} | "
            f"distance={connectivity_qa.get('end_distance_m')} m"
        ),
    ...
    <div style="position: fixed; top: 10px; right: 10px; z-index: 9999;
                ...
                max-width: 360px; box-shadow: 0 1px 5px rgba(0,0,0,0.35);">
      <div style="font-weight:700; font-size:14px; margin-bottom:6px;">ROUTE_SUBSET_CONNECTIVITY_QA</div>
      ...
      <div><b>Route samples:</b> {connectivity_qa.get('route_sample_count')}</div>
      <div><b>Near ratio:</b> {near_ratio_text} (required {thresholds.get('near_ratio')})</div>
      <div><b>Near distance:</b> &le; {thresholds.get('max_distance_m')} m</div>
      <div><b>OSM components:</b> {connectivity_qa.get('component_count')}</div>
      ...
      <hr style="margin:6px 0;">
      <div style="font-weight:700;">QUERY_GEOMETRY_QA</div>
      <div><b>Contract:</b> {query_geometry_qa.get('query_geometry_contract')}</div>
      <div><b>Strategy:</b> {qfix.get('strategy')}</div>
      <div><b>Fallback triggered:</b> {qfix.get('fallback_triggered')}</div>
      <div><b>Fallback reason:</b> {qfix.get('fallback_reason') or 'NONE'}</div>
      <div><b>Direct contract pass:</b> {qfix.get('direct_contract_pass')}</div>
      <div><b>Raw buffer covers GPX:</b> {qfix.get('raw_buffer_covers_route')}</div>
      <div><b>Simplified buffer covers GPX:</b> {qfix.get('simplified_buffer_covers_route')}</div>
      <div><b>Route outside raw:</b> {qfix.get('route_outside_raw_buffer_length_m')} m</div>
      <div><b>Route outside simplified:</b> {qfix.get('route_outside_simplified_buffer_length_m')} m</div>
    </div>
    """
...
def main() -> None:
    ...
    tile_qa = (
        read_osm_source_store_v2_tile_qa(selected_entry)

...
except ImportError:  # compatibility with older Folium releases
...
VERSION = "v1.5.14-source-store-tile-qa-grouped-map-controls"
STATUS = (
    "GPX probe first + NLSC 0.125deg theoretical tile authority + OSM Source Store v2 current.json authority + "
    "formal reusable TILE_FULL source vs GPX-derived ROUTE_ANALYSIS_SUBSET separation + "
    "route-derived outputs isolated under outputs/ia1_route_analysis + full TILE_FULL semantic parse + "
    "explicit route-polygon clipping using adaptive direct-buffer-first GPX query geometry + route-layer conservation QA + "
    "direct-buffer self-QA with dissolved-segment fallback + hard query-buffer coverage invariants + route-subset connectivity QA HTML + OSM element/id preservation QA + "
    "OSM core-tag schema expansion + lossless extra-tag JSON preservation + 7-grade sac_scale normalization + tag coverage QA + Source Store v2 TILE_FULL QA writer + grouped HTML layer controls + "
    "hiking/foot relation-member enrichment + explicit-only network fallback"
)
...
# OSMnx adds structural geometry/member columns that are not OSM tags.
# These are deliberately excluded from osm_tags_extra_json. All other source
# columns that are not part of PROTECT_COLS are retained losslessly as JSON.
OSMNX_STRUCTURAL_COLUMNS = {
    "geometry",
    "element",
    "element_type",
    "id",
...
SAC_SCALE_INFO = {
    "strolling": {"rank": 0, "t_grade": "none", "hint": "very_easy"},
    "hiking": {"rank": 1, "t_grade": "T1", "hint": "easy"},
    "mountain_hiking": {"rank": 2, "t_grade": "T2", "hint": "moderate"},
    "demanding_mountain_hiking": {"rank": 3, "t_grade": "T3", "hint": "hard"},
    "alpine_hiking": {"rank": 4, "t_grade": "T4", "hint": "very_hard"},
    "demanding_alpine_hiking": {"rank": 5, "t_grade": "T5", "hint": "extreme"},
    "difficult_alpine_hiking": {"rank": 6, "t_grade": "T6", "hint": "extreme_plus"},
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "IA1 OSM source manager: GPX-first NLSC theoretical tile authority, OSM Source Store v2 "
            "current.json local TILE_FULL reuse, and explicit governed network fallback."
        )
    )
    ...
    parser.add_argument(
        "--route-output-root",
        default="outputs/ia1_route_analysis",
        help=(
            "Default GPX-derived IA1 route-analysis root. When --out-dir is omitted, outputs are written as "
            "<route-output-root>/<case-id>. Reusable OSM/TILE_FULL authority remains under "
            "<osm-raw-root>/<NLSC tile>/current.json and is never stored inside this derived-output tree."
        ),
    )
    parser.add_argument(
        "--write-route-analysis-source-geojson",
        action="store_true",
        help=(
            "Diagnostic only: write the complete spatial ROUTE_ANALYSIS_SUBSET before semantic layer splitting "
            "as route_analysis_source_all.geojson. This is useful for proving whether an OSM way was lost "
            "during spatial extraction or during layer construction."
        ),
    )
    ...
    parser.add_argument(
        ...
        default="auto",
        help=(
            "auto: resolve local TILE_FULL through OSM Source Store v2 current.json and only use Overpass "
            "when explicitly allowed; local: current.json only, no network; overpass: explicitly use Overpass."
        ),
    )
    parser.add_argument(
        "--data-dir",
        default="data/osm",
        help=(
            "Compatibility/Overpass data root. Normal local/auto Source Store v2 runs do not scan this root. "
    ...
    parser.add_argument(
        ...
        default="osm_raw",
        help=(
            "OSM Source Store v2 root. IA1 resolves reusable TILE_FULL source as "
            "<osm-raw-root>/<NLSC tile>/current.json."
        ),
    )
    parser.add_argument(
        "--allow-legacy-index-fallback",
        action="store_true",
        help=(
            "Compatibility only: if Source Store v2 current.json is missing for a required tile, "
            "allow fallback to the old data/osm dataset index. Default is fail-closed."
        ),
    )
    parser.add_argument(
        "--dataset-index",
        default=None,
        help=(
            "Compatibility-only legacy dataset index JSON. Default when legacy fallback is explicitly enabled: "
            "<data-dir>/osm_dataset_index.json."
        ),
    )
    parser.add_argument(
        "--pbf-fp",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Cached NLSC theoretical tile index CSV. Default: <osm-raw-root>/_index/nlsc_tile_index.csv; "
            "normal Source Store v2 runs do not read data/osm/tile_index."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Compatibility-only legacy IA1 output root to scan for osm_raw_fetch_manifest.csv bundles. "
            "Used only with --allow-legacy-index-fallback. Can be repeated."
        ),
    )
...
SOURCE_STORE_SCHEMA = "mountain-osm-source-store/v2"


def _read_json_utf8_sig(fp: Path) -> dict:
    try:
        return json.loads(fp.read_text(encoding="utf-8-sig"))
    except Exception as exc:
        raise RuntimeError(f"OSM_SOURCE_STORE_INVALID_JSON: {fp}: {exc}") from exc


def _source_store_path(tile_dir: Path, value: str | None, field_name: str) -> Path:
    if not value:
        raise RuntimeError(
            f"OSM_SOURCE_STORE_INVALID_CURRENT: missing {field_name} in {tile_dir / 'current.json'}"
        )
    fp = Path(str(value))
...
def load_osm_source_store_v2_current(
    tile_ids: list[str],
    osm_raw_root: Path,
    *,
    required: bool,
) -> dict | None:
    """Resolve all required NLSC tiles through osm_raw/<tile>/current.json."""
    osm_raw_root = osm_raw_root.resolve()
    records: list[dict] = []
    missing_current: list[Path] = []

    for tile_id in tile_ids:
        tile_id = str(tile_id).upper()
        tile_dir = osm_raw_root / tile_id
        current_fp = tile_dir / "current.json"
        if not current_fp.exists():
            missing_current.append(current_fp)
            continue

        current = _read_json_utf8_sig(current_fp)
        if current.get("schema") != SOURCE_STORE_SCHEMA:
            raise RuntimeError(
                "OSM_SOURCE_STORE_SCHEMA_MISMATCH: "
                f"{current_fp} schema={current.get('schema')!r}, expected={SOURCE_STORE_SCHEMA!r}"
            )
        if str(current.get("tile_id") or "").upper() != tile_id:
            raise RuntimeError(
                "OSM_SOURCE_STORE_TILE_MISMATCH: "
                f"{current_fp} declares tile_id={current.get('tile_id')!r}, expected={tile_id!r}"
            )
        if str(current.get("status") or "").upper() != "CURRENT":
            raise RuntimeError(
                f"OSM_SOURCE_STORE_NOT_CURRENT: {current_fp} status={current.get('status')!r}"
            )

        raw_fp = _source_store_path(tile_dir, current.get("raw_path"), "raw_path")
        semantic_fp = _source_store_path(tile_dir, current.get("semantic_path"), "semantic_path")
        if not raw_fp.exists():
            raise RuntimeError(f"OSM_SOURCE_STORE_RAW_MISSING: {raw_fp}")
        if not semantic_fp.exists():
            raise RuntimeError(f"OSM_SOURCE_STORE_SEMANTIC_MISSING: {semantic_fp}")

        raw_sha = sha256_file(raw_fp).upper()
        expected_raw_sha = str(current.get("current_raw_sha256") or "").strip().upper()
        if expected_raw_sha and raw_sha != expected_raw_sha:
            raise RuntimeError(
                "OSM_SOURCE_STORE_RAW_SHA256_MISMATCH: "
                f"tile={tile_id} expected={expected_raw_sha} actual={raw_sha}"
            )

        raw_id = str(current.get("current_raw_id") or "").strip().upper()
        if raw_id and len(raw_id) == 8 and all(c in "0123456789ABCDEF" for c in raw_id):
            if not raw_sha.startswith(raw_id):
                raise RuntimeError(
                    "OSM_SOURCE_STORE_RAW_ID_MISMATCH: "
                    f"tile={tile_id} current_raw_id={raw_id} raw_sha256={raw_sha}"
                )

        semantic_profile = str(current.get("current_semantic_profile") or "").strip()
        if not semantic_profile:
            raise RuntimeError(
                f"OSM_SOURCE_STORE_INVALID_CURRENT: missing current_semantic_profile in {current_fp}"
            )
        semantic_sha = sha256_file(semantic_fp).upper()
        qa_manifest_fp = semantic_fp.parent / "tile_qa" / "osm_tile_full_manifest.json"
        qa_manifest = _read_json_utf8_sig(qa_manifest_fp) if qa_manifest_fp.exists() else {}

        records.append(
            {
                "tile_id": tile_id,
                "current_json": portable_path(current_fp),
                "snapshot_date": current.get("current_snapshot_date"),
                "raw_id": raw_id or raw_sha[:8],
                "raw_sha256": raw_sha,
                "semantic_sha256": semantic_sha,
                "semantic_profile": semantic_profile,
                "raw_path": portable_path(raw_fp),
                "semantic_path": portable_path(semantic_fp),
                "qa_manifest_path": portable_path(qa_manifest_fp) if qa_manifest_fp.exists() else None,
                "tile_dataset_profile": qa_manifest.get("tile_dataset_profile"),
                "extract_strategy": qa_manifest.get("extract_strategy"),
                "relation_completion_profile": qa_manifest.get("relation_completion_profile"),
            }
        )
    ...
    if missing_current:
        if required:
            raise RuntimeError(
                "OSM_SOURCE_STORE_CURRENT_MISSING: "
                + "; ".join(str(fp) for fp in missing_current)
            )
        return None
    if len(records) != len(tile_ids):
        if required:
            raise RuntimeError("OSM_SOURCE_STORE_INCOMPLETE: not all required NLSC tiles resolved.")
        return None
    ...
    aggregate_id = hashlib.sha256(seed).hexdigest()[:12]
    raw_sha_value = (
        records[0]["raw_sha256"]
        if len(records) == 1
        else hashlib.sha256(";".join(r["raw_sha256"] for r in records).encode("utf-8")).hexdigest().upper()
    )
    semantic_sha_value = (
        records[0]["semantic_sha256"]
        if len(records) == 1
        else hashlib.sha256(";".join(r["semantic_sha256"] for r in records).encode("utf-8")).hexdigest().upper()
    )
    dataset_id = (
        f"osmstore_v2_{records[0]['tile_id']}_{records[0]['raw_id']}"
        if len(records) == 1
        else f"osmstore_v2_{'__'.join(r['tile_id'] for r in records)}_{aggregate_id}"
    )

    profile_values = {r.get("tile_dataset_profile") for r in records if r.get("tile_dataset_profile")}
    extract_values = {r.get("extract_strategy") for r in records if r.get("extract_strategy")}
    relation_values = {r.get("relation_completion_profile") for r in records if r.get("relation_completion_profile")}

    return {
        "dataset_id": dataset_id,
        "kind": "osm_source_store_v2",
        "path": records[0]["raw_path"],
        "semantic_pbf": records[0]["semantic_path"],
        "nlsc_tiles": [r["tile_id"] for r in records],
        "tile_authority": "NLSC_THEORETICAL_GRID",
        "source_store_schema": SOURCE_STORE_SCHEMA,
        "source_store_root": portable_path(osm_raw_root),
        "source_store_action": "REUSE_LOCAL_CURRENT",
        "source_store_tiles": records,
        "source_dataset_id": ";".join(f"{r['tile_id']}:{r['raw_id']}" for r in records),
        "source_url": None,
        "sha256": raw_sha_value,
        "semantic_sha256": semantic_sha_value,
        "fetched_at": records[0].get("snapshot_date") if len(records) == 1 else None,
        "osm_base_timestamp": None,
        "tile_dataset_profile": next(iter(profile_values)) if len(profile_values) == 1 else None,
        "extract_strategy": next(iter(extract_values)) if len(extract_values) == 1 else None,
        "relation_completion_profile": next(iter(relation_values)) if len(relation_values) == 1 else None,
        "shared_qa": {"coverage": "PASS", "connectivity": "NOT_APPLICABLE_TILE_DATASET"},
    }


def ensure_osm_source_store_v2_tile_qa(
    entry: dict | None,
    args: argparse.Namespace,
    tile_matches: list[dict],
) -> dict:
    """Generate/refresh TILE_FULL QA artifacts inside the CURRENT Source Store v2 snapshot.

    Normal runs remain read-only. Heavy GeoJSON/HTML generation occurs only when
    --write-tile-full-highway-geojson is explicitly requested. Existing manifests
    remain reusable and are augmented rather than relocated to legacy data/osm paths.
    """
    if not entry or entry.get("kind") != "osm_source_store_v2":
        return {}

    # Fast path: normal runs only read the CURRENT manifest(s).
    if not args.write_tile_full_highway_geojson:
        return read_osm_source_store_v2_tile_qa(entry)

    match_by_tile = {str(item.get("tile_id")): item for item in tile_matches}
    osmium = require_osmium(args)

    for record in entry.get("source_store_tiles", []):
        tile_id = str(record.get("tile_id"))
        raw_pbf = resolve_path(record.get("raw_path"))
        semantic_pbf = resolve_path(record.get("semantic_path"))
        if not raw_pbf.exists() or not semantic_pbf.exists():
            raise RuntimeError(f"OSM_SOURCE_STORE_TILE_QA_SOURCE_MISSING: tile={tile_id}")

        qa_dir = semantic_pbf.parent / "tile_qa"
        ...
        highway_map = qa_dir / "osm_tile_full_highway_map.html"

        cached = {}
        if manifest_fp.exists() and not args.force_refresh:
            try:
                cached = _read_json_utf8_sig(manifest_fp)
            except Exception:
                cached = {}

        # Keep an exact tag-count summary next to the CURRENT semantic snapshot.
        if args.force_refresh or not summary_fp.exists():
            rows: list[dict] = []
            ...
            try:
                rows = pd.read_csv(summary_fp).to_dict("records")
            except Exception:
                rows = []

        all_total = int(sum(int(r.get("count", 0)) for r in rows if r.get("object_scope") == "all"))
        way_total = int(sum(int(r.get("count", 0)) for r in rows if r.get("object_scope") == "way"))
        node_total = int(sum(int(r.get("count", 0)) for r in rows if r.get("object_scope") == "node"))
        relation_total = int(sum(int(r.get("count", 0)) for r in rows if r.get("object_scope") == "relation"))

        route_relation_entities = cached.get("route_relation_entities") if isinstance(cached.get("route_relation_entities"), dict) else {}
        if args.force_refresh or not route_relation_entities:
            route_relation_entities = {}
            for route in ("hiking", "foot"):
                expression = f"route={route}"
                raw_count = osmium_tag_match_total(
                    osmium, raw_pbf, expression, object_type="relation", timeout=args.download_timeout
                )
        ...
        if args.force_refresh or not highway_geojson.exists():
            run_command(
        ...
        match = match_by_tile.get(tile_id)
        theoretical_geom = None
        fetch_geom = None
        if match is not None:
            theoretical_geom, fetch_geom = build_nlsc_tile_geometry([match], args.tile_fetch_buffer_m)
        ...
        try:
            ...
            print(f"WARNING: Source Store v2 TILE_FULL highway map could not be written for {tile_id}: {exc}")
        ...
        qa.update(
            {
                ...
                "dataset_id": f"osmstore_v2_{tile_id}_{record.get('raw_id')}",
                "nlsc_tiles": [tile_id],
                "source_store_schema": SOURCE_STORE_SCHEMA,
                "source_store_snapshot_date": record.get("snapshot_date"),
                "source_store_raw_id": record.get("raw_id"),
                "raw_pbf": portable_path(raw_pbf),
                ...
                "optional_highway_way_geojson": portable_path(highway_geojson) if highway_geojson.exists() else None,
                "optional_highway_map": portable_path(highway_map) if highway_map.exists() else None,
                "tile_theoretical_bbox": list(geometry_bbox(theoretical_geom)) if theoretical_geom is not None else cached.get("tile_theoretical_bbox"),
                "osm_fetch_bbox": list(geometry_bbox(fetch_geom)) if fetch_geom is not None else cached.get("osm_fetch_bbox"),
        ...
        record["qa_manifest_path"] = portable_path(manifest_fp)

    return read_osm_source_store_v2_tile_qa(entry)


def print_osm_source_store_v2(entry: dict, osm_raw_root: Path) -> None:
    print("OSM SOURCE STORE: v2")
    print("  root:", osm_raw_root.resolve())
    for record in entry.get("source_store_tiles", []):
        print("  tile:", record.get("tile_id"))
        print("    current:", resolve_path(record.get("current_json")))
        print("    snapshot:", record.get("snapshot_date"))
        print("    raw id:", record.get("raw_id"))
        print("    raw sha256:", record.get("raw_sha256"))
        print("    raw:", resolve_path(record.get("raw_path")))
        print("    semantic profile:", record.get("semantic_profile"))
        print("    semantic sha256:", record.get("semantic_sha256"))
        print("    semantic:", resolve_path(record.get("semantic_path")))
    print("  source action:", entry.get("source_store_action"))


def read_osm_source_store_v2_tile_qa(entry: dict | None) -> dict:
    if not entry or entry.get("kind") != "osm_source_store_v2":
        return {}
    tile_manifests: dict[str, dict] = {}
    for record in entry.get("source_store_tiles", []):
        qa_rel = record.get("qa_manifest_path")
        if not qa_rel:
            continue
        qa_fp = resolve_path(qa_rel)
        if qa_fp.exists():
            tile_manifests[str(record.get("tile_id"))] = _read_json_utf8_sig(qa_fp)
    if not tile_manifests:
        return {}
    if len(tile_manifests) == 1:
        return next(iter(tile_manifests.values()))

    relation_qa: dict[str, dict] = {}
    for route in ("hiking", "foot"):
        statuses = []
        for manifest in tile_manifests.values():
            item = (manifest.get("route_relation_entities") or {}).get(route) or {}
            if item.get("semantic_retention_status"):
                statuses.append(str(item["semantic_retention_status"]).upper())
        relation_qa[route] = {
            "raw_relation_entities": None,
            "semantic_relation_entities": None,
            "semantic_retention_status": (
                "PASS" if statuses and all(s == "PASS" for s in statuses) else "UNKNOWN"
            ),
            "count_semantics": "PER_TILE_ONLY_NOT_SUMMED_DUE_TO_POSSIBLE_BUFFER_OVERLAP",
        }

    return {
        "qa_schema": "mountain-osm-source-store-multitile-qa/v1",
        "data_scope": "TILE_FULL",
        "nlsc_tiles": sorted(tile_manifests),
        "highway_tagged_objects_total": None,
        "highway_way_total": None,
        "relation_completion_profile": "MULTI_TILE_SEE_TILE_MANIFESTS",
        "route_relation_entities": relation_qa,
        "tile_manifests": tile_manifests,
    }
...
def _contiguous_segment_ranges(outside_segments: list[dict]) -> list[dict]:
    """Collapse uncovered segment indices into compact contiguous ranges."""
    if not outside_segments:
        return []
    ordered = sorted(outside_segments, key=lambda item: int(item["segment_index"]))
    ranges: list[dict] = []
    current = None
    for item in ordered:
        idx = int(item["segment_index"])
        if current is None or idx != current["end_index"] + 1:
            ...
            current = {
                ...
                "end_index": idx,
                "segment_count": 1,
                "summed_length_m": float(item["segment_length_m"]),
                "summed_outside_length_m": float(item["outside_length_m"]),
            }
        else:
            current["end_index"] = idx
            current["segment_count"] += 1
            current["summed_length_m"] += float(item["segment_length_m"])
            current["summed_outside_length_m"] += float(item["outside_length_m"])
    if current is not None:
        ranges.append(current)
    return ranges


def _range_lines_from_coords(coords: list[tuple[float, float]], ranges: list[dict]) -> list[dict]:
    result = []
    for item in ranges:
        start_idx = int(item["start_index"])
        end_idx = int(item["end_index"])
        line_coords = coords[start_idx : end_idx + 2]
        if len(line_coords) < 2:
            continue
        geom = LineString(line_coords)
        result.append({**item, "geometry": geom, "length_m": float(geom.length)})
    return result


def _segment_reverse_overlap_qa(
    outside_segments: list[dict],
    *,
    tolerance_m: float = 0.5,
) -> dict:
    """Diagnostic-only reversed segment pairing using quantized endpoints.

    The tolerance is not a routing/matching threshold and never affects query geometry.
    It only asks whether uncovered GPX segments are repeated in the opposite direction.
    """
    if tolerance_m <= 0:
        tolerance_m = 0.5

    def point_key(x: float, y: float) -> tuple[int, int]:
        return (round(float(x) / tolerance_m), round(float(y) / tolerance_m))

    buckets: dict[tuple[tuple[int, int], tuple[int, int]], list[dict]] = {}
    for item in outside_segments:
        a = point_key(item["start_x"], item["start_y"])
        b = point_key(item["end_x"], item["end_y"])
        buckets.setdefault((a, b), []).append(item)
    ...
    for item in outside_segments:
        ...
        a = point_key(item["start_x"], item["start_y"])
        b = point_key(item["end_x"], item["end_y"])
        candidates = buckets.get((b, a), [])
        mate = next(
            (
                other
                for other in candidates
                if int(other["segment_index"]) != idx
                and int(other["segment_index"]) not in used_indices
            ),
            None,
        )
        if mate is None:
            continue
        mate_idx = int(mate["segment_index"])
        used_indices.add(idx)
        used_indices.add(mate_idx)
        pair_count += 1
        paired_length_m += min(float(item["segment_length_m"]), float(mate["segment_length_m"]))
        if len(pair_examples) < 10:
            pair_examples.append({
                "segment_index_a": idx,
                "segment_index_b": mate_idx,
                "length_a_m": float(item["segment_length_m"]),
                "length_b_m": float(mate["segment_length_m"]),
            })

    return {
        "tolerance_m": float(tolerance_m),
        "reversed_segment_pair_count": pair_count,
        "reversed_segment_paired_physical_length_m": paired_length_m,
        "reversed_segment_pair_examples": pair_examples,
    }


def _reversed_range_overlap_qa(range_lines: list[dict], *, tolerance_m: float = 5.0) -> dict:
    """Find the best pair of uncovered contiguous ranges that retrace in reverse."""
    best = None
    for i in range(len(range_lines)):
        a = range_lines[i]
        ga = a["geometry"]
        a_coords = list(ga.coords)
        if len(a_coords) < 2:
            continue
        a_start = Point(a_coords[0])
        a_end = Point(a_coords[-1])
        for j in range(i + 1, len(range_lines)):
            b = range_lines[j]
            gb = b["geometry"]
            b_coords = list(gb.coords)
            if len(b_coords) < 2:
                continue
            b_start = Point(b_coords[0])
            b_end = Point(b_coords[-1])
            start_to_reverse_end_m = float(a_start.distance(b_end))
            end_to_reverse_start_m = float(a_end.distance(b_start))
            hausdorff_m = float(ga.hausdorff_distance(gb))
            score = max(start_to_reverse_end_m, end_to_reverse_start_m, hausdorff_m)
            candidate = {
                "range_a_start_index": int(a["start_index"]),
                "range_a_end_index": int(a["end_index"]),
                "range_b_start_index": int(b["start_index"]),
                "range_b_end_index": int(b["end_index"]),
                "range_a_length_m": float(ga.length),
                "range_b_length_m": float(gb.length),
                "start_to_reverse_end_m": start_to_reverse_end_m,
                "end_to_reverse_start_m": end_to_reverse_start_m,
                "hausdorff_distance_m": hausdorff_m,
                "score_m": score,
            }
            if best is None or candidate["score_m"] < best["score_m"]:
                best = candidate

    detected = bool(best is not None and best["score_m"] <= tolerance_m)
    return {
        "diagnostic_tolerance_m": float(tolerance_m),
        "reversed_range_pair_detected": detected,
        "best_reversed_range_pair": best,
        "best_reversed_range_overlap_length_m": (
            min(best["range_a_length_m"], best["range_b_length_m"])
            if detected and best is not None else 0.0
        ),
    }


def build_query_geometry_with_self_qa(
    line: LineString,
    buffer_m: float,
    simplify_m: float,
    out_dir: Path | None = None,
) -> tuple[object, dict]:
    """Build IA1 query geometry with a conservative adaptive contract.

    v1.5.12 contract:
    1. Preserve the original GPX LineString as traversal/activity geometry.
    2. First build the historical direct route buffer (v1.5.9 behavior).
    3. Accept the direct buffer only if both raw and simplified buffers are valid,
       non-empty, and fully cover the source GPX with zero outside length.
    4. Only when the direct construction violates that invariant, rebuild the
       spatial query centerline from non-zero adjacent segments via unary_union
       and buffer that dissolved spatial footprint.
    5. The fallback result must pass the same hard invariants or IA1 aborts.

    Buffer distance, simplify distance, metric CRS selection, and downstream
    geometry.intersects(query_geometry) remain unchanged.
    """
    route_gdf = gpd.GeoDataFrame([{"geometry": line}], geometry="geometry", crs="EPSG:4326")
    metric_crs = route_gdf.estimate_utm_crs()
    route_m = route_gdf.to_crs(metric_crs).geometry.iloc[0]
    coords = list(route_m.coords) if hasattr(route_m, "coords") else []
    route_length_m = float(route_m.length)
    vertices_total = len(coords)
    segments_total = max(0, vertices_total - 1)

    non_finite_coordinate_count = sum(
        1 for x, y in coords if not (math.isfinite(float(x)) and math.isfinite(float(y)))
    )
    zero_length_segment_count = 0
    max_segment_length_m = 0.0
    all_segments = []
    for idx in range(segments_total):
        segment = LineString([coords[idx], coords[idx + 1]])
        length_m = float(segment.length)
        if length_m == 0.0:
            zero_length_segment_count += 1
            continue
        max_segment_length_m = max(max_segment_length_m, length_m)
        all_segments.append(segment)

    if non_finite_coordinate_count:
        raise RuntimeError(
            f"QUERY_GEOMETRY_INVALID_COORDINATES: non-finite metric coordinates={non_finite_coordinate_count}"
        )
    if not all_segments:
        raise RuntimeError("QUERY_GEOMETRY_EMPTY_CENTERLINE: GPX has no non-zero metric segments.")
    ...
    def outside_length(route_geom, buffer_geom) -> tuple[object, float]:
        outside = route_geom.difference(buffer_geom)
        length_m = float(outside.length) if not outside.is_empty else 0.0
        return outside, length_m

    # ------------------------------------------------------------------
    # Stage A: direct route buffer. This is the v1.5.9 geometry contract
    # and remains the preferred production path when its invariants pass.
    # ------------------------------------------------------------------
    direct_raw_buffer_m = route_m.buffer(buffer_m)
    direct_simplified_buffer_m = direct_raw_buffer_m
    if simplify_m and simplify_m > 0:
        direct_simplified_buffer_m = direct_raw_buffer_m.simplify(
            simplify_m, preserve_topology=True
        )

    direct_raw_outside_m, direct_raw_outside_length_m = outside_length(route_m, direct_raw_buffer_m)
    direct_simplified_outside_m, direct_simplified_outside_length_m = outside_length(
        route_m, direct_simplified_buffer_m
    )
    direct_raw_covers = bool(direct_raw_buffer_m.covers(route_m))
    direct_simplified_covers = bool(direct_simplified_buffer_m.covers(route_m))
    direct_raw_ok = bool(
        direct_raw_buffer_m.is_valid
        and not direct_raw_buffer_m.is_empty
        and direct_raw_covers
        and direct_raw_outside_length_m <= 1e-6
    )
    direct_simplified_ok = bool(
        direct_simplified_buffer_m.is_valid
        and not direct_simplified_buffer_m.is_empty
        and direct_simplified_covers
        and direct_simplified_outside_length_m <= 1e-6
    )
    direct_contract_pass = bool(direct_raw_ok and direct_simplified_ok)

    direct_failure_reasons = []
    if not direct_raw_buffer_m.is_valid:
        direct_failure_reasons.append("DIRECT_RAW_BUFFER_INVALID")
    if direct_raw_buffer_m.is_empty:
        direct_failure_reasons.append("DIRECT_RAW_BUFFER_EMPTY")
    if not direct_raw_covers or direct_raw_outside_length_m > 1e-6:
        direct_failure_reasons.append("DIRECT_RAW_BUFFER_DID_NOT_COVER_ROUTE")
    if not direct_simplified_buffer_m.is_valid:
        direct_failure_reasons.append("DIRECT_SIMPLIFIED_BUFFER_INVALID")
    if direct_simplified_buffer_m.is_empty:
        direct_failure_reasons.append("DIRECT_SIMPLIFIED_BUFFER_EMPTY")
    if not direct_simplified_covers or direct_simplified_outside_length_m > 1e-6:
        direct_failure_reasons.append("DIRECT_SIMPLIFIED_BUFFER_DID_NOT_COVER_ROUTE")

    # Root-cause evidence is always computed against the direct construction.
    direct_outside_segments = []
    for idx in range(segments_total):
        segment = LineString([coords[idx], coords[idx + 1]])
        if direct_raw_buffer_m.covers(segment):
            continue
        outside = segment.difference(direct_raw_buffer_m)
        direct_outside_segments.append(
            {
                "segment_index": idx,
                "segment_length_m": float(segment.length),
                "outside_length_m": float(outside.length) if not outside.is_empty else 0.0,
                "start_x": float(coords[idx][0]),
                "start_y": float(coords[idx][1]),
                "end_x": float(coords[idx + 1][0]),
                "end_y": float(coords[idx + 1][1]),
            }
        )

    uncovered_ranges = _contiguous_segment_ranges(direct_outside_segments)
    range_lines = _range_lines_from_coords(coords, uncovered_ranges)
    reversed_segment_qa = _segment_reverse_overlap_qa(direct_outside_segments, tolerance_m=0.5)
    reversed_range_qa = _reversed_range_overlap_qa(range_lines, tolerance_m=5.0)
    direct_outside_segment_sum_m = float(
        sum(item["outside_length_m"] for item in direct_outside_segments)
    )
    direct_collapsed_outside_ratio = (
        direct_outside_segment_sum_m / direct_raw_outside_length_m
        if direct_raw_outside_length_m > 0 else None
    )
    reversed_overlap_detected = bool(
        reversed_segment_qa["reversed_segment_pair_count"] > 0
        or reversed_range_qa["reversed_range_pair_detected"]
    )
    reversed_overlap_length_m = max(
        float(reversed_segment_qa["reversed_segment_paired_physical_length_m"]),
        float(reversed_range_qa["best_reversed_range_overlap_length_m"]),
    )

    # ------------------------------------------------------------------
    # Stage B: choose production strategy. Direct is authoritative when it
    # passes. Dissolved-segment geometry is a recovery path, not the default.
    # ------------------------------------------------------------------
    query_centerline_m = route_m
    fallback_triggered = not direct_contract_pass
    fallback_reason = ";".join(direct_failure_reasons) if direct_failure_reasons else None

    if direct_contract_pass:
        strategy = "DIRECT_ROUTE_BUFFER"
        raw_buffer_m = direct_raw_buffer_m
        simplified_buffer_m = direct_simplified_buffer_m
    else:
        strategy = "DISSOLVED_SEGMENT_FALLBACK"
        query_centerline_m = unary_union(all_segments)
        raw_buffer_m = query_centerline_m.buffer(buffer_m)
        simplified_buffer_m = raw_buffer_m
        if simplify_m and simplify_m > 0:
            simplified_buffer_m = raw_buffer_m.simplify(simplify_m, preserve_topology=True)

    raw_covers_route = bool(raw_buffer_m.covers(route_m))
    simplified_covers_route = bool(simplified_buffer_m.covers(route_m))
    route_outside_raw_buffer_m, route_outside_raw_length_m = outside_length(route_m, raw_buffer_m)
    route_outside_simplified_buffer_m, route_outside_simplified_length_m = outside_length(
        route_m, simplified_buffer_m
    )

    if (
        not raw_buffer_m.is_valid
        or raw_buffer_m.is_empty
        or not raw_covers_route
        or route_outside_raw_length_m > 1e-6
    ):
        raise RuntimeError(
            "QUERY_GEOMETRY_INVARIANT_FAIL: production raw query buffer does not fully cover the source GPX "
            f"(strategy={strategy}, valid={raw_buffer_m.is_valid}, empty={raw_buffer_m.is_empty}, "
            f"covers={raw_covers_route}, outside_length_m={route_outside_raw_length_m})."
        )
    if (
        not simplified_buffer_m.is_valid
        or simplified_buffer_m.is_empty
        or not simplified_covers_route
        or route_outside_simplified_length_m > 1e-6
    ):
        raise RuntimeError(
            "QUERY_GEOMETRY_INVARIANT_FAIL: production simplified query buffer does not fully cover the source GPX "
            f"(strategy={strategy}, valid={simplified_buffer_m.is_valid}, empty={simplified_buffer_m.is_empty}, "
            f"covers={simplified_covers_route}, outside_length_m={route_outside_simplified_length_m})."
        )

    query_geometry = gpd.GeoSeries(
        [simplified_buffer_m], crs=metric_crs
    ).to_crs("EPSG:4326").iloc[0]
    direct_query_geometry = gpd.GeoSeries(
        [direct_simplified_buffer_m], crs=metric_crs
    ).to_crs("EPSG:4326").iloc[0]
    production_vs_direct_symdiff = query_geometry.symmetric_difference(direct_query_geometry)

    root_cause_qa = {
        "route_is_simple": bool(route_m.is_simple),
        "coordinate_count": vertices_total,
        ...
        "direct_uncovered_segment_ranges": [
            {k: v for k, v in item.items() if k != "geometry"} for item in range_lines
        ],
        "direct_uncovered_range_count": len(range_lines),
        "direct_outside_segment_sum_to_difference_ratio": direct_collapsed_outside_ratio,
        "reversed_overlap_detected": reversed_overlap_detected,
        "reversed_overlap_length_m": reversed_overlap_length_m,
        "reversed_segment_qa": reversed_segment_qa,
        "reversed_range_qa": reversed_range_qa,
    }
    ...
    query_fix_qa = {
        ...
        "direct_contract_pass": direct_contract_pass,
        "direct_raw_buffer_geom_type": direct_raw_buffer_m.geom_type,
        "direct_raw_buffer_valid": bool(direct_raw_buffer_m.is_valid),
        "direct_raw_buffer_components": component_count(direct_raw_buffer_m),
        "direct_raw_buffer_area_m2": float(direct_raw_buffer_m.area),
        "direct_raw_buffer_covers_route": direct_raw_covers,
        "direct_route_outside_raw_buffer_length_m": direct_raw_outside_length_m,
        "direct_simplified_buffer_geom_type": direct_simplified_buffer_m.geom_type,
        "direct_simplified_buffer_valid": bool(direct_simplified_buffer_m.is_valid),
        "direct_simplified_buffer_components": component_count(direct_simplified_buffer_m),
        "direct_simplified_buffer_covers_route": direct_simplified_covers,
        "direct_route_outside_simplified_buffer_length_m": direct_simplified_outside_length_m,
        "query_centerline_geom_type": query_centerline_m.geom_type,
        "query_centerline_valid": bool(query_centerline_m.is_valid),
        "query_centerline_length_m": float(query_centerline_m.length),
        "raw_buffer_geom_type": raw_buffer_m.geom_type,
        "raw_buffer_valid": bool(raw_buffer_m.is_valid),
        "raw_buffer_components": component_count(raw_buffer_m),
        "raw_buffer_area_m2": float(raw_buffer_m.area),
        "raw_buffer_covers_route": raw_covers_route,
        "route_outside_raw_buffer_length_m": route_outside_raw_length_m,
        "simplified_buffer_geom_type": simplified_buffer_m.geom_type,
        "simplified_buffer_valid": bool(simplified_buffer_m.is_valid),
        "simplified_buffer_components": component_count(simplified_buffer_m),
        "simplified_buffer_covers_route": simplified_covers_route,
        "route_outside_simplified_buffer_length_m": route_outside_simplified_length_m,
        "direct_query_equals_production": bool(query_geometry.equals(direct_query_geometry)),
        "production_vs_direct_symmetric_difference_empty": bool(production_vs_direct_symdiff.is_empty),
        "production_vs_direct_symmetric_difference_area_degrees2": float(production_vs_direct_symdiff.area),
    }

    direct_qa = {
        "raw_buffer_geom_type": direct_raw_buffer_m.geom_type,
        "raw_buffer_valid": bool(direct_raw_buffer_m.is_valid),
        "raw_buffer_components": component_count(direct_raw_buffer_m),
        "raw_buffer_area_m2": float(direct_raw_buffer_m.area),
        "raw_buffer_covers_route": direct_raw_covers,
        "route_outside_raw_buffer_geom_type": direct_raw_outside_m.geom_type,
        "route_outside_raw_buffer_empty": bool(direct_raw_outside_m.is_empty),
        "route_outside_raw_buffer_length_m": direct_raw_outside_length_m,
        "simplified_buffer_covers_route": direct_simplified_covers,
        "route_outside_simplified_buffer_length_m": direct_simplified_outside_length_m,
        "segments_with_any_part_outside_raw_buffer": len(direct_outside_segments),
        "outside_segment_length_m": direct_outside_segment_sum_m,
        "outside_segments_first20": direct_outside_segments[:20],
    }

    qa = {
        "metric_crs": str(metric_crs),
        "route_geom_type": route_m.geom_type,
        "route_valid": bool(route_m.is_valid),
        "route_empty": bool(route_m.is_empty),
        "route_length_m": route_length_m,
        "vertices_total": vertices_total,
        "segments_total": segments_total,
        "query_geometry_contract": "ADAPTIVE_DIRECT_THEN_DISSOLVED_FALLBACK",
        "query_geometry_fix_qa": query_fix_qa,
        "direct_route_buffer_qa": direct_qa,
        # Compatibility alias for v1.5.11-era manifests/debug readers.
        "legacy_v1_5_9_buffer_qa": direct_qa,
        "query_geometry_root_cause_qa": root_cause_qa,
    }

    if out_dir is not None:
        diagnostic_features = [
            {"debug_role": "route", "geometry": route_m},
            {"debug_role": "direct_route_centerline", "geometry": route_m},
            {"debug_role": f"direct_raw_buffer_{int(buffer_m)}m", "geometry": direct_raw_buffer_m},
            {"debug_role": f"production_raw_buffer_{int(buffer_m)}m", "geometry": raw_buffer_m},
            {"debug_role": f"production_simplified_buffer_{int(simplify_m)}m", "geometry": simplified_buffer_m},
        ]
        if fallback_triggered:
            diagnostic_features.append(
                {"debug_role": "fallback_query_centerline_dissolved", "geometry": query_centerline_m}
            )
        if not direct_raw_outside_m.is_empty:
            diagnostic_features.append(
                {"debug_role": "direct_route_outside_raw_buffer", "geometry": direct_raw_outside_m}
            )
        diagnostic_gdf = gpd.GeoDataFrame(
            diagnostic_features, geometry="geometry", crs=metric_crs
        ).to_crs("EPSG:4326")
        diagnostic_gdf.to_file(out_dir / "query_geometry_self_qa.geojson", driver="GeoJSON")

        root_features = [
            {"debug_role": "direct_route_centerline", "geometry": route_m},
            {"debug_role": f"direct_buffer_{int(buffer_m)}m", "geometry": direct_raw_buffer_m},
            {"debug_role": f"production_buffer_{int(buffer_m)}m", "geometry": raw_buffer_m},
        ]
        if fallback_triggered:
        ...
        for item in range_lines:
            root_features.append(
                {
                    "debug_role": f"direct_uncovered_range_{item['start_index']}_{item['end_index']}",
                    "geometry": item["geometry"],
                }
            )
        gpd.GeoDataFrame(root_features, geometry="geometry", crs=metric_crs).to_crs(
            "EPSG:4326"
        ).to_file(out_dir / "query_geometry_root_cause_qa.geojson", driver="GeoJSON")

    return query_geometry, qa

def print_query_geometry_self_qa(qa: dict) -> None:
    print("QUERY_GEOMETRY_SELF_QA")
    for key in [
        "metric_crs",
        "route_geom_type",
        "route_valid",
        ...
        print(f"  {key}: {qa.get(key)}")

    fix = qa.get("query_geometry_fix_qa") or {}
    print("QUERY_GEOMETRY_FIX_QA")
    for key in [
        "strategy",
        "fallback_triggered",
        "fallback_reason",
        "direct_contract_pass",
        "direct_raw_buffer_geom_type",
        "direct_raw_buffer_valid",
        "direct_raw_buffer_components",
        "direct_raw_buffer_area_m2",
        "direct_raw_buffer_covers_route",
        "direct_route_outside_raw_buffer_length_m",
        "direct_simplified_buffer_geom_type",
        "direct_simplified_buffer_valid",
        "direct_simplified_buffer_components",
        "direct_simplified_buffer_covers_route",
        "direct_route_outside_simplified_buffer_length_m",
        "query_centerline_geom_type",
        "query_centerline_valid",
        "query_centerline_length_m",
        "raw_buffer_geom_type",
        "raw_buffer_valid",
        "raw_buffer_components",
        "raw_buffer_area_m2",
        "raw_buffer_covers_route",
        "route_outside_raw_buffer_length_m",
        "simplified_buffer_geom_type",
        "simplified_buffer_valid",
        "simplified_buffer_components",
        "simplified_buffer_covers_route",
        "route_outside_simplified_buffer_length_m",
        "direct_query_equals_production",
        "production_vs_direct_symmetric_difference_empty",
        "production_vs_direct_symmetric_difference_area_degrees2",
    ]:
        print(f"  {key}: {fix.get(key)}")

    direct = qa.get("direct_route_buffer_qa") or qa.get("legacy_v1_5_9_buffer_qa") or {}
    print("DIRECT_ROUTE_BUFFER_QA")
    for key in [
        "raw_buffer_geom_type",
        "raw_buffer_valid",
        "raw_buffer_components",
        "raw_buffer_area_m2",
        "raw_buffer_covers_route",
        "route_outside_raw_buffer_geom_type",
        "route_outside_raw_buffer_empty",
        "route_outside_raw_buffer_length_m",
        "simplified_buffer_covers_route",
        "route_outside_simplified_buffer_length_m",
        "segments_with_any_part_outside_raw_buffer",
        "outside_segment_length_m",
    ]:
        print(f"  {key}: {direct.get(key)}")

    root = qa.get("query_geometry_root_cause_qa") or {}
    print("QUERY_GEOMETRY_ROOT_CAUSE_QA")
    for key in [
        "route_is_simple",
        "coordinate_count",
        ...
        "direct_uncovered_range_count",
        "direct_outside_segment_sum_to_difference_ratio",
        "reversed_overlap_detected",
        "reversed_overlap_length_m",
    ]:
        print(f"  {key}: {root.get(key)}")
    ranges = root.get("direct_uncovered_segment_ranges") or []
    if ranges:
        print("  direct_uncovered_segment_ranges:")
        for item in ranges:
            print(
                "    "
                f"start_index={item['start_index']} end_index={item['end_index']} "
                f"segment_count={item['segment_count']} "
                f"summed_length_m={item['summed_length_m']} "
                f"summed_outside_length_m={item['summed_outside_length_m']}"
            )
...
def osmium_executable(osmium_bin: str) -> str | None:
    ...
    if candidate.exists():
        ...
        exe = shutil.which(osmium_bin)
    if exe and os.name == "nt" and exe.lower().endswith(".exe"):
        # Conda's Windows osmium executable treats an uppercase ".EXE" argv[0]
        # as a subcommand. shutil.which("osmium") can return that casing.
        exe = exe[:-4] + ".exe"
    return exe
...
def load_features_from_local_dataset(
    ...
    if entry.get("kind") == "osm_source_store_v2":
        feature_frames = []
        membership_frames = []
        parse_available = False
        for record in entry.get("source_store_tiles", []):
            child_entry = {
                "dataset_id": f"osmstore_v2_{record.get('tile_id')}_{record.get('raw_id')}",
                "kind": "nlsc_tile",
                "path": record.get("raw_path"),
                "semantic_pbf": record.get("semantic_path"),
                "sha256": record.get("raw_sha256"),
                "semantic_sha256": record.get("semantic_sha256"),
                "nlsc_tiles": [record.get("tile_id")],
                "tile_authority": "NLSC_THEORETICAL_GRID",
                "tile_dataset_profile": record.get("tile_dataset_profile"),
                "extract_strategy": record.get("extract_strategy"),
                "relation_completion_profile": record.get("relation_completion_profile"),
                "fetched_at": record.get("snapshot_date"),
                "osm_base_timestamp": None,
            }
            child_gdf, child_memberships, child_available = load_features_from_local_dataset(
                child_entry, query_geometry, data_dir, args
            )
            if child_gdf is not None and not child_gdf.empty:
                feature_frames.append(child_gdf)
            if child_memberships is not None and not child_memberships.empty:
                membership_frames.append(child_memberships)
            parse_available = parse_available or bool(child_available)

        if feature_frames:
            combined = pd.concat(feature_frames, axis=0, sort=False)
            combined = combined[~combined.index.duplicated(keep="first")]
            gdf = gpd.GeoDataFrame(
                combined, geometry="geometry", crs=feature_frames[0].crs or "EPSG:4326"
            )
        else:
            gdf = gpd.GeoDataFrame(geometry=[], crs="EPSG:4326")

        if membership_frames:
            route_relation_memberships = (
        ...
        print("SOURCE_STORE_MULTI_TILE_MERGE_QA")
        print("  tile count:", len(entry.get("source_store_tiles", [])))
        print("  route-subset features after canonical index dedup:", len(gdf))
        print("  route-relation membership rows after dedup:", len(route_relation_memberships))
        return gdf, route_relation_memberships, parse_available
    ...
    with tempfile.TemporaryDirectory(prefix="ia1_osm_xml_") as tmpdir:
        ...
        try:
            tile_gdf = ox.features_from_xml(xml_fp)
        except InsufficientResponseError:
            tile_gdf = gpd.GeoDataFrame(geometry=[], crs="EPSG:4326")
    ...
    elif tile_gdf.crs.to_string().upper() != "EPSG:4326":
        tile_gdf = tile_gdf.to_crs("EPSG:4326")

    tile_gdf = tile_gdf[tile_gdf.geometry.notna() & ~tile_gdf.geometry.is_empty].copy()
    try:
        intersects_mask = tile_gdf.geometry.intersects(query_geometry)
    except Exception as exc:
        raise RuntimeError(
            f"ROUTE_ANALYSIS_SUBSET_CLIP_FAILED: explicit TILE_FULL geometry clip failed: {exc}"
        ) from exc

    gdf = tile_gdf[intersects_mask].copy()
    print("ROUTE_SUBSET_SOURCE_QA")
    print("  TILE_FULL semantic features parsed:", len(tile_gdf))
    print("  ROUTE_ANALYSIS_SUBSET intersecting features:", len(gdf))
    print("  subset rule: explicit shapely geometry.intersects(query_geometry)")
...
def run_basic_connectivity_qa(
    ...
    qa["start_component"] = best_start[1]
    qa["end_component"] = best_end[1]
...
def _normalize_sac_scale_value(sac) -> str | None:
    sac = normalize_scalar(sac)
    if pd.isna(sac):
        return None
    text = str(sac).strip().lower()
    return text or None


def classify_trail_difficulty(sac):
    """Return a broad seven-grade difficulty hint without discarding SAC detail."""
    value = _normalize_sac_scale_value(sac)
    if value is None:
        return "unknown"
    info = SAC_SCALE_INFO.get(value)
    return info["hint"] if info else "unknown_value"


def classify_sac_scale_rank(sac):
    """Canonical OSM sac_scale ordinal: strolling=0, T1..T6=1..6."""
    value = _normalize_sac_scale_value(sac)
    if value is None:
        return pd.NA
    info = SAC_SCALE_INFO.get(value)
    return info["rank"] if info else pd.NA


def classify_sac_scale_t_grade(sac):
    """Canonical SAC label used by current OSM sac_scale documentation."""
    value = _normalize_sac_scale_value(sac)
    if value is None:
        return pd.NA
    info = SAC_SCALE_INFO.get(value)
    return info["t_grade"] if info else pd.NA
...
def normalize_highway(gdf: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    ...
    sac_series = gdf.get("sac_scale", pd.Series(pd.NA, index=gdf.index))
    gdf["trail_difficulty_hint"] = sac_series.apply(classify_trail_difficulty)
    gdf["sac_scale_rank"] = sac_series.apply(classify_sac_scale_rank)
    gdf["sac_scale_t_grade"] = sac_series.apply(classify_sac_scale_t_grade)
...
def _json_safe_osm_tag_value(value):
    """Convert an OSMnx cell to a deterministic JSON-safe value."""
    if value is None:
        return None
    if isinstance(value, dict):
        out = {}
        for key, item in value.items():
            safe = _json_safe_osm_tag_value(item)
            if safe is not None:
                out[str(key)] = safe
        return out or None
...
def _source_osm_tag_columns(source_columns) -> list[str]:
    """Columns originating in the OSM source, excluding OSMnx structure."""
    return sorted(
        str(col)
        for col in source_columns
        if str(col) not in OSMNX_STRUCTURAL_COLUMNS
    )


def attach_osm_extra_tags(
    gdf: gpd.GeoDataFrame,
    source_columns,
) -> tuple[gpd.GeoDataFrame, list[str]]:
    """Preserve every non-schema OSM source tag in osm_tags_extra_json.

    PROTECT_COLS remains the stable normalized schema. This function provides a
    lossless forward-compatibility channel for source tags that IA1 does not yet
    model explicitly, so new/rare OSM keys do not silently disappear on export.
    """
    gdf = gdf.copy()
    source_tag_cols = _source_osm_tag_columns(source_columns)
    extra_cols = [
        col for col in source_tag_cols
        if col not in PROTECT_COLS and col in gdf.columns
    ]
    ...
    for _, row in gdf.iterrows():
        ...
        for col in extra_cols:
            safe = _json_safe_osm_tag_value(row.get(col))
            if safe is not None:
                payload[col] = safe
        payloads.append(
            json.dumps(payload, ensure_ascii=False, sort_keys=True, separators=(",", ":"))
            if payload else pd.NA
        )
        counts.append(len(payload))
        keys_text.append(";".join(sorted(payload)) if payload else pd.NA)

    gdf["osm_extra_tag_count"] = counts
    gdf["osm_extra_tag_keys"] = keys_text
    gdf["osm_tags_extra_json"] = payloads
    return gdf, extra_cols


def build_osm_tag_coverage_qa(
    gdf: gpd.GeoDataFrame,
    source_columns,
) -> pd.DataFrame:
    """Inventory OSM source keys and prove how each key is preserved."""
    rows = []
    feature_n = int(len(gdf))
    for col in _source_osm_tag_columns(source_columns):
        if col not in gdf.columns:
            continue
        present_n = int(
            gdf[col].apply(lambda v: _json_safe_osm_tag_value(v) is not None).sum()
        )
        if present_n == 0:
            continue
        explicit = col in PROTECT_COLS
        rows.append(
            {
                "tag_key": col,
                "feature_count": present_n,
                "feature_ratio": (present_n / feature_n) if feature_n else 0.0,
                "schema_status": "EXPLICIT_COLUMN" if explicit else "EXTRA_JSON",
                "explicit_column": bool(explicit),
                "preserved_in_extra_json": bool(not explicit),
    ...
    if not rows:
        return pd.DataFrame(
            columns=[
                "tag_key", "feature_count", "feature_ratio", "schema_status",
                "explicit_column", "preserved_in_extra_json", "preservation_status",
            ]
        )
    return pd.DataFrame(rows).sort_values(
        ["schema_status", "feature_count", "tag_key"],
...
def _new_qa_map(center, zoom_start: int) -> folium.Map:
    """Create a map with a compact basemap selector separate from overlay controls."""
    m = folium.Map(location=[center.y, center.x], zoom_start=zoom_start, tiles=None, control_scale=True)
    folium.TileLayer("CartoDB positron", name="Light basemap", show=True, control=True).add_to(m)
    folium.TileLayer("OpenStreetMap", name="OpenStreetMap basemap", show=False, control=True).add_to(m)
    return m
...
def _folium_outline(
    ...
    *,
    ...
    if geometry is None or getattr(geometry, "is_empty", True):
        return None
    style = {"color": color, "weight": weight, "fillOpacity": 0.0, "opacity": 0.9}
    if dash_array:
...
def _compact_gdf_for_map(gdf: gpd.GeoDataFrame, extra_fields: list[str] | None = None) -> gpd.GeoDataFrame:
    """Keep map HTML readable by dropping nonessential columns from embedded GeoJSON."""
    if gdf is None or gdf.empty:
        return gdf
    preferred = [
        "osm_type", "osm_id", "@id", "name", "highway", "highway_norm", "route_role",
        "surface", "surface:material", "tracktype", "smoothness", "sac_scale",
        "trail_visibility", "incline", "step_count", "bridge", "ford",
        "safety_rope", "assisted_trail", "handrail", "rungs", "via_ferrata_scale",
        "barrier", "hazard", "natural", "waterway", "tourism", "information",
        "emergency", "amenity", "man_made", "route_relation_name", "route_relation_ref",
        "route_relation_network", "route_relation_osmc_symbol",
    ]
    if extra_fields:
        preferred.extend(extra_fields)
    cols = []
...
def _geojson_tooltip(gdf: gpd.GeoDataFrame, fields: list[str] | None = None):
    if gdf is None or gdf.empty:
        return None
    wanted = fields or [
        "name", "highway", "surface", "surface:material", "tracktype", "smoothness",
        "sac_scale", "incline", "step_count", "route_relation_name", "route_relation_ref",
        "barrier", "hazard", "osm_type", "osm_id", "@id",
    ]
    present = [f for f in wanted if f in gdf.columns]
    if not present:
        return None
    aliases = [f"{f}:" for f in present]
    return folium.GeoJsonTooltip(fields=present, aliases=aliases, sticky=False, localize=True)


def _add_gdf_map_layer(
    m: folium.Map,
    gdf: gpd.GeoDataFrame,
    *,
    name: str,
    ...
    if gdf is None or gdf.empty:
        return None
    sample = gdf[gdf.geometry.notna()].copy()
    if sample.empty:
        return None
    sample = _compact_gdf_for_map(sample, tooltip_fields)
    tooltip = _geojson_tooltip(sample, tooltip_fields)
    layer = folium.GeoJson(
        sample,
        name=name,
        show=show,
        ...
        highlight_function=lambda _feature: {"weight": max(weight + 2, 4), "opacity": 1.0},
        tooltip=tooltip,
    )
    layer.add_to(m)
    return layer
...
def _add_grouped_controls(m: folium.Map, groups: dict[str, list], *, collapsed: bool = False) -> None:
    clean = {name: [layer for layer in layers if layer is not None] for name, layers in groups.items()}
    clean = {name: layers for name, layers in clean.items() if layers}
    if GroupedLayerControl is not None and clean:
        GroupedLayerControl(
            groups=clean,
        ...
        # GroupedLayerControl marks grouped overlays as control=False, leaving this
        # ordinary LayerControl as a compact basemap-only selector.
...
def _layer_group_name(layer_name: str) -> str:
    groups = {
        "highway": "02 道路與路線關係",
        "hiking_route": "02 道路與路線關係",
        "foot_route": "02 道路與路線關係",
        "safety_rope": "04 技術設施與障礙",
        "assisted_trail": "04 技術設施與障礙",
        "handrail": "04 技術設施與障礙",
        "rungs": "04 技術設施與障礙",
        "ladder": "04 技術設施與障礙",
        "via_ferrata": "04 技術設施與障礙",
        "barrier": "04 技術設施與障礙",
        "obstacle": "04 技術設施與障礙",
        "overgrown": "04 技術設施與障礙",
        "cliff": "05 地形、水文與天然風險",
        "scree": "05 地形、水文與天然風險",
        "bare_rock": "05 地形、水文與天然風險",
        "landslide": "05 地形、水文與天然風險",
        "hazard": "05 地形、水文與天然風險",
        "waterway": "05 地形、水文與天然風險",
        "water_area": "05 地形、水文與天然風險",
        "wetland": "05 地形、水文與天然風險",
        "waterfall": "05 地形、水文與天然風險",
        "volcano": "05 地形、水文與天然風險",
        "trailhead": "06 導航與登山資訊",
        "peak": "06 導航與登山資訊",
        "guidepost": "06 導航與登山資訊",
        "visitor_centre": "06 導航與登山資訊",
        "information_office": "06 導航與登山資訊",
        "route_information": "06 導航與登山資訊",
        "route_marker": "06 導航與登山資訊",
        "board_map": "06 導航與登山資訊",
        "guide_map_attraction": "06 導航與登山資訊",
        "summit_board": "06 導航與登山資訊",
        "milestone": "06 導航與登山資訊",
        "survey_point": "06 導航與登山資訊",
        "shelter": "07 休憩、補給與旅遊",
        "alpine_hut": "07 休憩、補給與旅遊",
        "wilderness_hut": "07 休憩、補給與旅遊",
        "bench": "07 休憩、補給與旅遊",
        "picnic_table": "07 休憩、補給與旅遊",
        "picnic_site": "07 休憩、補給與旅遊",
        "drinking_water": "07 休憩、補給與旅遊",
        "toilets": "07 休憩、補給與旅遊",
        "viewpoint": "07 休憩、補給與旅遊",
        "tourism": "07 休憩、補給與旅遊",
        "spring": "07 休憩、補給與旅遊",
        "hot_spring": "07 休憩、補給與旅遊",
        "warning_notice": "08 緊急安全與基礎設施",
        "emergency": "08 緊急安全與基礎設施",
        "defibrillator": "08 緊急安全與基礎設施",
        "fire_hydrant": "08 緊急安全與基礎設施",
        "monitoring_station": "08 緊急安全與基礎設施",
        "street_lamp": "08 緊急安全與基礎設施",
        "man_made": "09 管制與其他背景",
        "protected_area": "09 管制與其他背景",
        "national_park": "09 管制與其他背景",
        "generic_context": "09 管制與其他背景",
    }
    return groups.get(layer_name, "09 管制與其他背景")


def _layer_style(layer_name: str) -> tuple[str, int]:
    if layer_name in {"highway", "hiking_route", "foot_route"}:
        return "#1565C0", 3
    if layer_name in {"safety_rope", "assisted_trail", "handrail", "rungs", "ladder", "via_ferrata", "barrier", "obstacle", "overgrown"}:
        return "#8E24AA", 4
    if layer_name in {"cliff", "scree", "bare_rock", "landslide", "hazard", "waterway", "water_area", "wetland", "waterfall", "volcano"}:
        return "#6D4C41", 3
    if layer_name in {"warning_notice", "emergency", "defibrillator", "fire_hydrant", "monitoring_station", "street_lamp"}:
        return "#C62828", 4
    if layer_name in {"trailhead", "peak", "guidepost", "route_information", "route_marker", "board_map", "guide_map_attraction", "summit_board", "milestone", "survey_point", "visitor_centre", "information_office"}:
        return "#00838F", 3
    if layer_name in {"shelter", "alpine_hut", "wilderness_hut", "bench", "picnic_table", "picnic_site", "drinking_water", "toilets", "viewpoint", "tourism", "spring", "hot_spring"}:
        return "#2E7D32", 3
    return "#546E7A", 2
...
def write_route_analysis_map(
    ...
    *,
    ...
    """Human-facing route-analysis map with grouped, quiet-by-default layer controls."""
    center = route_line.centroid
    m = _new_qa_map(center, zoom_start=15)
    groups: dict[str, list] = {}

    def add(group: str, layer) -> None:
        if layer is not None:
            groups.setdefault(group, []).append(layer)

    add("01 路線與範圍", _folium_outline(
        m, tile_fetch_geometry,
        name="OSM TILE_FULL fetch extent (+buffer)", color="#7B1FA2", weight=2,
        dash_array="8 6", show=False,
    ))
    add("01 路線與範圍", _folium_outline(
        m, tile_theoretical_geometry,
        name="NLSC theoretical tile extent", color="#1565C0", weight=3, show=False,
    ))
    add("01 路線與範圍", _folium_outline(
        m, analysis_geometry,
        name="ROUTE_ANALYSIS_SUBSET extent", color="#FB8C00", weight=3,
        dash_array="5 5", show=False,
    ))
    gpx_layer = folium.GeoJson(
        route_line,
        name="Activity GPX",
        show=True,
        style_function=lambda _: {"color": "#D32F2F", "weight": 5, "opacity": 0.95},
    ).add_to(m)
    add("01 路線與範圍", gpx_layer)

    # Primary stored layers. Only the highway network is enabled by default; all
    # secondary semantic layers are opt-in so the map opens readable rather than saturated.
    for layer_name, gdf in layers.items():
        if gdf is None or gdf.empty:
            continue
        color, weight = _layer_style(layer_name)
        label = f"{layer_name} ({len(gdf)})"
        layer = _add_gdf_map_layer(
            m,
            gdf,
            name=label,
            color=color,
            show=(layer_name == "highway"),
            weight=weight,
        )
        add(_layer_group_name(layer_name), layer)

    # Display-only highway attribute overlays answer common QA questions without
    # changing any persisted IA1 layer or downstream matching input.
    highway = layers.get("highway")
    if highway is not None and not highway.empty:
        thematic_specs = [
            ("階梯 steps", lambda x: x.get("highway_norm", x.get("highway", pd.Series(index=x.index, dtype=object))).astype(str).str.lower().eq("steps"), "#D32F2F"),
            ("有 surface 標記", lambda x: x["surface"].notna() if "surface" in x.columns else pd.Series(False, index=x.index), "#00897B"),
            ("有 smoothness 標記", lambda x: x["smoothness"].notna() if "smoothness" in x.columns else pd.Series(False, index=x.index), "#5E35B1"),
            ("有 tracktype 標記", lambda x: x["tracktype"].notna() if "tracktype" in x.columns else pd.Series(False, index=x.index), "#6D4C41"),
            ("有 sac_scale 標記", lambda x: x["sac_scale"].notna() if "sac_scale" in x.columns else pd.Series(False, index=x.index), "#F4511E"),
            ("有 incline 標記", lambda x: x["incline"].notna() if "incline" in x.columns else pd.Series(False, index=x.index), "#3949AB"),
        ]
        for label, mask_fn, color in thematic_specs:
            try:
                mask = mask_fn(highway)
                subset = highway[mask.fillna(False)].copy()
            except Exception:
                subset = highway.iloc[0:0].copy()
            if subset.empty:
                continue
            layer = _add_gdf_map_layer(
                m,
                ...
                weight=5,
                tooltip_fields=[
                    "name", "highway", "surface", "surface:material", "tracktype",
                    "smoothness", "sac_scale", "trail_visibility", "incline", "step_count",
                    "osm_type", "osm_id",
                ],
            )
            add("03 路面與通行屬性", layer)

    panel = """
    <div style="position:fixed; left:10px; bottom:22px; z-index:9998; background:white;
                border:1px solid #777; border-radius:5px; padding:7px 9px;
                font-size:12px; line-height:1.35; box-shadow:0 1px 4px rgba(0,0,0,.25);">
      <b>IA1 map layers</b><br>
      Default: GPX + highway only<br>
      Use grouped control to inspect surface / steps / hazards / POI.<br>
      Map overlays are QA views; GeoJSON outputs remain authoritative.
    </div>
    """
    m.get_root().html.add_child(folium.Element(panel))
    _add_grouped_controls(m, groups, collapsed=False)
    m.save(out_fp)


def write_route_subset_connectivity_qa_map(
    route_line: LineString,
    highway_gdf: gpd.GeoDataFrame,
    connectivity_qa: dict,
    query_geometry,
    query_geometry_qa: dict,
    out_fp: Path,
    args: argparse.Namespace,
) -> None:
    """Visualize IA1 route-subset sanity QA without changing QA decisions."""
    center = route_line.centroid
    m = _new_qa_map(center, zoom_start=15)
    groups: dict[str, list] = {}
    ...
    route_gdf = gpd.GeoDataFrame([{"geometry": route_line}], geometry="geometry", crs="EPSG:4326")
    metric_crs = route_gdf.estimate_utm_crs()
    route_m = route_gdf.to_crs(metric_crs).geometry.iloc[0]

    qfix = query_geometry_qa.get("query_geometry_fix_qa") or {}
    strategy = str(qfix.get("strategy") or "DIRECT_ROUTE_BUFFER")
    coords = list(route_m.coords)
    if strategy == "DISSOLVED_SEGMENT_FALLBACK":
        segments = [
            LineString([coords[i], coords[i + 1]])
            for i in range(len(coords) - 1)
            if coords[i] != coords[i + 1]
        ]
        query_centerline_m = unary_union(segments)
    else:
        query_centerline_m = route_m
    raw_query_buffer_m = query_centerline_m.buffer(args.buffer_m)
    raw_query_buffer_wgs = gpd.GeoSeries([raw_query_buffer_m], crs=metric_crs).to_crs("EPSG:4326").iloc[0]

    add("01 Query geometry", _folium_outline(
        m, raw_query_buffer_wgs,
        name=f"Production raw query buffer ({args.buffer_m:g} m)", color="#FB8C00",
        weight=2, dash_array="8 5", show=False,
    ))
    add("01 Query geometry", _folium_outline(
        m, query_geometry,
        name=f"Production simplified query geometry ({args.buffer_simplify_m:g} m simplify)",
        color="#F9A825", weight=3, dash_array="4 4", show=False,
    ))
    gpx_layer = folium.GeoJson(
        route_line,
        name="Activity GPX",
        show=True,
        style_function=lambda _: {"color": "#D32F2F", "weight": 5},
    ).add_to(m)
    add("02 Route & QA samples", gpx_layer)

    lines = highway_gdf[highway_gdf.geometry.notna()].copy()
    lines = lines[lines.geometry.geom_type.isin(["LineString", "MultiLineString"])]
    network_geom = None
    component_geoms: dict[int, object] = {}
    if not lines.empty:
        lines_m = lines.to_crs(metric_crs)
        network_geom = unary_union(list(lines_m.geometry))
        graph = nx.Graph()
        ...
        for ka, kb, geom in segment_records:
            comp_id = node_to_component.get(ka)
            if comp_id is not None:
                temp.setdefault(comp_id, []).append(geom)
        component_geoms = {comp_id: unary_union(geoms) for comp_id, geoms in temp.items()}

        palette = [
            "#1565C0", "#2E7D32", "#6A1B9A", "#00838F", "#5D4037",
            "#C62828", "#283593", "#558B2F", "#AD1457", "#4527A0",
        ]
        for comp_id, geom_m in sorted(component_geoms.items()):
            geom_wgs = gpd.GeoSeries([geom_m], crs=metric_crs).to_crs("EPSG:4326").iloc[0]
            color = palette[comp_id % len(palette)]
            layer = folium.GeoJson(
                mapping(geom_wgs),
                name=f"OSM highway component {comp_id}",
                show=True,
                style_function=lambda _feature, color=color: {"color": color, "weight": 3, "opacity": 0.85},
                tooltip=f"Component {comp_id}",
            ).add_to(m)
            add("03 OSM connectivity components", layer)

    if network_geom is not None and not network_geom.is_empty:
        step = max(5.0, float(args.qa_route_sample_m))
        n = max(2, int(math.ceil(route_m.length / step)) + 1)
        samples_m = [route_m.interpolate(min(i * step, route_m.length)) for i in range(n)]
        samples_wgs = gpd.GeoSeries(samples_m, crs=metric_crs).to_crs("EPSG:4326")
        sample_group = folium.FeatureGroup(name="GPX QA samples", show=True)
        for idx, (point_m, point_wgs) in enumerate(zip(samples_m, samples_wgs)):
            distance_m = float(point_m.distance(network_geom))
            near = distance_m <= float(args.qa_route_max_distance_m)
            folium.CircleMarker(
                location=[float(point_wgs.y), float(point_wgs.x)],
                radius=4,
                color="#2E7D32" if near else "#C62828",
                fill=True,
                fill_opacity=0.9,
                tooltip=f"sample={idx} distance_to_OSM={distance_m:.1f} m {'PASS' if near else 'FAIL'}",
            ).add_to(sample_group)
        sample_group.add_to(m)
        add("02 Route & QA samples", sample_group)

    endpoint_group = folium.FeatureGroup(name="Start / End", show=True)
    start = Point(route_line.coords[0])
    end = Point(route_line.coords[-1])
    folium.Marker(
        [start.y, start.x],
        tooltip=f"START | component={connectivity_qa.get('start_component')} | distance={connectivity_qa.get('start_distance_m')} m",
        icon=folium.Icon(color="green", icon="play"),
    ).add_to(endpoint_group)
    folium.Marker(
        [end.y, end.x],
        tooltip=f"END | component={connectivity_qa.get('end_component')} | distance={connectivity_qa.get('end_distance_m')} m",
        icon=folium.Icon(color="red", icon="stop"),
    ).add_to(endpoint_group)
    endpoint_group.add_to(m)
    add("02 Route & QA samples", endpoint_group)

    thresholds = connectivity_qa.get("thresholds") or {}
    status = str(connectivity_qa.get("status") or "UNKNOWN")
    status_color = "#2E7D32" if status == "PASS" else "#C62828"
    near_ratio = connectivity_qa.get("route_near_ratio")
    near_ratio_text = f"{float(near_ratio):.3f}" if near_ratio is not None else "N/A"
    start_d = connectivity_qa.get("start_distance_m")
    end_d = connectivity_qa.get("end_distance_m")
    start_d_text = f"{float(start_d):.1f} m" if start_d is not None else "N/A"
    end_d_text = f"{float(end_d):.1f} m" if end_d is not None else "N/A"

    panel = f"""
    <div style="position: fixed; top: 10px; left: 50px; z-index: 9999;
                background: white; padding: 10px 12px; border: 2px solid #555;
                border-radius: 6px; font-size: 12px; line-height: 1.4;
                max-width: 350px; box-shadow: 0 1px 5px rgba(0,0,0,0.35);">
      <div style="font-weight:700; font-size:14px; margin-bottom:5px;">ROUTE_SUBSET_CONNECTIVITY_QA</div>
      <div><b>Status:</b> <span style="color:{status_color}; font-weight:700;">{status}</span></div>
      <div><b>Reason:</b> {connectivity_qa.get('reason')}</div>
      <hr style="margin:5px 0;">
      <div><b>Samples:</b> {connectivity_qa.get('route_sample_count')} | <b>Near ratio:</b> {near_ratio_text} / {thresholds.get('near_ratio')}</div>
      <div><b>Near distance:</b> &le; {thresholds.get('max_distance_m')} m | <b>Components:</b> {connectivity_qa.get('component_count')}</div>
      <div><b>Start:</b> comp {connectivity_qa.get('start_component')}, {start_d_text}</div>
      <div><b>End:</b> comp {connectivity_qa.get('end_component')}, {end_d_text}</div>
      <div><b>Same component:</b> {connectivity_qa.get('start_end_same_component')}</div>
      <hr style="margin:5px 0;">
      <div><b>Query strategy:</b> {qfix.get('strategy')}</div>
      <div><b>Fallback:</b> {qfix.get('fallback_triggered')} | {qfix.get('fallback_reason') or 'NONE'}</div>
      <div><b>Direct contract:</b> {qfix.get('direct_contract_pass')}</div>
      <div><b>Outside raw / simplified:</b> {qfix.get('route_outside_raw_buffer_length_m')} / {qfix.get('route_outside_simplified_buffer_length_m')} m</div>
    </div>
    """
    m.get_root().html.add_child(folium.Element(panel))
    _add_grouped_controls(m, groups, collapsed=False)
...
def _tile_highway_family(value: object) -> str:
    v = str(value or "").strip().lower()
    if v in {"path", "footway", "steps", "track", "pedestrian", "bridleway"}:
        return "步道與徒步核心"
    if v in {"service", "residential", "living_street", "unclassified", "road"}:
        return "地方道路與接近道路"
    if v in {
        ...
        return "主要道路"
    if v in {"cycleway", "corridor", "platform", "construction", "proposed", "raceway"}:
        return "特殊或非徒步道路"
    return "其他 highway"
...
def write_tile_full_highway_map(
    ...
    *,
    ...
    """TILE_FULL QA map grouped by highway family instead of one overwhelming layer."""
    ...
    m = _new_qa_map(center, zoom_start=13)
    groups: dict[str, list] = {}
    ...
    add("01 TILE_FULL 範圍", _folium_outline(
        m, tile_fetch_geometry,
        name="OSM TILE_FULL fetch extent (+buffer)", color="#7B1FA2", weight=2,
        dash_array="8 6", show=False,
    ))
    add("01 TILE_FULL 範圍", _folium_outline(
        m, tile_theoretical_geometry,
        name="NLSC theoretical tile extent", color="#1565C0", weight=3, show=True,
    ))

    if highway_gdf is not None and not highway_gdf.empty:
        sample = highway_gdf[highway_gdf.geometry.notna()].copy()
        if not sample.empty:
            if "highway" not in sample.columns:
                sample["highway"] = "unknown"
            sample["_qa_highway_family"] = sample["highway"].map(_tile_highway_family)
            family_styles = {
                "步道與徒步核心": ("#2E7D32", 3, True),
                "地方道路與接近道路": ("#1565C0", 2, False),
                "主要道路": ("#616161", 2, False),
                "特殊或非徒步道路": ("#8E24AA", 2, False),
                "其他 highway": ("#8D6E63", 2, False),
            }
            for family in family_styles:
                subset = sample[sample["_qa_highway_family"].eq(family)].copy()
                if subset.empty:
                    continue
                color, weight, show = family_styles[family]
                layer = _add_gdf_map_layer(
                    m,
                    subset,
                    name=f"{family} ({len(subset)})",
                    color=color,
                    show=show,
                    weight=weight,
                    tooltip_fields=["name", "highway", "surface", "tracktype", "smoothness", "sac_scale", "@id", "osm_id"],
                )
                add("02 TILE_FULL highway", layer)

            # High-value thematic overlays for inspecting road-surface metadata.
            thematic = [
                ("steps", sample["highway"].astype(str).str.lower().eq("steps"), "#D32F2F"),
                ("surface tagged", sample["surface"].notna() if "surface" in sample.columns else pd.Series(False, index=sample.index), "#00897B"),
                ("smoothness tagged", sample["smoothness"].notna() if "smoothness" in sample.columns else pd.Series(False, index=sample.index), "#5E35B1"),
                ("sac_scale tagged", sample["sac_scale"].notna() if "sac_scale" in sample.columns else pd.Series(False, index=sample.index), "#F4511E"),
            ]
            for label, mask, color in thematic:
                subset = sample[mask.fillna(False)].copy()
                if subset.empty:
                    continue
                layer = _add_gdf_map_layer(
                    m,
                    ...
                    show=False,
                    weight=4,
                    tooltip_fields=["name", "highway", "surface", "tracktype", "smoothness", "sac_scale", "step_count", "@id", "osm_id"],
                )
                add("03 TILE_FULL 路面與難度屬性", layer)

    panel = """
    <div style="position:fixed; left:10px; bottom:22px; z-index:9998; background:white;
                border:1px solid #777; border-radius:5px; padding:7px 9px;
                font-size:12px; line-height:1.35; box-shadow:0 1px 4px rgba(0,0,0,.25);">
      <b>TILE_FULL highway QA</b><br>
      Default: NLSC extent + hiking/walking core.<br>
      Other road families and surface attributes are opt-in.<br>
      This map is display-only; the GeoJSON/PBF remain authoritative.
    </div>
    """
    m.get_root().html.add_child(folium.Element(panel))
    _add_grouped_controls(m, groups, collapsed=False)
...
def main() -> None:
    ...
    osm_raw_root = resolve_path(args.osm_raw_root)
    compatibility_index_enabled = bool(args.allow_legacy_index_fallback)
    dataset_index_needed = (
        compatibility_index_enabled
        or args.source == "overpass"
        or bool(args.allow_overpass_fallback)
        or bool(args.sync_taiwan_pbf)
        or bool(args.pbf_fp)
    )
    if dataset_index_needed:
        data_dir.mkdir(parents=True, exist_ok=True)
    nlsc_raw_dir = resolve_path(args.nlsc_raw_dir)
    nlsc_tile_index_fp = (
        resolve_path(args.nlsc_tile_index)
        if args.nlsc_tile_index
        else osm_raw_root / "_index" / "nlsc_tile_index.csv"
    )
    # v1.5.7: the default derived-output location is resolved only after GPX -> NLSC tile authority.
    # An explicit --out-dir remains a supported compatibility override.
    explicit_out_dir = resolve_path(args.out_dir) if args.out_dir else None
    ...
    # Source Store v2 storage contract:
    # - NLSC/TILE_FULL owns reusable OSM source authority under <osm-raw-root>/<tile>/current.json.
    # - GPX/case owns only derived route-analysis artifacts under outputs/ia1_route_analysis.
    # Tile identity is lineage metadata, not a parent directory for derived artifacts.
    tile_token = "__".join(nlsc_tile_ids)
    route_output_root = resolve_path(args.route_output_root)
    out_dir = explicit_out_dir or (route_output_root / case_id)
    out_dir.mkdir(parents=True, exist_ok=True)
    query_geometry, query_geometry_self_qa = build_query_geometry_with_self_qa(
        route_line,
        args.buffer_m,
        args.buffer_simplify_m,
        out_dir=out_dir,
    )
    ...
    print("OUTPUT_LAYOUT: ROUTE_DERIVED_SEPARATE")
    print("SOURCE_TILE_TOKEN:", tile_token)
    print("OSM_SOURCE_STORE_ROOT:", osm_raw_root)
    print("ROUTE_ANALYSIS_OUT_DIR:", out_dir)
    print_query_geometry_self_qa(query_geometry_self_qa)

    source_store_entry = None
    if args.source in {"auto", "local"}:
        source_store_entry = load_osm_source_store_v2_current(
            nlsc_tile_ids,
            osm_raw_root,
            required=not args.allow_legacy_index_fallback,
        )
        if source_store_entry is not None:
            print_osm_source_store_v2(source_store_entry, osm_raw_root)
        elif args.allow_legacy_index_fallback:
            print(
                "WARNING: Source Store v2 current.json unavailable; explicit legacy index fallback enabled."
            )
    ...
    # ------------------------------------------------------------------
    # Phase 1: configure local parsers. Normal local/auto runs are Source
    # Store v2 only and do not scan data/osm indexes or legacy IA1 bundles.
    # ------------------------------------------------------------------
    index = {"schema": DATASET_INDEX_SCHEMA, "updated_at": fetched_at, "datasets": []}
    ...
    print("source order: GPX -> NLSC tile authority -> OSM Source Store v2 current.json -> CURRENT raw/semantic")
    if compatibility_index_enabled:
        print("compatibility fallback: ENABLED (legacy data/osm index and IA1 bundles may be scanned)")
    else:
        print("compatibility fallback: DISABLED (legacy data/osm index and IA1 bundles not scanned)")
    ...
    print("NLSC tile authority index:", nlsc_tile_index_fp)

    indexed_analysis_entry = None
    indexed_tile_entry = None
    indexed_analysis_state = "NOT_SCANNED_SOURCE_STORE_V2_NORMAL_PATH"
    indexed_tile_state = "NOT_SCANNED_SOURCE_STORE_V2_NORMAL_PATH"
    legacy_selected = None
    legacy_rows: list[dict] = []

    if dataset_index_needed:
        index = load_dataset_index(index_fp)
        if compatibility_index_enabled or args.sync_taiwan_pbf or args.pbf_fp:
    ...
    if compatibility_index_enabled:
        print("legacy dataset index:", index_fp)
    ...
    # ------------------------------------------------------------------
    # Phase 2: choose an existing local source. Normal local/auto uses only
    # OSM Source Store v2 current.json. Legacy index/bundle fallback is an
    # explicit compatibility path.
    # ------------------------------------------------------------------
    if args.source in {"auto", "local"}:
        if source_store_entry is not None:
            selected_entry = source_store_entry
            coverage_decision = "TILE_FULL"
            print(
                f"LOCAL CURRENT TILE_FULL: {selected_entry.get('dataset_id')} "
                f"kind={selected_entry.get('kind')}"
            )
            raw, route_relation_memberships, route_relation_parse_available = load_features_from_local_dataset(
                selected_entry, query_geometry, data_dir, args
            )
            source_info = source_descriptor(
                selected_entry, source_kind="osm_source_store_v2"
            )
        elif compatibility_index_enabled and (indexed_tile_entry is not None or indexed_analysis_entry is not None):
            ...
            print(
                f"LOCAL LEGACY INDEX FALLBACK {coverage_decision}: {selected_entry.get('dataset_id')} "
                f"kind={selected_entry.get('kind')}"
            )
            ...
            source_info = source_descriptor(selected_entry, source_kind="local_pbf_legacy_index")
        elif compatibility_index_enabled and legacy_selected is not None:
        ...
        elif compatibility_index_enabled:
        ...
        if selected_entry is None and selected_legacy is None:
            if args.source == "local":
                raise RuntimeError(
                    "--source local requested, but OSM Source Store v2 current.json did not provide TILE_FULL. "
                    "Legacy index/bundle fallback is disabled unless --allow-legacy-index-fallback is provided. "
                    "No network request was sent."
                )
            if not args.allow_overpass_fallback:
                raise RuntimeError(
                    "OSM Source Store v2 current.json did not provide local TILE_FULL. v1.5.9 normal policy is "
                    "current.json-first and fail-closed, so no network request was sent. To use old data/osm index "
                    "or legacy IA1 bundles, explicitly add --allow-legacy-index-fallback; for a small one-off query, explicitly add "
    ...
    # Capture exactly which attributes came from the source parser before IA1
    # adds identity, metadata, and normalized semantic fields.
    osm_source_columns = list(raw.columns)
    ...
    raw = add_metadata(
        ...
        osm_base_timestamp=source_info.get("osm_base_timestamp"),
        coverage_status="ROUTE_ANALYSIS_SUBSET",
    )
    raw["data_scope"] = "ROUTE_ANALYSIS_SUBSET"
    raw["source_coverage_status"] = coverage_decision
    raw["source_tile_ids"] = ";".join(nlsc_tile_ids)
    ...
    raw, extra_tag_columns = attach_osm_extra_tags(raw, osm_source_columns)

    tag_coverage_qa = build_osm_tag_coverage_qa(raw, osm_source_columns)
    tag_coverage_fp = out_dir / "osm_tag_coverage_qa.csv"
    tag_coverage_qa.to_csv(tag_coverage_fp, index=False, encoding="utf-8-sig")
    explicit_key_n = int(tag_coverage_qa["explicit_column"].sum()) if not tag_coverage_qa.empty else 0
    extra_key_n = int(tag_coverage_qa["preserved_in_extra_json"].sum()) if not tag_coverage_qa.empty else 0
    features_with_extra_n = int((raw["osm_extra_tag_count"] > 0).sum()) if len(raw) else 0
    print("OSM_TAG_PRESERVATION_QA")
    print("  source tag keys with values:", len(tag_coverage_qa))
    print("  explicit schema keys present:", explicit_key_n)
    print("  extra JSON keys present:", extra_key_n)
    print(f"  features carrying extra tags: {features_with_extra_n}/{len(raw)}")
    print("  extra tag columns:", ";".join(extra_tag_columns) if extra_tag_columns else "None")
    print("  coverage QA:", tag_coverage_fp)

    if args.write_route_analysis_source_geojson:
        route_source_fp = out_dir / "route_analysis_source_all.geojson"
        select_columns(raw).to_file(route_source_fp, driver="GeoJSON")
        print("ROUTE_ANALYSIS_SOURCE_DIAGNOSTIC:", route_source_fp)
        print("  features:", len(raw))
    ...
    tile_qa = (
        ensure_osm_source_store_v2_tile_qa(selected_entry, args, tile_matches)
        if selected_entry and selected_entry.get("kind") == "osm_source_store_v2"
        else write_tile_dataset_qa(selected_entry, args)
    )
        ...
        layer_summary.append(
            {
                ...
                "source_coverage_status": coverage_decision,
    ...
    raw_highway_line_count = int(
        (
            raw["highway"].notna()
            & raw.geometry.geom_type.isin(["LineString", "MultiLineString"])
        ).sum()
    ) if "highway" in raw.columns else 0
    emitted_highway_count = len(written_layers.get("highway", []))
    highway_conservation_status = "PASS" if raw_highway_line_count == emitted_highway_count else "FAIL"
    print("ROUTE_LAYER_CONSERVATION_QA")
    print("  raw highway line features:", raw_highway_line_count)
    print("  emitted highway layer features:", emitted_highway_count)
    print("  highway conservation:", highway_conservation_status)
    if highway_conservation_status != "PASS":
        raise RuntimeError(
            "ROUTE_HIGHWAY_LAYER_CONSERVATION_FAILED: highway-tagged line features were lost during layer construction."
        )
    ...
    dataset_manifest = {
        ...
        "map_outputs": {
            ...
            "route_subset_connectivity_qa_map": portable_path(out_dir / "osm_route_subset_connectivity_qa.html") if not args.no_map else None,
            "tile_full_highway_qa_map": tile_qa.get("optional_highway_map") if tile_qa else None,
        },
        "coverage_status": "ROUTE_ANALYSIS_SUBSET",
        "source_coverage_status": coverage_decision,
        "output_layout": "ROUTE_DERIVED_SEPARATE",
        "source_tile_token": tile_token,
        "source_store_root": portable_path(resolve_path(args.osm_raw_root)),
        "source_store_schema": (
            selected_entry.get("source_store_schema")
            if selected_entry and selected_entry.get("kind") == "osm_source_store_v2"
            else None
        ),
        "source_store_action": (
            selected_entry.get("source_store_action")
            if selected_entry and selected_entry.get("kind") == "osm_source_store_v2"
            else None
        ),
        "source_store_tiles": (
            selected_entry.get("source_store_tiles")
            if selected_entry and selected_entry.get("kind") == "osm_source_store_v2"
            else None
        ),
        "source_semantic_sha256": (
            selected_entry.get("semantic_sha256")
            if selected_entry and selected_entry.get("kind") == "osm_source_store_v2"
            else None
        ),
        "route_analysis_out_dir": portable_path(out_dir),
        "route_analysis_source_diagnostic": portable_path(out_dir / "route_analysis_source_all.geojson") if args.write_route_analysis_source_geojson else None,
        "query_geometry_self_qa": query_geometry_self_qa,
        "query_geometry_self_qa_geojson": portable_path(out_dir / "query_geometry_self_qa.geojson"),
        "query_geometry_root_cause_qa_geojson": portable_path(out_dir / "query_geometry_root_cause_qa.geojson"),
        "route_highway_layer_conservation": {
            "raw_highway_line_features": raw_highway_line_count,
            "emitted_highway_features": emitted_highway_count,
            "status": highway_conservation_status,
        },
        "script_version": VERSION,
        "script_status": STATUS,
        "attribution": OSM_ATTRIBUTION,
    ...
    if not args.no_map:
        ...
        connectivity_map_fp = out_dir / "osm_route_subset_connectivity_qa.html"
        write_route_subset_connectivity_qa_map(
            route_line,
            highway_layer,
            connectivity_qa,
            query_geometry,
            query_geometry_self_qa,
            connectivity_map_fp,
            args,
        )
        print("MAP_OUTPUTS")
        print("  ROUTE_ANALYSIS:", route_map_fp)
        print("  ROUTE_SUBSET_CONNECTIVITY_QA:", connectivity_map_fp)
    ...
    print("COVERAGE: ROUTE_ANALYSIS_SUBSET")
    print("SOURCE_COVERAGE:", coverage_decision)
    print("ROUTE_SUBSET_CONNECTIVITY_QA:", connectivity_qa.get("status"), connectivity_qa.get("reason"))
    print("CONNECTIVITY_QA:", connectivity_qa.get("status"), connectivity_qa.get("reason"))
    if selected_entry and selected_entry.get("kind") == "osm_source_store_v2":
        print("SOURCE_STORE_ACTION:", selected_entry.get("source_store_action"))
        print("SOURCE_STORE_ROOT:", resolve_path(args.osm_raw_root))
    print("DERIVED_ROUTE_ANALYSIS_DONE:", out_dir.resolve())

# Changelog - CH6.5.5 Route-Following Radar v1.1

## 2026-06-20

Documented the route-following radar v1.1 closure from proxy admission through governed limited proxy radar preview.

No analysis scripts were added in this documentation branch. No outputs were regenerated or modified.

## Closed Workstream

The completed chain is:

`route-following proxy admission -> axis contract patch -> data table patch -> radar plot v1.1`
...
- 18 baseline activities contain mid-activity route issue burden.
- 7 baseline activities contain only `terminal_artifact`; these are detector-covered and are not missing evidence.
...
It is not a formal ability score, ability rank, ability class, THCI score, final hiking risk score, route suitability score, or go/no-go decision.

`route_following_stability` is a limited proxy axis only.

`deviation_correction_ability` remains missing evidence because it requires a formal deviation-start -> correction/rejoin event-chain review. It must not be inferred or upgraded directly from route issue keywords.
